...
export class ShopcategoriesComponent implements OnInit {
 ...
 dealsOfWeek: Product[] = [];
  ...
  searchedProductId: number | null = null;
  ...
  ngOnInit() {
    ...
    // Single subscription for query params
    this.route.queryParams.subscribe(queryParams => {
      const query = queryParams['search'];
      const brand = queryParams['brand'];
      const parentCategory = queryParams['parentCategory'];
      const category = this.route.snapshot.paramMap.get('category'); // if using route param

      if (query) {
        this.filteredProducts = this.searchService.searchProducts(query);

        if (this.filteredProducts.length > 0) {
          this.searchedProductId = this.filteredProducts[0].id;
        }
      }
      else if (brand && parentCategory) {
        this.filteredProducts = this.products.filter(product =>
          product.brand?.toLowerCase() === brand.toLowerCase() &&
          product.parentCategory?.toLowerCase() === parentCategory.toLowerCase()
        );
      }
      else if (brand) {
        this.filteredProducts = this.products.filter(product =>
  ...
  // Add this function inside ShopcategoriesComponent
