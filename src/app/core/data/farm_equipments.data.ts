import { Product } from '../models/Product';

export const FARM_EQUIPMENTS_PRODUCTS: Product[] = [


    // start 25000

    {
        id: 25000,
        name: 'TOTAL Gasoline Tiller 196cc (TGC5001)',
        category: 'farm-machines',
        group: 'farm equipments',
        brand: 'TOTAL',
        price: 450000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306524/Astonic-Mart/Products/farm%20equipment/TOTAL_GASOLINE_TILLER_196cc_TGC5001.jpg',
        ],

        inStock: true,

        description: 'The TOTAL Gasoline Tiller 196cc (TGC5001) is a powerful agricultural machine designed for efficient soil cultivation, tilling, and land preparation. Equipped with a robust 196cc gasoline engine, it delivers high performance suitable for both small-scale farms and professional agricultural use.\n\nThis tiller is built to break up hard soil, mix nutrients, and prepare planting beds with ease. Its strong rotating tines penetrate the soil effectively, improving aeration and ensuring optimal conditions for crop growth.\n\nDesigned for durability and ease of use, it features sturdy construction, ergonomic handles, and smooth maneuverability. It is ideal for farmers, gardeners, and landscapers looking for a reliable and efficient tilling solution.',

        specification: `
  Model: TGC5001
  Brand: TOTAL
  Engine Type: Gasoline engine
  Engine Capacity: 196cc
  Power Output: High-performance engine
  Tilling Width: Approx. 500mm
  Tilling Depth: Adjustable
  Fuel Type: Petrol (Gasoline)
  Application: Soil cultivation, farming, gardening
  `,

        features: `
  Powerful 196cc gasoline engine for heavy-duty performance
  Efficient soil tilling and land preparation
  Strong rotating tines for deep soil penetration
  Adjustable tilling depth for different applications
  Durable construction for long-term use
  Ergonomic handle design for comfort and control
  Suitable for farming, gardening, and landscaping
  Easy to operate and maintain
  `
    },

    {
        id: 25001,
        name: 'Carbon Steel Garden Spade Shovel with Handle',
        category: 'farm-tools',
        group: 'farm equipments',
        brand: 'Generic',
        price: 18000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306516/Astonic-Mart/Products/farm%20equipment/Steel_Garden_Spade_Shovel_With_Handle.avif',
        ],

        inStock: true,

        description: 'The Carbon Steel Garden Spade Shovel with Handle is a durable and versatile agricultural tool designed for digging, lifting, and moving soil, sand, and other materials. Made from high-quality carbon steel, the blade offers excellent strength, sharpness, and resistance to wear, making it suitable for tough farming and gardening tasks.\n\nThe shovel features a sturdy handle that provides a comfortable grip and better control during use. Its ergonomic design reduces strain, allowing for extended use without discomfort. The strong blade edge easily penetrates soil, making digging and planting more efficient.\n\nIdeal for farmers, gardeners, and construction workers, this spade shovel is a reliable tool for land preparation, planting, and general outdoor work.',

        specification: `
  Material: Carbon steel blade
  Handle Material: Wood / reinforced metal (varies)
  Blade Type: Flat / slightly curved spade
  Length: Standard full-length handle
  Grip: Ergonomic handle design
  Application: Digging, lifting, soil preparation, gardening
  `,

        features: `
  Strong carbon steel blade for durability and long life
  Sharp edge for easy soil penetration
  Sturdy handle for better control and comfort
  Suitable for farming, gardening, and construction tasks
  Ergonomic design reduces user fatigue
  Multi-purpose tool for digging and lifting materials
  Weather-resistant and reliable performance
  Easy to use and maintain
  `
    },


    {
        id: 25002,
        name: 'Knapsack Sprayer 3WF-3A-24L Tank Petrol Engine',
        category: 'knapsack-sprayers',
        group: 'Farm Equipments',

        brand: 'Generic',

        price: 255000,
        discountPrice: 240000,

        power: '2-Stroke Petrol Engine',

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306502/Astonic-Mart/Products/farm%20equipment/knapsack_sprayers_24l.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306503/Astonic-Mart/Products/farm%20equipment/knapsack_sprayers_24l-3.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306505/Astonic-Mart/Products/farm%20equipment/knapsack_sprayers_24l-4.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306506/Astonic-Mart/Products/farm%20equipment/knapsack_sprayers_24l-5.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306508/Astonic-Mart/Products/farm%20equipment/knapsack_sprayers_24l-6.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306509/Astonic-Mart/Products/farm%20equipment/knapsack_sprayers_24l-7.webp'
        ],

        inStock: true,

        jumiaLink: '',
        kongaLink: '',

        description: `
Knapsack Sprayer 3WF-3A Agriculture Mist Duster is a powerful petrol-powered agricultural spraying machine designed for pesticide application, fertilizing, crop protection, pest control, and disinfection operations. It is suitable for farms, plantations, orchards, gardens, greenhouses, and agricultural field applications.
Equipped with a high-performance 2-stroke petrol engine, the mist duster delivers strong spraying pressure and long-range mist coverage for efficient application of liquid chemicals and powder substances. The large 24-liter tank capacity allows extended working time with fewer refills, improving productivity during farm operations.
Its ergonomic backpack design provides comfort during prolonged use, while the durable construction ensures reliable performance in demanding agricultural environments. The 3WF-3A is ideal for spraying insecticides, herbicides, fungicides, fertilizers, and sanitation chemicals.
`,

        specification: `
  - Product Name: Agriculture Mist Duster Knapsack Sprayer
  - Model: 3WF-3A
  - Engine Type: 2-Stroke Petrol Engine
  - Power Output: Approx. 1.5kW - 2.13kW
  - Tank Capacity: 24 Liters
  - Fuel Tank Capacity: Approx. 1.2L - 2.3L
  - Fuel Mixture Ratio: 25:1
  - Starting System: Recoil/Kick Start
  - Spray Range: Up to 12-20 Meters
  - Spray Type: Mist and Dust Spraying
  - Usage: Agriculture, Pest Control, Fertilizer and Chemical Spraying
  - Carry Type: Backpack/Knapsack Design
  `,

        features: `
  - Powerful petrol engine performance
  - Large 24L chemical tank capacity
  - Long-range mist spraying operation
  - Suitable for liquid and powder application
  - Ergonomic backpack design for comfort
  - Durable construction for agricultural use
  - Efficient pesticide and fertilizer application
  - Easy recoil starting system
  - Ideal for farms, orchards, and plantations
  `
    },

    {
        id: 25004,
        name: 'Force-Up Glyphosate Herbicide 1L',
        category: 'agro-chemicals',
        group: 'Farm Equipments',


        brand: 'Force-Up',

        price: 8500,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306493/Astonic-Mart/Products/farm%20equipment/force_up_chemicals.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306493/Astonic-Mart/Products/farm%20equipment/force_up_chemicals.jpg-2.jpg'
        ],

        inStock: true,

        jumiaLink: '',
        kongaLink: '',

        description: `
Force-Up Glyphosate Herbicide 1L is a non-selective systemic herbicide designed for effective control of annual and perennial weeds in agricultural fields, plantations, gardens, and industrial areas. It is widely used for land preparation, weed control around crops, and vegetation management.
The herbicide is absorbed through the leaves and transported throughout the plant to eliminate weeds from the roots. It is suitable for controlling grasses, broadleaf weeds, and unwanted vegetation in farms, pathways, fence lines, and non-crop areas.
Force-Up Glyphosate Herbicide provides fast and effective weed control while helping farmers maintain cleaner and more productive farmland.
`,

        specification: `
  - Product Name: Glyphosate Herbicide
  - Brand: Force-Up
  - Capacity: 1 Liter
  - Chemical Type: Systemic Non-Selective Herbicide
  - Active Ingredient: Glyphosate
  - Formulation Type: Liquid Concentrate
  - Application Method: Knapsack Sprayer or Boom Sprayer
  - Target: Grasses and Broadleaf Weeds
  - Usage: Farm Weed Control, Land Preparation, Vegetation Management
  - Suitable Areas: Farms, Plantations, Gardens, Industrial Areas
  `,

        features: `
  - Effective control of annual and perennial weeds
  - Fast absorption through leaves and stems
  - Kills weeds from root level
  - Suitable for pre-planting land preparation
  - Easy application with sprayers
  - Ideal for farms and agricultural use
  - Reliable vegetation management solution
  - Suitable for grasses and broadleaf weeds
  - Helps improve farm productivity
  `
    },

    {
        id: 25005,
        name: '16L Knapsack Sprayer',
        category: 'knapsack-sprayers',
        group: 'Farm Machines',

        brand: 'Generic',

        price: 33000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306513/Astonic-Mart/Products/farm%20equipment/manual_knapsack_sprayer.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306514/Astonic-Mart/Products/farm%20equipment/manual_knapsack_sprayer-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306515/Astonic-Mart/Products/farm%20equipment/manual_knapsack_sprayer-3.jpg'
        ],

        inStock: true,

        jumiaLink: '',
        kongaLink: '',

        description: `
16L Knapsack Sprayer is a manual agricultural spraying equipment designed for applying pesticides, herbicides, fungicides, disinfectants, fertilizers, and other liquid solutions. It is widely used on farms, gardens, plantations, greenhouses, lawns, and landscaping projects.
The sprayer features a durable chemical-resistant tank, adjustable spray lance, comfortable shoulder straps, and a manual pumping system that delivers consistent spraying performance for crop protection and maintenance tasks.
`,

        specification: `
  - Product Name: Knapsack Sprayer
  - Tank Capacity: 16 Liters
  - Type: Manual Pressure Sprayer
  - Tank Material: High-Density Polyethylene (HDPE)
  - Lance Material: Stainless Steel
  - Hose Length: Approx. 1.2m
  - Spray Nozzles: Adjustable Nozzle
  - Pump Type: Piston Pump
  - Working Pressure: 2-4 Bar
  - Carrying System: Adjustable Shoulder Straps
  - Application: Agriculture, Gardening, Pest Control and Disinfection
  `,

        features: `
  - Large 16-liter tank capacity
  - Durable chemical-resistant tank
  - Adjustable spray pattern nozzle
  - Comfortable backpack design
  - Strong piston pumping system
  - Lightweight and easy to operate
  - Suitable for pesticides and herbicides
  - Leak-resistant construction
  - Easy filling and cleaning
  - Ideal for farms and gardens
  `
    },


    {
        id: 25006,
        name: 'Comfortable Garden 4-Stroke Knapsack Sprayer (3WZ-768)',
        category: 'knapsack-sprayers',
        group: 'Farm Machines',

        brand: 'Comfortable Garden',

        price: 300000,
        discountPrice: 280000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306491/Astonic-Mart/Products/farm%20equipment/comfort_garden_knapsack_sprayer.png'
        ],

        inStock: true,

        jumiaLink: '',
        kongaLink: '',

        description: `
Comfortable Garden 4-Stroke Knapsack Sprayer (3WZ-768) is a petrol-powered agricultural sprayer designed for large-scale spraying of pesticides, herbicides, fungicides, liquid fertilizers, and disinfectants. It is ideal for farms, plantations, orchards, gardens, greenhouses, and commercial agricultural operations.
Powered by a fuel-efficient 4-stroke engine, the sprayer delivers high spraying pressure and long operating time while reducing operator fatigue compared to manual sprayers.
`,

        specification: `
  - Product Name: Motorized Knapsack Sprayer
  - Model: 3WZ-768
  - Brand: Comfortable Garden
  - Engine Type: 4-Stroke Gasoline Engine
  - Tank Capacity: 25 Liters
  - Engine Capacity: 31cc - 35cc
  - Fuel Type: Petrol
  - Starting System: Recoil Start
  - Working Pressure: 15 - 25 Bar
  - Spray Distance: Up to 8 Meters
  - Flow Rate: 3 - 8 L/min
  - Fuel Tank Capacity: 0.7 Liters
  - Lance Material: Stainless Steel
  - Application: Agriculture, Pest Control and Disinfection


      accessories: 
  - 1 x Motorized Knapsack Sprayer
  - 1 x Spray Lance
  - 1 x Spray Hose
  - Assorted Spray Nozzles
  - Tool Kit
  - User Manual
  
  `,



        features: `
  - Powerful 4-stroke gasoline engine
  - Large-capacity chemical tank
  - High spraying pressure
  - Fuel-efficient operation
  - Easy recoil starting system
  - Long spraying range
  - Durable chemical-resistant tank
  - Comfortable shoulder harness
  - Suitable for commercial farming
  - Ideal for pesticides, herbicides and fertilizers
  `
    },


    {
        id: 25007,
        name: 'Aspero 2-Stroke Knapsack Power Sprayer',
        category: 'knapsack-sprayers',
        group: 'Farm Machines',

        brand: 'Aspero',

        price: 270000,
        discountPrice: 250000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306483/Astonic-Mart/Products/farm%20equipment/apero_knapsack_sprayer.png'
        ],

        inStock: true,

        jumiaLink: '',
        kongaLink: '',

        description: `
Aspero 2-Stroke Knapsack Power Sprayer is a petrol-powered agricultural spraying machine designed for efficient application of pesticides, herbicides, fungicides, liquid fertilizers, and disinfectants. It is suitable for farms, plantations, orchards, gardens, greenhouses, and agricultural maintenance operations.
Powered by a compact 2-stroke gasoline engine, the sprayer provides strong spraying pressure, long spraying range, and improved productivity compared to manual sprayers.
`,

        specification: `
  - Product Name: Knapsack Power Sprayer
  - Brand: Aspero
  - Engine Type: Air-Cooled 2-Stroke Gasoline Engine
  - Engine Capacity: 25.6cc
  - Tank Capacity: 20 Liters
  - Fuel Type: Petrol + 2T Oil Mixture
  - Fuel Tank Capacity: 0.6 Liters
  - Starting System: Recoil Starter
  - Working Pressure: 15 - 25 Bar
  - Flow Rate: 3 - 8 L/min
  - Spray Distance: Up to 8 Meters
  - Lance Material: Stainless Steel
  - Carrying Type: Backpack / Knapsack
  - Application: Agriculture, Pest Control, Fertilizer and Disinfection

    accessories: 
  - 1 x Aspero Power Sprayer
  - 1 x Spray Lance
  - 1 x Spray Hose
  - Assorted Spray Nozzles
  - Tool Kit
  - User Manual
  `,




        features: `
  - Powerful 2-stroke engine
  - High spraying pressure output
  - Large-capacity chemical tank
  - Lightweight backpack design
  - Easy recoil starting system
  - Long spraying range
  - Durable chemical-resistant tank
  - Suitable for pesticides and herbicides
  - Faster coverage than manual sprayers
  - Ideal for commercial farming operations
  `
    },


    {
        id: 25008,
        name: 'Brush Cutter Tiller Head',
        category: 'farm-tools',
        group: 'farm equipments',
        brand: 'Generic',
        price: 55000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306520/Astonic-Mart/Products/farm%20equipment/tiller_head-1.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306521/Astonic-Mart/Products/farm%20equipment/tiller_head-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306518/Astonic-Mart/Products/farm%20equipment/tiller_head.jpg'
        ],

        inStock: true,

        description: `The Brush Cutter Tiller Head is a versatile cultivation attachment designed to convert compatible brush cutters into compact tilling machines. Ideal for loosening soil, preparing seedbeds, mixing fertilizers, and cultivating small farms, gardens, and plantations. Manufactured from durable steel with heavy-duty tines, the tiller head provides efficient soil penetration and long-lasting performance in demanding agricultural environments.`,

        specification: `
Category: Brush Cutter Accessories
Product Type: Tiller Head
Material: Hardened Steel
Blade Type: Cultivator Tines
Mounting Type: Universal Brush Cutter Fitment
Application: Soil Cultivation and Land Preparation
Compatibility: Most Brush Cutters and Backpack Weeding Machines
`,

        features: `
Converts brush cutter into mini tiller
Heavy-duty steel tines
Ideal for soil cultivation
Suitable for farms and gardens
Easy installation and removal
Durable construction
Efficient soil loosening
Professional agricultural accessory
Long service life
`
    },

    {
        id: 25009,
        name: 'Brush Cutter Weeder Head',
        category: 'farm-tools',
        group: 'farm equipments',
        brand: 'Generic',
        price: 55000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306526/Astonic-Mart/Products/farm%20equipment/weeder_head.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306527/Astonic-Mart/Products/farm%20equipment/weeder_head-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306528/Astonic-Mart/Products/farm%20equipment/weeder_head-3.jpg'
        ],

        inStock: true,

        description: `The Brush Cutter Weeder Head is a specialized agricultural attachment designed for efficient weed removal around crops, plantations, gardens, and farm rows. Compatible with most brush cutters and backpack weeding machines, this attachment helps clear unwanted vegetation while minimizing damage to surrounding plants. Built from durable steel components, it provides reliable performance for both commercial and domestic farming applications.`,

        specification: `
Category: Brush Cutter Accessories
Product Type: Weeder Head
Material: Hardened Steel
Mounting Type: Universal Brush Cutter Fitment
Application: Weed Removal and Farm Maintenance
Compatibility: Most Brush Cutters and Backpack Weeding Machines
`,

        features: `
Efficient weed removal
Heavy-duty steel construction
Easy installation
Suitable for farm and garden use
Durable and long-lasting
Compatible with most brush cutters
Reduces manual weeding effort
Professional agricultural accessory
`
    },

    {
        id: 25010,
        name: 'Manual Double Seed Planter',
        category: 'farm-machines',
        group: 'farm equipments',
        brand: 'Generic',
        price: 85000,
        discountPrice: 78000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306510/Astonic-Mart/Products/farm%20equipment/Manual_Double_Seed_Planter.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306511/Astonic-Mart/Products/farm%20equipment/Manual_Double_Seed_Planter-2.jpg'
        ],

        inStock: true,


        description: 'Manual double seed planter designed to sow two seeds per hole with consistent spacing and depth. Ideal for maize, beans, groundnuts, soybeans and other grain crops. The planter improves planting efficiency, reduces labor costs and ensures uniform crop establishment.',

        specification: `
  Product Type: Manual Double Seed Planter
  Planting System: Double Seed Drop
  Seeds per Hole: 2 Seeds
  Operation: Push Type
  Planting Rows: Single Row
  Planting Depth: Adjustable
  Seed Spacing: Adjustable
  Hopper Material: Transparent Plastic
  Frame Material: Steel
  Suitable Crops:
  - Maize
  - Groundnuts
  - Beans
  - Soybeans
  - Peas

  Accessories Included:
  Seed Hopper
  Seed Metering Mechanism
  Ground Wheel
  Adjustable Handle
  Mounting Hardware
  User Manual
  `,

        features: `
  Drops two seeds per hole
  Uniform seed spacing
  Adjustable planting depth
  Ground-wheel driven mechanism
  Reduces seed wastage
  Improves planting speed
  Durable steel construction
  Easy operation and maintenance
  Suitable for small and medium farms
  Transparent hopper for seed monitoring
  `,

        showSimilar: true
    },


    {
        id: 25011,
        name: 'INGCO Cordless Backpack Sprayer 16L (CSPLI20162)',
        category: 'farm-machines',
        group: 'farm equipments',
        brand: 'INGCO',
        price: 230000,
        inStock: true,

        model: 'CSPLI20162',
        power: '20V',

        images: [

            'https://res.cloudinary.com/taus5tit/image/upload/v1786306496/Astonic-Mart/Products/farm%20equipment/INGCO_Cordless_Backpack_Sprayer_16L_CSPLI20162.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306497/Astonic-Mart/Products/farm%20equipment/INGCO_Cordless_Backpack_Sprayer_16L_CSPLI20162-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306500/Astonic-Mart/Products/farm%20equipment/INGCO_Cordless_Backpack_Sprayer_16L_CSPLI20162-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306499/Astonic-Mart/Products/farm%20equipment/INGCO_Cordless_Backpack_Sprayer_16L_CSPLI20162-3.jpg'
        ],

        description: 'The INGCO CSPLI20162 Cordless Backpack Sprayer is a professional 16L battery-powered sprayer designed for gardening, agriculture, landscaping, pest control, and outdoor maintenance. Powered by the INGCO P20S 20V battery platform, it delivers consistent spraying performance with dual-speed control, making it ideal for fertilizers, herbicides, pesticides, and disinfectants.',

        specification: `
Model: CSPLI20162
Type: Cordless Backpack Sprayer
Tank Capacity: 16L
Voltage: 20V
Battery Platform: INGCO P20S
Pressure: 5 Bar
Flow Rate: 1.35L/min
Spray Distance: >5m
Speed Control: Dual-Speed
Tank Material: PE (Polyethylene)
Plastic Parts Material: PP
Seal Material: NBR
Spray Lance: Stainless Steel
Net Weight: 3.9kg

Accessories:
1 x 2.0Ah Lithium-Ion Battery (FBLI20011)
1 x Fast Charger (FCLI2001)
Stainless Steel Spray Lance
Spray Hose
Trigger Handle
Shoulder Harness
User Manual
`,

        features: `
• Powerful 20V cordless operation
• Large 16L tank capacity
• Dual-speed spray control
• 5 Bar spraying pressure
• Long spray distance exceeding 5 meters
• Comfortable backpack design
• Stainless steel spray lance
• Lightweight and portable
• Suitable for pesticides, herbicides and fertilizers
• Compatible with INGCO P20S battery platform
`,
    },

    {
        id: 25012,
        name: 'TOTAL Cordless Backpack Sprayer 20V (TSPLI2016)',
        category: 'knapsack-sprayers',
        group: 'Farm Machines',
        brand: 'TOTAL',

        price: 235000,
        inStock: true,

        model: 'TSPLI2016',
        power: '20V',

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306522/Astonic-Mart/Products/farm%20equipment/TOTAL_Cordless_Backpack_Sprayer_20V_TSPLI2016.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306523/Astonic-Mart/Products/farm%20equipment/TOTAL_Cordless_Backpack_Sprayer_20V_TSPLI2016-2.webp'
        ],

        description: 'The TOTAL TSPLI2016 Cordless Backpack Sprayer is a 16L battery-powered sprayer designed for agriculture, gardening, landscaping, pest control, and general outdoor spraying applications. Featuring two-speed control, 5 Bar working pressure, and a stainless steel spray lance, it provides efficient and consistent spraying performance while eliminating manual pumping.',

        specification: `
Model: TSPLI2016
Type: Cordless Backpack Sprayer
Tank Capacity: 16L
Voltage: 20V
Pressure: 5 Bar
Flow Rate: 1.35L/min
Output Capacity: 380L
Spray Distance: >5m
Speed Control: Two-Speed
Tank Material: PE (Polyethylene)
Plastic Components: PP (Polypropylene)
Seal Material: NBR
Spray Lance: Stainless Steel
Net Weight: 3.9kg
Battery Platform: TOTAL P20S

Accessories:
Stainless Steel Spray Lance
Spray Hose
Trigger Handle
Shoulder Harness
User Manual

Note:
Battery and Charger Sold Separately
`,

        features: `
• Powerful 20V cordless operation
• Large 16L tank capacity
• Two-speed spray control
• 5 Bar spraying pressure
• Long spray distance over 5 meters
• Stainless steel spray lance
• Comfortable backpack design
• Lightweight and portable
• Suitable for pesticides, herbicides and fertilizers
• Compatible with TOTAL P20S battery system
`,


        capacity: '16L',

        showSimilar: true
    },


    {
        id: 25013, // Replace with your product ID
        name: "4-Tine Heavy Duty Garden Digging Fork",
        category: 'farm-tools',
        group: 'farm equipments',
        brand: "Generic",
        price: 25000,

        images: [
            "https://res.cloudinary.com/taus5tit/image/upload/v1786306471/Astonic-Mart/Products/farm%20equipment/4-Tine_Heavy_Duty_Garden_Digging_Fork.png",
            "https://res.cloudinary.com/taus5tit/image/upload/v1786306472/Astonic-Mart/Products/farm%20equipment/4-Tine_Heavy_Duty_Garden_Digging_Fork-2.jpg",
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306474/Astonic-Mart/Products/farm%20equipment/4-Tine_Heavy_Duty_Garden_Digging_Fork-3.jpg'
        ],

        description: `The 4-Tine Heavy Duty Garden Digging Fork is designed for digging, turning, loosening, and aerating soil with minimal effort. Constructed from high-strength forged steel with a durable handle, it is ideal for gardening, landscaping, farming, and agricultural applications. The sharp tines easily penetrate compacted soil while providing excellent leverage for lifting compost, mulch, manure, and garden waste.`,

        specification: `
• Product Type: Garden Digging Fork
• Fork Type: 4-Tine
• Material: Forged Carbon Steel
• Handle Material: Steel Shaft with D-Grip Handle
• Grip Type: Ergonomic D-Handle
• Finish: Powder-Coated
• Corrosion Resistant
• Heavy-Duty Construction
• Sharp Penetrating Tines
• Suitable for Digging, Turning and Aerating Soil
• Ideal for Gardening, Landscaping and Agricultural Use
• Comfortable Anti-Slip Grip
• Applications: Soil Preparation, Compost Turning, Mulch Handling, Farm Work, Garden Maintenance
• Accessories: Garden Digging Fork
`,

        features: `
• Heavy-duty forged steel construction
• Strong 4-tine design for efficient digging
• Ergonomic D-handle for better control
• Rust and corrosion resistant finish
• Excellent for loosening compacted soil
• Ideal for compost and manure handling
• Durable and long service life
• Suitable for professional and domestic use
• Easy to clean and maintain
`,

        model: "4-Tine Garden Fork",

        inStock: true,
        showSimilar: true
    },

    // farm equipments ends at 27000

]