import { Product } from '../models/Product';

export const CONSTRUCTION_EQUIPMENTS_PRODUCTS: Product[] = [

    // start from 19001
    // ends at 21000
    {
        id: 19001,
        name: 'Poker Vibrator Machine Robin Engine Gasoline 5HP',
        category: 'concrete-vibrators',
        group: 'construction equipments',
        brand: 'Robin',
        price: 400000, // adjust based on your market price (commonly Γéª220,000 - Γéª380,000 in Nigeria with 6m hose)

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310116/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Poker_Vibrator_Machine_Robin_Engine_Gasoline.webp',
        ],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The Poker Vibrator Machine with Robin 5HP Gasoline Engine is a powerful and reliable concrete vibrator designed for effective compaction of concrete on construction sites. Powered by the famous Robin EY20 5HP (maximum) air-cooled 4-stroke gasoline engine, it delivers strong vibration to remove air pockets, voids, and honeycombs from fresh concrete, ensuring dense and high-strength structures. The set typically includes the engine mounted on a sturdy frame with coupling, flexible shaft (usually 6 meters), and poker head (38mm or 45mm). It is widely used for foundations, columns, beams, slabs, and large pours where high-frequency vibration is required.`,

        specification: `
  - Product Name: Poker Vibrator Machine / Concrete Vibrator
  - Engine Brand: Robin EY20 (or EY20D)
  - Engine Type: Air-cooled, 4-stroke, single cylinder, side valve
  - Engine Power: Max 5HP / 3.6kW @ 3600 rpm (Rated ~3.5HP)
  - Displacement: 183 cm┬│
  - Fuel Tank Capacity: 3.8 Liters
  - Starting System: Recoil starter
  - Vibration Frequency: 9800 - 12000 VPM (vibrations per minute)
  - Flexible Shaft Length: 6 meters (standard)
  - Poker Head Diameter: 38mm or 45mm (common sizes)
  - Poker Head Length: Approx. 470mm
  - Amplitude: ~1.2 mm
  - Frame: Heavy-duty steel base with coupling
  - Fuel Consumption: Approx. 313g/kWh
  - Usage: Concrete compaction for foundations, columns, beams, slabs, and general construction
  `,

        features: `
  - Robust Robin EY20 5HP gasoline engine for reliable high performance
  - High vibration frequency (up to 12,000 VPM) for excellent concrete consolidation
  - Long 6-meter flexible shaft for deep reach in formwork
  - Available with 38mm or 45mm poker head for different applications
  - Sturdy steel frame with easy coupling system for quick setup
  - Recoil start with efficient air-cooled design
  - Portable yet powerful ΓÇô ideal for medium to large construction projects
  - Low maintenance and durable components
  - Widely used by professional contractors in Nigeria for quality concrete work
  - Reduces voids and improves concrete strength and finish
  `
    },



    {
        id: 19002,
        name: '4 Wheels 500L Portable Concrete Cement Mixer',
        category: 'concrete-mixers',
        group: 'construction equipments',
        brand: 'Generic',
        price: 1850000, // adjust based on your market price (commonly Γéª1,200,000 - Γéª2,500,000 in Nigeria for 500L 4-wheel diesel models)

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305599/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/concrete-mixers-500l.webp',
        ],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The 4 Wheels 500L Portable Concrete Cement Mixer is a heavy-duty mobile concrete mixing machine designed for construction sites, block production, and small to medium building projects. With a 500-liter drum capacity and four large wheels, it offers excellent mobility and can be easily moved around the site by hand or towed. Most versions are powered by a reliable diesel engine (typically 6-10HP), providing strong mixing performance for cement, sand, gravel, and water. The drum rotates for efficient mixing and reverses for easy discharge. Its robust steel construction and large wheels make it ideal for daily use in Nigeria's construction industry.`,

        specification: `
  - Product Name: 4 Wheels Portable Concrete Cement Mixer
  - Drum Capacity: 500 Liters
  - Mixing Capacity: Approx. 350-400 kg per batch (about 2 bags of 50kg cement)
  - Drum Material: Thickened steel (usually 2.5-4mm)
  - Wheels: 4 heavy-duty wheels for easy mobility
  - Power Source: Diesel Engine (common 6HP - 10HP water-cooled or air-cooled)
  - Drum Speed: 20 - 30 RPM
  - Productivity: 6 - 10 m┬│/hour (theoretical)
  - Discharge Method: Manual tilting / drum reverse
  - Water Supply: Manual or optional pump
  - Max Aggregate Size: 60mm
  - Dimensions: Approx. 2300 x 1100 x 1300 mm (varies by model)
  - Weight: Approx. 400 - 600 kg
  - Usage: Mixing concrete, mortar, plaster for foundations, blocks, slabs, and general construction
  `,

        features: `
  - Large 500L drum capacity for higher output per batch
  - Four large wheels for smooth movement on rough construction sites
  - Powerful diesel engine for reliable performance even in tough conditions
  - Heavy-duty steel drum and frame for long service life
  - Efficient mixing with good homogeneity in short time
  - Easy tilting mechanism for fast and clean discharge
  - Portable design ΓÇô can be moved manually or towed
  - Suitable for block making, building projects, road works, and small concrete pours
  - Low maintenance with widely available spare parts in Nigeria
  - Great value for contractors and block manufacturers
  `
    },



    {
        id: 19003,
        name: 'Robin Engine EY20 Vibratory Plate Compactor C90 (90kg)',
        category: 'plate-compactors',
        group: 'construction equipments',
        brand: 'Robin',
        price: 320000, // adjust based on your market price (commonly Γéª250,000 - Γéª420,000 in Nigeria for 90kg model)

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310118/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Robin_Engine_Ey15_Ey20_Excavator_Viber_Plate_Compactor.webp',
        ],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The Robin Engine EY20 Vibratory Plate Compactor C90 (90kg) is a heavy-duty gasoline-powered plate compactor designed for efficient compaction of soil, sand, gravel, backfill, and asphalt. Powered by the reliable Robin EY20 5HP air-cooled 4-stroke gasoline engine, it delivers strong centrifugal force and consistent vibration for excellent compaction results. With a 90kg operating weight and approximately 15 kN centrifugal force, it is ideal for medium to large construction projects including road base preparation, foundation work, trench backfilling, and paving. The robust steel plate, foldable handle, and efficient design make it durable and easy to operate on job sites.`,

        specification: `
  - Product Name: Vibratory Plate Compactor C90
  - Engine Brand: Robin EY20
  - Engine Type: Air-cooled, 4-stroke, single cylinder gasoline
  - Engine Power: 5.0 HP (max) / approx. 3.6 kW
  - Vibration Frequency: 5600 VPM (vibrations per minute)
  - Centrifugal Force: 15 kN
  - Compaction Depth: Up to 30 cm (in soil)
  - Plate Size: Approx. 550 x 420 mm to 600 x 470 mm
  - Travel Speed: 15 - 25 m/min
  - Operating Weight: 90 kg
  - Fuel Tank Capacity: 3.8 Liters
  - Starting System: Recoil starter
  - Features: Foldable handle, heavy-duty steel base plate, open self-cleaning design
  - Usage: Soil compaction, gravel, sand, asphalt paving, foundation preparation, road construction, trench backfill
  `,

        features: `
  - Powerful Robin EY20 5HP gasoline engine for reliable performance
  - 90kg operating weight provides optimal balance between maneuverability and compaction force
  - Strong 15 kN centrifugal force for deep and uniform compaction
  - High vibration frequency (5600 VPM) ensures excellent density in granular materials
  - Heavy-duty steel plate for durability and long service life
  - Foldable handle for easy transport and compact storage
  - Efficient air-cooled engine with low fuel consumption
  - Suitable for both forward compaction and light asphalt finishing
  - Ideal for professional contractors working on foundations, roads, and landscaping projects
  - Proven reliability in tough construction environments
  - Easy maintenance and widely available spare parts in Nigeria
  `
    },



    {
        id: 19004,
        name: 'TOTAL Gasoline Tamping Rammer 196cc / 6.5HP (TP880-2)',
        category: 'jumping-rammers',
        group: 'construction equipments',
        brand: 'TOTAL',
        price: 285000, // adjust based on your market price (commonly Γéª220,000 - Γéª380,000 in Nigeria)

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305607/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/TOTAL_GASOLINE_TAMPING_RAMMER_196cc_6.5HP_TP880-2.jpg',

        ],

        inStock: false,
        jumiaLink: '',
        kongaLink: '',

        description: `The TOTAL Gasoline Tamping Rammer 196cc / 6.5HP (TP880-2) is a powerful and durable compaction tool designed for tight spaces and narrow trenches where plate compactors cannot reach. Powered by a robust 196cc 4-stroke gasoline engine delivering 6.5HP, it provides strong impact force for effective compaction of cohesive soils, backfill, gravel, and asphalt. Its high jumping frequency and heavy-duty shoe plate make it ideal for foundation work, road construction, pipeline trenches, and landscaping projects. The ergonomic handle with vibration dampening system ensures operator comfort during prolonged use.`,

        specification: `
  - Product Name: Gasoline Tamping Rammer
  - Model: TP880-2
  - Brand: TOTAL
  - Engine Type: 196cc, 4-stroke, single cylinder, air-cooled gasoline engine
  - Engine Power: 6.5 HP
  - Impact Force: 13 - 15 kN
  - Jumping Height: 40 - 65 mm
  - Impact Frequency: 600 - 700 times/min
  - Plate / Shoe Size: Approx. 330 x 285 mm
  - Fuel Tank Capacity: 3.6 Liters
  - Starting System: Recoil starter
  - Operating Weight: Approx. 85 - 95 kg
  - Features: Anti-vibration handle, heavy-duty polyurethane bellows, reinforced base plate
  - Usage: Trench compaction, foundation backfill, road shoulder, asphalt patching, landscaping, narrow areas
  `,

        features: `
  - Powerful 196cc 6.5HP gasoline engine for strong compaction performance
  - High impact force (13-15 kN) and jumping frequency for excellent soil density
  - Heavy-duty shoe plate for durability and effective compaction in tight spaces
  - Ergonomic anti-vibration handle reduces operator fatigue
  - Robust construction with reinforced bellows for long service life
  - Easy recoil starting and simple maintenance
  - Ideal for compacting cohesive soils, gravel, sand, and asphalt in confined areas
  - Widely used by contractors for pipeline trenches, foundations, and road works
  - Portable and efficient for jobs where larger compactors cannot access
  - Trusted TOTAL quality with good after-sales support in Nigeria
  `
    },




    {
        id: 19006,
        name: 'INGCO Industrial Soft Grip Handle Wheelbarrow 90L (HHWB64018G)',
        category: 'wheel-barrows',
        group: 'construction equipments',
        brand: 'INGCO',
        price: 45000, // adjust based on your market price (commonly Γéª35,000 - Γéª65,000 in Nigeria)

        images: [],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The INGCO Industrial Soft Grip Handle Wheelbarrow 90L (HHWB64018G) is a heavy-duty construction wheelbarrow built for tough daily use on construction sites, farms, and gardens. It features a strong steel tray with 90L water capacity (approx. 6 cubic feet sand capacity) and a maximum load of 150kg. The two-tone soft grip handles provide excellent comfort and control, reducing hand fatigue during long hours of work. Equipped with a 16" x 4.00-8 pneumatic wheel with bearings and diamond tread pattern for smooth rolling on rough surfaces. Its robust steel frame and reinforced legs make it highly durable for transporting sand, cement, gravel, bricks, and other materials.`,

        specification: `
  - Product Name: Industrial Wheelbarrow
  - Model: HHWB64018G
  - Brand: INGCO
  - Tray Capacity: 90 Liters (Water)
  - Sand Capacity: 6 CBF (Cubic Feet)
  - Max Load Capacity: 150 kg
  - Wheel Size: 16" x 4.00-8 pneumatic wheel with bearing
  - Tyre Type: Pneumatic with diamond tread pattern (4PR)
  - Handle Type: Two-tone soft grip ergonomic handles
  - Tray Thickness: Heavy-duty steel (approx. 0.8 - 1.0mm)
  - Frame/Leg: Reinforced steel tube (Handle: 1.2 x 32mm, Leg: 1.2 x 28mm)
  - Dimensions (LxHxCxAxBxh): Approx. 1400 x 610 x 595 x 860 x 600 x 280 mm
  - Weight: Approx. 15 - 18 kg (empty)
  - Usage: Construction sites, block making, farming, gardening, material transport
  `,

        features: `
  - Large 90L capacity ideal for heavy loads of sand, cement, gravel, and bricks
  - Maximum load capacity of 150kg for industrial and construction use
  - Comfortable two-tone soft grip handles reduce fatigue and improve control
  - Pneumatic 16" wheel with bearings for smooth and easy rolling on uneven terrain
  - Diamond tread tyre provides excellent grip and durability
  - Heavy-duty steel tray and reinforced frame for long-lasting performance
  - Robust leg design for stability when loaded or parked
  - Suitable for professional contractors, builders, farmers, and landscapers
  - Trusted INGCO industrial quality with good resistance to rust and wear
  - Easy to maneuver and maintain
  `
    },


    {
        id: 19007,
        name: 'TOTAL Vibrator Poker 45mm / 6m (TPVP1451)',
        category: 'poker-hoses',
        group: 'construction equipments',
        brand: 'TOTAL',
        price: 140000, // adjust based on your market price (commonly Γéª75,000 - Γéª120,000 in Nigeria)

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310130/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/total_vibrator_poker_45mm_by_6m.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310132/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/total_vibrator_poker_45mm_by_6m2.jpg',

        ],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The TOTAL Vibrator Poker 45mm / 6m (TPVP1451) is a high-performance concrete vibrator head designed for effective compaction of concrete. With a 45mm diameter poker head and 6-meter flexible shaft, it delivers strong vibration to remove air pockets and voids from fresh concrete, resulting in dense, high-strength structures. It is commonly paired with gasoline engines (such as TOTAL or Robin 5HP) or electric motors. The 45mm head size offers a good balance between compaction power and maneuverability, making it ideal for columns, beams, slabs, foundations, and general construction work.`,

        specification: `
  - Product Name: Concrete Vibrator Poker
  - Model: TPVP1451
  - Brand: TOTAL
  - Poker Head Diameter: 45mm
  - Flexible Shaft Length: 6 meters
  - Vibration Frequency: 10,000 - 12,000 VPM (vibrations per minute)
  - Amplitude: Approx. 1.2 - 1.4 mm
  - Hose Material: High-quality rubber with steel reinforcement
  - Poker Head Material: Hardened steel with eccentric rotor
  - Connection Type: Standard coupling (compatible with TOTAL and most common engines)
  - Weight: Approx. 12 - 15 kg (complete with 6m hose)
  - Usage: Compaction of concrete in columns, beams, slabs, foundations, walls, and floor slabs
  `,

        features: `
  - 45mm diameter poker head for powerful and effective vibration
  - Extra-long 6-meter flexible shaft for deep reach in formwork
  - High vibration frequency (up to 12,000 VPM) for excellent concrete consolidation
  - Durable rubber hose with internal steel reinforcement for long service life
  - Hardened steel poker head with eccentric design for strong amplitude
  - Compatible with TOTAL gasoline engines and most 5HP engines
  - Ideal for professional contractors and construction companies
  - Reduces voids, honeycombs, and improves concrete strength and finish
  - Easy to couple and operate on job sites
  - Reliable TOTAL quality at an affordable price
  `
    },


    {
        id: 19009,
        name: 'Steel Rod Cutter - 40mm',
        category: 'rebar-machines',
        group: 'construction equipments',
        brand: 'Generic',
        price: 450000, // adjust based on your market price (commonly Γéª130,000 - Γéª220,000 in Nigeria)

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786311458/Astonic-Mart/Products/rebar%20machines/Steel-Rod-Cutting-Machine-40mm.webp',

        ],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The Steel Rod Cutter 40mm is a heavy-duty manual rebar cutting machine designed for cutting mild steel reinforcement bars (rebar) up to 40mm diameter. It is widely used on construction sites, fabrication yards, and engineering projects for fast, clean, and accurate cutting of steel rods. Built with a strong steel frame and high-strength cutting blades, this cutter delivers efficient performance with minimal effort. Its long handle design provides excellent leverage, allowing operators to cut thick rods smoothly without electricity. This machine is an essential tool for steel fixers, contractors, and construction professionals who require reliable and precise cutting operations.`,

        specification: `
  - Product Name: Manual Steel Rod Cutter / Rebar Cutter
  - Max Cutting Capacity: 40mm (mild steel rebar)
  - Cutting Type: Manual shear cutting
  - Blade Material: Hardened alloy steel
  - Frame Material: Heavy-duty steel construction
  - Operation: Manual with long leverage handle
  - Cutting Speed: Fast and efficient manual cutting
  - Weight: Approx. 100 - 160 kg (depending on model)
  - Dimensions: Approx. 1000 x 700 x 800 mm
  - Usage: Construction sites, steel fabrication, reinforcement work, engineering projects
  `,

        features: `
  - Cuts rebar up to 40mm diameter efficiently
  - Heavy-duty steel frame ensures durability and stability
  - Sharp, hardened blades provide clean and precise cuts
  - Long handle design reduces operator effort
  - No electricity required ΓÇô perfect for remote job sites
  - Fast manual cutting improves work productivity
  - Suitable for mild steel and reinforced bars
  - Compact and rugged design for site use
  - Reliable performance for continuous heavy-duty operations
  - Essential tool for builders, contractors, and steel fabricators
  `
    },

    {
        id: 19008,
        name: 'Rod Bender - 50mm',
        category: 'rebar-machines',
        group: 'construction equipments',
        brand: 'Generic',
        price: 4020000, // adjust based on your market price (commonly Γéª150,000 - Γéª250,000 in Nigeria)

        images: [
            'src/https://res.cloudinary.com/taus5tit/image/upload/v1786311457/Astonic-Mart/Products/rebar%20machines/rod_benders_50mm.webp',
        ],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The Rod Bender 50mm is a heavy-duty manual rebar bending machine designed for bending mild steel reinforcement bars (rebar) up to 50mm diameter. It is widely used on construction sites for accurate bending of rods for columns, beams, slabs, foundations, and other structural elements. This robust bender features a strong steel frame, long leverage arms for reduced effort, and multiple bending angles (0┬░ to 180┬░). The machine allows precise and consistent bends without damaging the rebar, making it essential for steel fixers, fabricators, and civil engineering contractors.`,

        specification: `
  - Product Name: Manual Rod Bender / Rebar Bender
  - Max Bending Capacity: 50mm (mild steel rebar)
  - Bending Angle: 0┬░ - 180┬░
  - Frame Material: Heavy-duty cast iron and steel
  - Lever Arms: Long steel handles for maximum leverage
  - Bending Pins: Multiple interchangeable pins and rollers
  - Design: Manual operation with gear-assisted mechanism in some models
  - Weight: Approx. 120 - 180 kg (depending on exact model)
  - Dimensions: Approx. 1200 x 800 x 900 mm
  - Usage: Construction sites, rebar fabrication, column stirrups, beam hooks, foundation work
  `,

        features: `
  - Bends rebar up to 50mm diameter with ease
  - Heavy-duty construction for durability and long service life
  - Long leverage arms reduce operator effort significantly
  - Precise bending with consistent angles (0┬░ to 180┬░)
  - Multiple bending pins for different rod sizes and bend radii
  - Suitable for mild steel and high-yield reinforcement bars
  - No electricity required ΓÇô ideal for remote sites
  - Robust frame ensures stability during heavy bending
  - Essential tool for steel fixers and construction companies
  - Cost-effective solution for medium to large rebar bending needs
  `
    },





    {
        id: 19010,
        name: 'SGT KNOTS Twisted ProManila Rope ΓÇô 1.25" x 10ft (Tan)',
        category: 'ropes',
        group: 'construction equipments',
        brand: 'SGT KNOTS',
        price: 28000, // adjust based on your market (commonly Γéª25,000 - Γéª45,000 in Nigeria depending on import cost)

        images: [],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The SGT KNOTS Twisted ProManila Rope is a high-quality synthetic alternative to traditional Manila rope, designed for superior strength, durability, and resistance to harsh weather conditions. Measuring 1.25 inches in diameter and 10 feet in length, this tan-colored rope offers the natural look of manila while outperforming it in longevity and performance. Unlike natural fibers, ProManila rope resists rot, mold, moisture, and UV damage, making it ideal for both indoor and outdoor applications. It is perfect for construction, landscaping, marine use, lifting, pulling, and decorative purposes.`,

        specification: `
  - Product Name: Twisted ProManila Rope
  - Brand: SGT KNOTS
  - Diameter: 1.25 inches (approx. 32mm)
  - Length: 10 feet (approx. 3 meters)
  - Material: Synthetic polypropylene (UnManila)
  - Color: Tan (natural manila appearance)
  - Construction: 3-strand twisted rope
  - ┘à┘é╪º┘ê┘à╪¬: High resistance to moisture, UV, and chemicals
  - Weight: Lightweight and easy to handle
  - Usage: General-purpose, marine, construction, landscaping, decor
  `,

        features: `
  - Synthetic alternative to natural Manila rope with enhanced durability
  - Resistant to rot, mildew, moisture, and UV exposure
  - Strong and flexible for heavy-duty applications
  - Lightweight and easy to handle compared to natural fiber ropes
  - Maintains shape and strength even in wet conditions
  - Ideal for indoor and outdoor use
  - Natural tan color for aesthetic and decorative purposes
  - Suitable for tying, pulling, lifting, and general utility work
  - Long-lasting performance with minimal maintenance
  - Trusted quality from SGT KNOTS brand
  `
    },



    {
        id: 19011,
        name: 'Total Power Trowel ΓÇô 6.5HP Concrete Finishing Machine',
        category: 'power-trowels',
        group: 'construction equipments',
        brand: 'Total',
        price: 950000, // adjust based on your market (commonly Γéª850,000 - Γéª1,200,000 in Nigeria)

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310509/Astonic-Mart/Products/power%20trowel/total_power_trowels_6%2C5hp.jpg',
        ],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The Total Power Trowel 6.5HP is a high-performance concrete finishing machine designed for smoothing and polishing freshly poured concrete surfaces. Powered by a reliable 6.5 horsepower petrol engine, this walk-behind trowel delivers excellent power and efficiency for professional finishing of floors, slabs, and pavements. It is widely used on construction sites for achieving a smooth, level, and durable concrete surface. The machine features adjustable blades, a sturdy protective ring, and an ergonomic handle for easy control, making it ideal for contractors, builders, and flooring professionals.`,

        specification: `
  - Product Name: Power Trowel / Concrete Finishing Machine
  - Brand: Total
  - Engine Power: 6.5HP petrol engine
  - Engine Type: Air-cooled, 4-stroke engine
  - Blade Diameter: Approx. 900mm (36 inches)
  - Blades: 4 finishing blades (adjustable pitch)
  - Rotation Speed: Approx. 60 - 120 RPM
  - Fuel Type: Petrol (Gasoline)
  - Fuel Tank Capacity: Approx. 3.6L
  - Operation: Walk-behind
  - Weight: Approx. 80 - 100 kg
  - Usage: Concrete floor finishing, slab smoothing, surface polishing
  `,

        features: `
  - Powerful 6.5HP engine for efficient concrete finishing
  - Produces smooth, level, and professional surface finish
  - Adjustable blade pitch for different finishing stages
  - Durable steel guard ring for operator safety
  - Ergonomic handle for easy control and maneuverability
  - Heavy-duty construction for long-lasting performance
  - Ideal for large floor areas and construction projects
  - Stable operation with minimal vibration
  - Easy to maintain and operate
  - Trusted Total brand quality for construction professionals
  `
    },



    {
        id: 19012,
        name: '15m Fall Arrester Wire Rope Safety Equipment Falling Protector',
        category: 'fall-arresters',
        group: 'construction equipments',
        brand: 'Generic',
        price: 140000,
        images: ['https://res.cloudinary.com/taus5tit/image/upload/v1786306459/Astonic-Mart/Products/fall%20arresters/15m-Safety-Rope-Fall-Protection-Equipment-150kg-Capacity-Arrester.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306452/Astonic-Mart/Products/fall%20arresters/15m_fall_arrester.png',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306458/Astonic-Mart/Products/fall%20arresters/15m_fall_arrester-2.png'

        ],

        inStock: true,
        description: '15 meter retractable fall arrester designed to protect workers operating at height. This safety device uses a high-strength galvanized steel wire rope with an automatic locking mechanism that instantly stops a fall. It is widely used in construction, scaffolding, tower climbing, roofing, and industrial maintenance where reliable fall protection is required.',
        specification: `
       - Cable Length: 15 Meters
       - Cable Material: Galvanized Steel Wire Rope
       - Cable Diameter: 4-5 mm
       - Housing Material: High Strength Polymer / Alloy
       - Hook Type: Heavy Duty Steel Snap Hook
       - Locking System: Automatic Fall Arrest Lock
       - Weight Capacity: Up to 140kg
       - Application: Construction, Tower Climbing, Industrial Work
     `,
        features: `
       - Automatic fall arrest locking system
       - Durable galvanized steel wire rope
       - Heavy duty snap hook connector
       - High impact resistant casing
       - Smooth retractable cable mechanism
       - Reliable protection for working at heights
     `
    },


    {
        id: 19013,
        name: 'Toyo 6m Retractable Fall Arrestor',
        category: 'fall-arresters',
        group: 'construction equipments',
        brand: 'Toyo',
        price: 139000, // Current price range on Jumia Nigeria (around Γéª144,000 - Γéª150,000)
        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306462/Astonic-Mart/Products/fall%20arresters/Toyo-6m-retractable-fall-arrestor.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306461/Astonic-Mart/Products/fall%20arresters/Toyo-3m-retractable-all-arrestor.jpg',
        ],
        inStock: true,
        jumiaLink: 'https://www.jumia.com.ng/toyo-6m-retractable-fall-arrestor',
        kongaLink: '',
        description: `The Toyo 6m Retractable Fall Arrestor is a self-retracting lifeline (also known as a yo-yo or fall arrest block) designed for height safety and fall protection. It provides workers with freedom of movement while automatically locking in the event of a fall. Ideal for construction sites, roofing, scaffolding, steel fixing, maintenance work, and other elevated tasks. The 6-meter cable length offers a good working radius while maintaining high safety standards. It features a durable housing, galvanized steel cable, and energy-absorbing mechanism to reduce impact forces.`,
        specification: `
  - Product Name: Toyo 6m Retractable Fall Arrestor
  - Model: Toyo 6m Fall Arrest Block
  - Length: 6 meters
  - Cable: Galvanized steel rope (typically 4-5mm diameter)
  - Housing: High-impact ABS plastic or sturdy polymer casing
  - Anchorage: Swivel hook or eye with connector
  - Harness End: Snap hook with load indicator
  - Standards: Compliant with EN 360 (common for retractable fall arrestors)
  - Max User Weight: Up to 140 kg (typical for this class)
  - Arresting Distance: Limited fall distance with quick lock-up
  - Weight: Approx. 3 - 5 kg (depending on exact variant)
  - Usage: Vertical and limited horizontal use on construction sites, high-rise buildings, bridges, and industrial maintenance
  `,
        features: `
  - Self-retracting 6m cable for automatic tension and easy movement
  - Instant locking mechanism activates during a fall for immediate arrest
  - Integrated energy absorber reduces impact force on the user
  - Durable galvanized steel lifeline resistant to corrosion
  - Swivel anchorage point prevents twisting of the cable
  - Lightweight yet robust design for portability on site
  - No electricity or external power required
  - Essential personal protective equipment (PPE) for workers at height
  - Cost-effective safety solution for steel fixers, scaffolders, and civil contractors in Nigeria
  - Compatible with full body safety harnesses
  `
    },


    {
        id: 19014,
        name: 'C-90 Plate Compactor Diesel Engine',
        category: 'plate-compactors',
        group: 'construction equipments',
        brand: 'Generic',
        model: 'C-90',
        price: 850000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309907/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/desiel_plate_compactor-2.jpg',

        ],

        inStock: true,

        description: `The C-90 Plate Compactor Diesel Engine is a heavy-duty compaction machine designed for compacting soil, sand, gravel, asphalt, and paving materials in construction, landscaping, road maintenance, and foundation projects. Powered by a reliable diesel engine, it delivers strong compaction force and excellent maneuverability for efficient ground preparation. Its rugged steel base plate and vibration-isolated handle ensure durability and operator comfort during extended use. Items Included in the Package: ΓÇó 1 ├ù C-90 Plate Compactor ΓÇó Wheel Kit (Optional on some models) ΓÇó User Manual`,

        specification: `Category: Plate Compactors
Parent Category: Light Construction Equipment
Brand: Generic
Model: C-90
Engine Type: Air-Cooled Single-Cylinder Diesel Engine
Engine Model: 170F Diesel Engine
Engine Power: 4.8kW (6.5HP)
Starting System: Recoil Start
Fuel Type: Diesel
Plate Size: 580 ├ù 500mm
Centrifugal Force: 13.5kN
Frequency: 5600 VPM
Compaction Depth: 30cm
Travel Speed: 20m/min
Working Efficiency: 600m┬▓/h
Gradeability: 20┬░
Net Weight: 90kg
Application: Soil, gravel, sand and asphalt compaction
Suitable Use: Road construction, landscaping, paving and foundation work`,

        features: `Powerful 6.5HP diesel engine
13.5kN centrifugal compaction force
30cm compaction depth
5600 vibrations per minute
Heavy-duty steel base plate
Compact and easy-to-maneuver design
Low fuel consumption
Shock-absorbing handle system
Suitable for soil, gravel and asphalt compaction
Ideal for construction and roadwork applications`
    },


    {
        id: 19015,
        name: 'C-120 Plate Compactor Diesel Engine',
        category: 'plate-compactors',
        parentCategory: 'light construction equipment',
        brand: 'Generic',
        model: 'C-120',
        price: 1200000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309909/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/desiel_plate_compactor-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309905/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/desiel_plate_compactor.jpg',

        ],

        inStock: true,

        description: `The C-120 Plate Compactor Diesel Engine is a heavy-duty forward plate compactor designed for compacting soil, sand, gravel, asphalt, and other granular materials in construction, roadwork, landscaping, and foundation preparation. Powered by a reliable 170F diesel engine, it delivers high compaction force and deep penetration for demanding applications. Its robust steel plate, vibration-isolated handle, and durable construction ensure efficient operation and long service life. Items Included in the Package: ΓÇó 1 ├ù C-120 Plate Compactor ΓÇó User Manual`,

        specification: `Category: Plate Compactors
Parent Category: Light Construction Equipment
Brand: Generic
Model: C-120
Engine Type: Air-Cooled, Single-Cylinder, 4-Stroke Diesel Engine
Engine Model: 170F Diesel Engine
Engine Power: 4.0HP
Fuel Type: Diesel
Starting System: Recoil Start
Operating Weight: 120kg
Plate Size: 660 ├ù 510mm
Centrifugal Force: 20kN
Vibration Frequency: 4200-5800 VPM
Compaction Depth: 30-40cm
Travel Speed: 15-25m/min
Gradeability: 20┬░
Application: Soil, gravel, sand and asphalt compaction
Suitable Use: Road construction, paving, landscaping and foundation work`,

        features: `Heavy-duty 120kg operating weight
Reliable 170F diesel engine
20kN centrifugal compaction force
30-40cm compaction depth
Large 660 ├ù 510mm base plate
Travel speed up to 25m/min
Low fuel consumption
Heavy-duty steel construction
Shock-absorbing handle design
Ideal for construction and road maintenance projects`
    },


    {
        id: 19016,
        name: '38mm Concrete Vibrating Poker 6m hose(dynapc fitting)',
        category: 'poker-hoses',
        group: 'construction equipments',
        showSimilar: true,
        brand: 'Generic',
        model: 'Dynapac Type Fitting',

        price: 135000,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309889/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/38mm_concrete_vibrator_hose_with_dynapac.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309900/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/38mm_concrete_vibrator_hose_with_dynapac-8.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309897/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/38mm_concrete_vibrator_hose_with_dynapac-5.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309891/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/38mm_concrete_vibrator_hose_with_dynapac-2.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309894/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/38mm_concrete_vibrator_hose_with_dynapac-3.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309896/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/38mm_concrete_vibrator_hose_with_dynapac-4.webp',

        ],

        inStock: true,

        description: `38mm Concrete Vibrating Poker with 6m Flexible Hose is a heavy-duty concrete compaction attachment designed for removing trapped air bubbles and improving the strength, density and finish of freshly poured concrete. Featuring a durable 38mm vibrator head and a 6-meter flexible shaft, it is suitable for foundations, columns, beams, slabs, road works and general construction applications. The Dynapac-type fitting ensures compatibility with various concrete vibrator drive units used on construction sites.`,

        specification: `
  - Product Name: Concrete Vibrating Poker
  - Brand: Generic
  - Model: Dynapac Type Fitting
  - Vibrator Head Diameter: 38mm
  - Flexible Hose Length: 6 Meters
  - Fitting Type: Dynapac Type
  - Head Material: Hardened Steel
  - Shaft Type: Flexible Reinforced Shaft
  - Application: Concrete Compaction
  - Suitable For: Columns, Beams, Slabs and Foundations
  - Duty Type: Heavy-Duty Construction Use
  - Compatibility: Concrete Vibrator Drive Units with Dynapac Coupling

  Accessories Included:
  - 1 x 38mm Vibrator Poker Head
  - 1 x 6m Flexible Shaft Hose
  - 1 x Dynapac Type Coupling Assembly
  `,

        features: `
  - 38mm heavy-duty vibrator head
  - Long 6-meter flexible hose
  - Efficient concrete air removal
  - Improves concrete density and strength
  - Dynapac-type coupling connection
  - Durable steel construction
  - High vibration transmission efficiency
  - Suitable for large construction projects
  - Flexible shaft for hard-to-reach areas
  - Reduces honeycombing in concrete
  - Easy installation and operation
  - Professional-grade construction equipment
  `
    },

    {
        id: 19017,
        name: '38mm Concrete Vibrating Poker 6m hose(pin)',
        category: 'poker-hoses',
        group: 'construction equipments',
        showSimilar: true,

        brand: 'Generic',
        model: 'Pin Type',
        price: 115000,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309900/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/38mm_concrete_vibrator_hose_with_pin.avif',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309903/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/38mm_concrete_vibrator_hose_with_pin-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309901/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/38mm_concrete_vibrator_hose_with_pin-2.avif'
        ],

        inStock: true,

        description: `38mm Concrete Vibrating Poker with 6m Flexible Hose is a heavy-duty concrete compaction tool designed to eliminate air pockets and improve the density, strength and surface finish of freshly poured concrete. Equipped with a durable 38mm vibrating head and a 6-meter flexible shaft, it is ideal for foundations, columns, beams, slabs and other reinforced concrete structures. The pin-type connection allows easy attachment to compatible concrete vibrator drive units and petrol concrete vibrator engines.`,

        specification: `
  - Product Name: Concrete Vibrating Poker
  - Brand: Generic
  - Model: Pin Type
  - Vibrator Head Diameter: 38mm
  - Flexible Hose Length: 6 Meters
  - Connection Type: Pin Coupling
  - Head Material: Hardened Steel
  - Shaft Type: Reinforced Flexible Shaft
  - Vibration Frequency: High-Speed Mechanical Vibration
  - Application: Concrete Compaction and Air Removal
  - Suitable For: Columns, Beams, Foundations, Floors and Slabs
  - Duty Type: Heavy-Duty Construction Use
  - Compatible With: Petrol and Electric Concrete Vibrator Drive Units

  Accessories Included:
  - 1 x 38mm Vibrator Poker Head
  - 1 x 6m Flexible Shaft Hose
  - 1 x Pin-Type Coupling
  `,

        features: `
  - Heavy-duty 38mm vibrator head
  - Long 6-meter flexible shaft
  - Pin-type coupling connection
  - Efficient removal of trapped air bubbles
  - Improves concrete strength and density
  - Reduces honeycombing and voids
  - Durable steel vibrator head
  - Flexible shaft for deep concrete applications
  - Suitable for columns, slabs and foundations
  - Easy installation and operation
  - Compatible with most concrete vibrator engines
  - Professional construction-grade performance
  `
    },

    {
        id: 19018,
        name: '38mm Concrete Vibrator Head (Dynapac Type)',
        category: 'poker-hoses',
        group: 'construction equipments',
        showSimilar: true,
        brand: 'Generic',
        model: '38mm Dynapac Type',
        price: 35000,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309954/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/dynapac_poker_head.avif',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309957/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/dynapac_poker_head-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309958/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/dynapac_poker_head-3.jpg'
        ],

        inStock: true,

        description: `38mm Concrete Vibrator Head (Dynapac Type) is a heavy-duty replacement poker head designed for concrete vibration and compaction applications. It effectively removes trapped air bubbles from freshly poured concrete, improving density, strength and surface finish. Manufactured from hardened steel, the vibrator head is suitable for use in columns, beams, slabs, foundations and other reinforced concrete structures. It is compatible with Dynapac-type flexible shafts and concrete vibrator systems.`,

        specification: `
  - Product Name: Concrete Vibrator Head
  - Brand: Generic
  - Model: 38mm Dynapac Type
  - Head Diameter: 38mm
  - Fitting Type: Dynapac Type
  - Material: Hardened Steel
  - Head Shape: Cylindrical
  - Application: Concrete Compaction
  - Suitable For: Columns, Slabs, Beams and Foundations
  - Corrosion Resistant: Yes
  - Duty Type: Heavy-Duty Construction Use
  - Compatibility: Dynapac-Type Flexible Shafts

  Accessories Included:
  - 1 x 38mm Concrete Vibrator Head
  `,

        features: `
  - Durable hardened steel construction
  - 38mm vibration head diameter
  - Dynapac-type fitting connection
  - Efficient concrete compaction
  - Removes trapped air bubbles effectively
  - Improves concrete density and strength
  - Reduces honeycombing and void formation
  - Suitable for professional construction work
  - Long service life
  - Easy installation and replacement
  - Compatible with Dynapac-type shafts
  - Reliable performance in demanding environments
  `
    },


    {
        id: 19019,
        name: 'KROST PRO Series Cordless Concrete Vibrator 35mm',
        category: 'poker-hoses',
        group: 'construction equipments',
        showSimilar: true,

        brand: 'KROST',
        model: 'PRO Series 35mm',

        price: 185000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309986/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/KROST_PRO_Cordless_Vibrator.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309986/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/KROST_PRO_Cordless_Vibrator.jpg-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309986/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/KROST_PRO_Cordless_Vibrator.jpg-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309986/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/KROST_PRO_Cordless_Vibrator.jpg-5.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309986/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/KROST_PRO_Cordless_Vibrator.jpg-2.jpg'

        ],

        inStock: false,


        description: `KROST PRO Series Cordless Concrete Vibrator is a portable battery-powered concrete compaction tool designed to eliminate trapped air bubbles and improve the density, strength and finish of freshly poured concrete. Equipped with a 35mm vibrating poker head and high-performance rechargeable battery system, it is ideal for columns, beams, slabs, foundations, sidewalks and general construction work. Its cordless design provides maximum mobility and convenience on job sites without the need for power cables or petrol engines.`,

        specification: `
  - Product Name: Cordless Concrete Vibrator
  - Brand: KROST
  - Model: PRO Series 35mm
  - Rated Voltage: 21V
  - Maximum Voltage: 37V Max
  - Vibrator Head Diameter: 35mm
  - Power Source: Rechargeable Lithium-Ion Battery
  - Motor Type: Brushless Motor
  - No Load Speed: Up to 4500 RPM
  - Operation Type: Cordless
  - Application: Concrete Compaction
  - Suitable For: Columns, Beams, Slabs, Foundations and Floors
  - Construction Type: Portable Handheld Design

  Accessories Included:
  - 1 x KROST Cordless Concrete Vibrator
  - 1 x 35mm Vibrator Poker Head
  - 1 x Flexible Vibrator Shaft
  - 2 x Lithium-Ion Battery
  - 1 x Battery Charger
  - 1 x Carrying Case
  - 1 x User Manual
  `,

        features: `
  - Cordless operation for maximum mobility
  - Powerful lithium battery system
  - High-efficiency brushless motor
  - 35mm heavy-duty vibrator head
  - Eliminates air pockets in concrete
  - Improves concrete density and strength
  - Lightweight and portable design
  - Suitable for indoor and outdoor construction work
  - Ergonomic grip for comfortable operation
  - Low noise and vibration transmission
  - Easy maintenance and operation
  - Professional-grade construction performance
  `
    },

    {
        id: 19020,
        name: 'Electric Concrete Poker Machine 1.1KW',
        category: 'poker-hoses',
        group: 'construction equipments',
        showSimilar: true,

        brand: 'Generic',
        model: '1.1KW',

        price: 380000,
        discountPrice: 350000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309959/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/electric_poker_machine.webp'
        ],

        inStock: true,

        description: `Electric Concrete Poker Machine 1.1KW is a powerful concrete vibration motor designed to drive flexible concrete vibrator shafts for effective concrete compaction. It helps eliminate trapped air bubbles and voids in freshly poured concrete, resulting in stronger, denser and more durable structures. Suitable for foundations, columns, beams, slabs, retaining walls and general construction projects, the machine delivers reliable performance for both professional contractors and construction workers.`,

        specification: `
  - Product Name: Electric Concrete Poker Machine
  - Brand: Generic
  - Model: 1.1KW
  - Rated Power: 1.1KW (1100W)
  - Rated Voltage: 220-240V
  - Frequency: 50/60Hz
  - Motor Type: Single Phase Electric Motor
  - No Load Speed: 2800-3000 RPM
  - Drive Type: Flexible Shaft Drive
  - Shaft Connection: Pin Type Coupling
  - Cooling System: Air Cooled
  - Protection Class: IP44
  - Application: Concrete Compaction and Vibration
  - Suitable For: 35mm, 38mm, 45mm and 50mm Vibrator Shafts

  Accessories Included:
  - 1 x Electric Concrete Poker Machine 1.1KW
  - 1 x Carrying Frame
  - 1 x User Manual

  Note:
  - Flexible Shaft and Vibrator Head Sold Separately
  `,

        features: `
  - Powerful 1.1KW motor
  - High vibration transmission efficiency
  - Compatible with multiple poker shaft sizes
  - Durable heavy-duty construction
  - Air-cooled motor design
  - Stable and reliable performance
  - Easy operation and maintenance
  - Portable carrying frame
  - Suitable for continuous construction work
  - Ideal for foundations, slabs and columns
  - Professional-grade concrete compaction solution
  - Long service life
  `
    },

    {
        id: 19021,
        name: 'TOTAL 20V Cordless Concrete Vibrator Poker-TCVLI2023011',
        category: 'poker-hoses',
        group: 'construction equipments',
        showSimilar: false,
        brand: 'TOTAL',
        model: 'TCVLI2023011',

        price: 180000,
        discountPrice: 170000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310121/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Total_TCVLI2023011_20V_Cordless_Concrete_Vibrator_Poker.webp-2.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310121/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Total_TCVLI2023011_20V_Cordless_Concrete_Vibrator_Poker.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310123/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Total_TCVLI2023011_20V_Cordless_Concrete_Vibrator_Poker.webp-3.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310121/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Total_TCVLI2023011_20V_Cordless_Concrete_Vibrator_Poker.webp-4.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310121/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Total_TCVLI2023011_20V_Cordless_Concrete_Vibrator_Poker.webp-5.webp'
        ],

        inStock: true,

        description: `TOTAL TCVLI2023011 Cordless Concrete Vibrator is a professional battery-powered concrete compaction tool designed to remove trapped air bubbles from freshly poured concrete, improving strength, density and surface finish. Featuring a 35mm vibrator head and 1200mm flexible shaft, it is ideal for foundations, columns, beams, slabs and other concrete structures. The cordless design provides excellent portability and convenience on construction sites without the need for extension cables or fuel-powered engines.`,

        specification: `
  - Product Name: Cordless Concrete Vibrator Poker
  - Brand: TOTAL
  - Model: TCVLI2023011
  - Voltage: 20V
  - Battery Type: Lithium-Ion
  - Battery Capacity: 2.0Ah
  - Vibrator Head Diameter: 35mm
  - Shaft Length: 1200mm
  - No Load Speed: 2300 RPM
  - Charger Voltage: 220-240V~50/60Hz
  - Motor Type: High-Efficiency DC Motor
  - Operation Type: Cordless
  - Application: Concrete Compaction and Air Removal
  - Suitable For: Columns, Beams, Slabs, Foundations and Floors
  - Weight: Approx. 6.4kg

  Accessories Included:
  - 1 x TOTAL TCVLI2023011 Concrete Vibrator
  - 1 x 35mm x 1200mm Vibrator Poker
  - 1 x 20V 2.0Ah Lithium-Ion Battery
  - 1 x Fast Charger
  - 1 x User Manual
  `,

        features: `
  - Powerful 20V cordless operation
  - 35mm heavy-duty vibrator head
  - 1200mm flexible poker shaft
  - 2300 RPM vibration speed
  - Improves concrete strength and density
  - Removes trapped air pockets effectively
  - Lightweight and portable design
  - Suitable for professional construction work
  - Rechargeable lithium-ion battery system
  - Fast charging capability
  - Easy one-person operation
  - Durable construction for job-site use
  `
    },

    {
        id: 19022,
        name: 'INGCO Cordless Concrete Vibrator',
        category: 'poker-hoses',
        group: 'construction equipments',
        showSimilar: false,
        brand: 'INGCO',
        model: 'CCVLI202301',

        price: 180000,
        discountPrice: 175000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309973/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ingco_CCVLI202301_-_Cordless_concrete_vibrator.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309974/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ingco_CCVLI202301_-_Cordless_concrete_vibrator.jpg-2.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309976/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ingco_CCVLI202301_-_Cordless_concrete_vibrator.jpg-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309977/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ingco_CCVLI202301_-_Cordless_concrete_vibrator.jpg-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309973/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ingco_CCVLI202301_-_Cordless_concrete_vibrator.jpg-5.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309973/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ingco_CCVLI202301_-_Cordless_concrete_vibrator.jpg-6.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309973/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ingco_CCVLI202301_-_Cordless_concrete_vibrator.jpg-7.jpg'
        ],

        inStock: true,

        description: `INGCO CCVLI202301 Cordless Concrete Vibrator is a powerful battery-operated concrete compaction tool designed to eliminate air pockets and improve the strength, density and finish of freshly poured concrete. Featuring a 35mm vibrator poker and a 1200mm flexible shaft, it is ideal for foundations, columns, beams, slabs and general construction applications. Its cordless design provides maximum mobility and convenience on construction sites without the limitations of power cables.`,

        specification: `
  - Product Name: Cordless Concrete Vibrator
  - Brand: INGCO
  - Model: CCVLI202301
  - Voltage: 20V
  - Battery Platform: INGCO P20S Lithium-Ion System
  - Vibrator Poker Size: 35mm x 1200mm
  - No-Load Speed: 2300 RPM
  - No-Load Current: 2.5A
  - Motor Type: High-Efficiency DC Motor
  - Operation Type: Cordless
  - Application: Concrete Compaction and Air Removal
  - Suitable For: Columns, Beams, Slabs, Foundations and Floors
  - Weight: Approx. 4.2kg (Without Battery)

  Accessories Included:
  - 1 x INGCO CCVLI202301 Concrete Vibrator
  - 1 x 35mm x 1200mm Vibrator Poker

  Note:
  - Battery Sold Separately
  - Charger Sold Separately
  `,

        features: `
  - Cordless design for unrestricted movement
  - Powerful 20V lithium-ion platform
  - 35mm heavy-duty vibrator poker
  - 1200mm flexible shaft for deep concrete pours
  - High-speed 2300 RPM operation
  - Removes trapped air bubbles effectively
  - Improves concrete density and structural strength
  - Lightweight and portable design
  - Comfortable ergonomic grip
  - Suitable for professional construction projects
  - Compatible with INGCO P20S battery system
  - Durable and reliable job-site performance
  `
    },

    {
        id: 19023,
        name: 'Honda Concrete Vibrator with Flexible Hose',
        category: 'concrete-vibrators',
        group: 'construction equipments',

        brand: 'Honda',
        model: 'GX160',
        showSimilar: true,
        price: 350000,
        discountPrice: 320000,
        power: '5.5HP',

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309962/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Honda_Concrete_Vibrator_With_Hose.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309964/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Honda_Concrete_Vibrator_With_Hose-2.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309966/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Honda_Concrete_Vibrator_With_Hose-3.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309970/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Honda_Concrete_Vibrator_With_Hose-4.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309972/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Honda_Concrete_Vibrator_With_Hose-5.webp'
        ],

        inStock: true,

        description: 'The Honda GX160 Concrete Vibrator is a reliable concrete compaction machine designed to remove trapped air bubbles and excess water from freshly poured concrete. Powered by the durable Honda GX160 4-stroke gasoline engine, it delivers consistent vibration performance for construction projects, foundations, columns, beams, slabs, roads, and concrete structures. Its rugged frame and flexible vibrator shaft ensure efficient concrete consolidation and improved structural strength.',

        specification: `
Product Type: Concrete Vibrator
Brand: Honda
Model: GX160
Engine Type: 4-Stroke Gasoline Engine
Engine Model: GX160
Engine Power: 5.5HP
Fuel Type: Petrol
Starting System: Recoil Start
Fuel Tank Capacity: 3.1L
Oil Capacity: 0.6L
Vibrator Type: Flexible Shaft Concrete Vibrator
Application: Concrete Compaction
Frame Material: Heavy-Duty Steel
Cooling System: Air Cooled

Accessories Included:
1 ├ù Honda GX160 Engine Unit
1 ├ù Flexible Vibrator Shaft
1 ├ù Vibrator Poker Head
1 ├ù User Manual
  `,

        features: `
Original Honda GX160 engine
Powerful 5.5HP performance
Efficient concrete compaction
Removes trapped air bubbles
Improves concrete strength and durability
Heavy-duty protective frame
Fuel-efficient 4-stroke engine
Low vibration and smooth operation
Easy recoil starting system
Suitable for slabs, columns and foundations
Portable and easy to transport
Ideal for professional construction projects
  `
    },

    {
        id: 19024,
        name: 'Diesel Engine Concrete Poker Vibrator with 38mm Hose',
        category: 'concrete-vibrators',
        group: 'construction equipments',
        brand: 'Generic',

        price: 650000,
        discountPrice: 620000,

        power: '5HP',
        capacity: '38mm Vibrator Head',

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309928/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/desiel_poker_vibrator.png',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309951/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/desiel_poker_vibrator-2.png'
        ],

        inStock: true,

        description: 'The Diesel Engine Concrete Poker Vibrator with 38mm hose is designed for efficient concrete compaction in construction projects. Powered by a reliable diesel engine, it effectively removes trapped air bubbles and excess water from freshly poured concrete, improving density, strength, and durability. The flexible shaft and 38mm vibrator head make it suitable for columns, beams, slabs, foundations, and other reinforced concrete structures.',

        specification: `
Product Type: Concrete Poker Vibrator
Engine Type: Diesel Engine
Engine Power: 5HP
Fuel Type: Diesel
Starting System: Recoil Start
Vibrator Head Diameter: 38mm
Flexible Hose Length: 6m
Speed: 2800-3600 RPM
Frame Type: Heavy-Duty Steel Frame
Cooling System: Air Cooled
Application: Concrete Compaction

Accessories Included:
1 ├ù Diesel Engine Drive Unit
1 ├ù 38mm Vibrator Poker Head
1 ├ù 6m Flexible Hose Shaft
1 ├ù Tool Kit
1 ├ù User Manual
  `,

        features: `
Powerful diesel engine performance
38mm vibrator poker head
6m flexible shaft hose
Efficient concrete consolidation
Removes trapped air bubbles
Improves concrete strength and durability
Heavy-duty steel frame construction
Low fuel consumption
Easy operation and maintenance
Suitable for slabs, columns, beams and foundations
Portable and job-site ready
Ideal for construction and civil engineering projects
  `
    },

    {
        id: 19025,
        name: 'Robin Jumping Rammer 5.0HP',
        category: 'jumping-rammers',
        group: 'construction equipments',

        brand: 'Robin',
        model: '5.0HP Jumping Rammer',

        price: 1500000,
        discountPrice: 1480000,

        power: '5.0HP',

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305606/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/robin_engine_jumping_rammer-2.jpg',
        ],

        inStock: true,
        showSimilar: true,

        description: 'The Robin 5.0HP Jumping Rammer is a heavy-duty soil compaction machine designed for compacting cohesive soils, trenches, foundations, road works, and construction sites. Powered by a reliable 5.0HP Robin gasoline engine, it delivers powerful impact force for efficient ground compaction in confined areas where larger compactors cannot operate. Its rugged construction ensures durability and long service life under demanding job site conditions.',

        specification: `
Product Type: Jumping Rammer
Brand: Robin
Model: 5.0HP Jumping Rammer
Engine Type: 4-Stroke Petrol Engine
Engine Brand: Robin
Engine Power: 5.0HP
Fuel Type: Petrol
Starting System: Recoil Start
Impact Force: 10-15kN
Jumping Height: 40-65mm
Impact Frequency: 600-700 Times/Min
Travel Speed: 10-13m/Min
Compaction Depth: Up to 500mm
Shoe Size: 330 ├ù 285mm
Fuel Tank Capacity: 2.8L
Application: Soil, Sand and Trench Compaction

Accessories Included:
1 ├ù Jumping Rammer
1 ├ù Tool Kit
1 ├ù User Manual
  `,

        features: `
Powerful 5.0HP Robin engine
High-impact compaction performance
Suitable for trenches and foundations
Excellent maneuverability
Heavy-duty steel construction
Fuel-efficient operation
Reliable recoil starting system
Deep compaction capability
Compact and portable design
Low maintenance requirements
Ideal for road and construction projects
Professional-grade performance
  `
    },


    {
        id: 19026,
        name: 'TOTAL Gasoline Reversible Plate Compactor 9HP',
        category: 'plate-compactors',
        group: 'construction equipments',
        brand: 'TOTAL',

        model: 'TP7160-2',
        power: '6.0kW (9HP)',
        price: 1435000,
        inStock: true,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310120/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/total_Gasoline_reversible_plate_compactor_tp7160-2.jpg'
        ],

        description: 'The TOTAL TP7160-2 Gasoline Reversible Plate Compactor is a heavy-duty compaction machine designed for professional construction, roadwork, paving, landscaping, and trench applications. Equipped with a powerful 9HP gasoline engine and a reversible travel system, it provides excellent compaction performance for soil, gravel, sand, crushed stone, and paving blocks. Its forward and reverse operation improves maneuverability and productivity in confined work areas.',

        specification: `
Brand: TOTAL
Model: TP7160-2
Product Type: Gasoline Reversible Plate Compactor

Engine Type:
Single Cylinder, 4-Stroke, Air-Cooled Gasoline Engine

Engine Power:
6.0kW (9HP)

Engine Displacement:
270cc

Operating Weight:
160kg

Plate Size:
730 ├ù 450mm

Compaction Force:
30.5kN

Frequency:
4000VPM

Compaction Depth:
500mm

Travel Speed:
25cm/s

Working Efficiency:
405m┬▓/hr

Travel Direction:
Forward & Reverse

Fuel Type:
Petrol (Gasoline)

Applications:
Soil Compaction
Sand Compaction
Gravel Compaction
Road Construction
Paving Projects
Trench Backfilling
Foundation Preparation

Included Accessories:
- Operator Handle Assembly
- Engine Assembly
- Base Plate
- Tool Kit
- User Manual
- Wooden Transport Crate
`,

        features: `
Powerful 9HP gasoline engine
Forward and reverse travel operation
Heavy-duty 160kg design
High compaction force of 30.5kN
Compaction depth up to 500mm
Large 730 ├ù 450mm base plate
Suitable for soil, sand and gravel compaction
Excellent maneuverability in confined areas
Durable construction for demanding job sites
Ideal for professional contractors and road works
`,

        capacity: '405m┬▓/hr',

        showSimilar: true
    },

    {
        id: 19027,
        name: 'ZW 3HP Concrete Vibrator Motor - Single Phase - 100% Copper Coil',
        category: 'concrete-vibrators',
        group: 'construction equipment',
        brand: 'ZW',
        model: '3HP Single Phase',

        price: 180000,
        discountPrice: 170000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310133/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ZW_3HP_Concrete_Vibrator_Motor_-_Single_Phase_-_Copper_Coil.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310135/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ZW_3HP_Concrete_Vibrator_Motor_-_Single_Phase_-_Copper_Coil-2.avif',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310138/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ZW_3HP_Concrete_Vibrator_Motor_-_Single_Phase_-Copper_Coil-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310136/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/ZW_3HP_Concrete_Vibrator_Motor_-_Single_Phase_-_Copper_Coil-4.jpg'
        ],

        inStock: true,

        description: `The ZW 3HP Concrete Vibrator Motor is a heavy-duty construction machine designed for powering concrete vibrator shafts during concrete placement and compaction. Equipped with a powerful 3-horsepower single-phase motor and 100% copper winding, it delivers reliable performance, efficient power transmission, and long service life on construction sites. The motor effectively removes trapped air bubbles from freshly poured concrete, helping achieve higher density, strength, and a smooth surface finish. Its robust construction and stable operation make it suitable for residential, commercial, and industrial concrete works.`,

        specification: `
  Product Name: Concrete Vibrator Motor
  Brand: ZW
  Motor Power: 3HP (2.2kW)
  Phase: Single Phase
  Coil Type: 100% Copper Winding
  Voltage: 220V - 240V
  Frequency: 50Hz
  Speed: 2800 RPM
  Cooling Type: Air Cooled
  Duty Type: Continuous Duty
  Housing Material: Heavy-Duty Cast Aluminium
  Application: Concrete Compaction and Vibration
  `,

        features: `
  Powerful 3HP motor for efficient concrete vibration
  100% copper coil for durability and improved performance
  Single-phase operation for easy site installation
  High-speed output for effective air bubble removal
  Heavy-duty construction for demanding job sites
  Energy-efficient motor design
  Low maintenance requirements
  Stable and reliable operation
  Suitable for residential and commercial construction projects
  Long service life under continuous use


   accessories:
  1 x ZW 3HP Concrete Vibrator Motor
  1 x Motor Protection Cover
  1 x User Manual

  Note: Flexible vibrator shaft and poker head are usually sold separately unless specified.
  `,

        power: '3HP (2.2kW)'
    },

    {
        id: 19028,
        name: 'Total TPS890 3.2kW 4.0HP Gasoline Tamping Rammer',
        category: 'jumping-rammers',
        group: 'construction equipment',
        brand: 'Total',
        model: 'TPS890',
        price: 1350000,
        discountPrice: 1300000,

        images: [

            'https://res.cloudinary.com/taus5tit/image/upload/v1786305610/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/Total_TPS890_3.2kW_4.0HP_Gasoline_Tamping_Rammer-1.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305608/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/Total_TPS890_3.2kW_4.0HP_Gasoline_Tamping_Rammer.webp'
        ],

        description:
            'The Total TPS890 Gasoline Tamping Rammer is a professional-grade compaction machine designed for soil, sand, gravel, trenches, foundations, and road maintenance applications. Powered by a 3.2kW (4.0HP) gasoline engine, it delivers high impact force and excellent compaction efficiency for demanding construction projects.',

        specification: `
Model: TPS890
Brand: Total
Engine Type: Gasoline Engine
Engine Power: 3.2 kW (4.0 HP)
Engine Design: Specially Designed for Rammer
Weight: 79 kg
Impact Force: 12 kN
Impact Rate: 640 ΓÇô 680 rpm
Jumping Stroke: 75 mm
Shoe Size: 330 ├ù 290 mm
Fuel Tank Capacity: 2.0 L
Clutch Type: Japanese Clutch
Bellow Type: German Bellow
Application: Soil and Trench Compaction
Packaging: Carton Box

Accessories:
Fuel Tank Assembly
Japanese Clutch
German Bellow
Operating Handle
Compaction Shoe
Tool Kit
User Manual
`,

        features: `
ΓÇó Powerful 3.2kW (4.0HP) gasoline engine
ΓÇó Delivers 12kN impact force
ΓÇó Designed for trench and soil compaction
ΓÇó Durable German bellow construction
ΓÇó High-quality Japanese clutch system
ΓÇó Compact and easy to maneuver
ΓÇó Heavy-duty steel frame
ΓÇó Excellent performance in confined spaces
ΓÇó Fuel-efficient operation
ΓÇó Suitable for construction and road works
ΓÇó Reliable and long-lasting performance
`,

        jumiaLink: '',
        kongaLink: '',
        inStock: true,
        showSimilar: true
    },


    {
        id: 19029,
        name: '150mm Concrete Plastic Cube Mould',
        category: 'testing-equipment',
        group: 'construction equipment',
        brand: 'Generic',

        price: 35000,
        discountPrice: 30000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329595/Astonic-Mart/Products/testing%20equipment/150mm_Concrete_Plastic_Cube_Mould.avif',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329595/Astonic-Mart/Products/testing%20equipment/150mm_Concrete_Plastic_Cube_Mould-2.avif',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329595/Astonic-Mart/Products/testing%20equipment/150mm_Concrete_Plastic_Cube_Mould-4.webp'
        ],

        description: 'The 150mm Concrete Plastic Cube Mould is a durable testing mould used for preparing concrete specimens for compressive strength testing. Manufactured from high-quality engineering plastic, it provides accurate dimensions, easy demoulding and excellent resistance to corrosion, making it suitable for laboratories, construction sites and quality control applications.',

        specification: `
Brand: Generic
Product Type: Concrete Cube Mould
Material: Heavy-Duty Engineering Plastic
Cube Size: 150 ├ù 150 ├ù 150 mm
Internal Dimensions: 150mm ├ù 150mm ├ù 150mm
Mould Type: Single Cube
Construction: Reinforced Plastic Body
Surface Finish: Smooth Interior Surface
Assembly Type: Detachable
Reusable: Yes
Corrosion Resistant: Yes
Chemical Resistant: Yes
Impact Resistant: Yes
Colour: Black or Yellow (varies by manufacturer)
Application: Concrete compressive strength specimen preparation
Standards: BS 1881, ASTM C31, ASTM C39 compliant

Accessories:
150mm Plastic Cube Mould
Fastening Bolts
Wing Nuts
User Guide
`,

        features: `
ΓÇó Manufactured from durable engineering plastic
ΓÇó Lightweight and easy to transport
ΓÇó Smooth inner surface for easy specimen removal
ΓÇó Resistant to rust, chemicals and moisture
ΓÇó Reusable for multiple testing cycles
ΓÇó Accurate dimensions for reliable concrete testing
ΓÇó Easy assembly and dismantling
ΓÇó Suitable for laboratory and on-site applications
ΓÇó Long service life with minimal maintenance
ΓÇó Ideal for civil engineering and quality control testing
`,

        inStock: true,
        showSimilar: true,

        jumiaLink: '',
        kongaLink: ''
    },


    {
        id: 19030,
        name: '150mm Cast Iron Concrete Cube Mould',
        category: 'testing-equipment',
        group: 'construction equipment',
        brand: 'Generic',

        price: 70000,
        discountPrice: 59000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329593/Astonic-Mart/Products/testing%20equipment/150mm_Cast_Iron_Concrete_Cube_Mould-1.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329593/Astonic-Mart/Products/testing%20equipment/150mm_Cast_Iron_Concrete_Cube_Mould-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329593/Astonic-Mart/Products/testing%20equipment/150mm_Cast_Iron_Concrete_Cube_Mould-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329593/Astonic-Mart/Products/testing%20equipment/150mm_Cast_Iron_Concrete_Cube_Mould.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329595/Astonic-Mart/Products/testing%20equipment/150mm_Cast_Iron_Concrete_Cube_Mould-4.jpg'
        ],

        description: 'The 150mm Cast Iron Concrete Cube Mould is a heavy-duty testing mould designed for preparing concrete specimens for compressive strength testing. Manufactured from high-quality cast iron, it provides exceptional durability, dimensional stability and resistance to deformation, making it ideal for laboratories, construction sites and quality control applications.',

        specification: `
Brand: Generic
Product Type: Concrete Cube Mould
Material: Cast Iron
Cube Size: 150 ├ù 150 ├ù 150 mm
Internal Dimensions: 150mm ├ù 150mm ├ù 150mm
Mould Type: Single Cube
Construction: Heavy-Duty Cast Iron Body
Surface Finish: Machined Interior Surface
Assembly Type: Bolt Fastened
Reusable: Yes
Corrosion Resistant: Yes
Wear Resistant: Yes
Weight: Approximately 8ΓÇô12kg
Colour: Black
Application: Concrete compressive strength specimen preparation
Standards: BS 1881, ASTM C31, ASTM C39, EN 12390 compliant

Accessories:
Cast Iron Cube Mould
Fastening Bolts
Wing Nuts
Base Plate
User Guide
`,

        features: `
ΓÇó Heavy-duty cast iron construction
ΓÇó High dimensional accuracy
ΓÇó Resistant to deformation during repeated use
ΓÇó Precision machined interior surfaces
ΓÇó Long service life
ΓÇó Excellent wear resistance
ΓÇó Suitable for laboratory and field testing
ΓÇó Easy assembly and dismantling
ΓÇó Complies with standard concrete testing requirements
ΓÇó Ideal for quality control and civil engineering applications
`,

        inStock: true,
        showSimilar: true,

        jumiaLink: '',
        kongaLink: ''
    },


    {
        id: 19031,
        name: 'GT6-14 Scrap Steel Bar Straightening Machine',
        category: 'rebar-machines',
        group: 'construction equipments',
        brand: 'Generic',
        model: 'GT6-14',
        price: 9500000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786311450/Astonic-Mart/Products/rebar%20machines/GT6-14_Scrap_Steel_Bar_Straightening_Machine.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786311452/Astonic-Mart/Products/rebar%20machines/GT6-14_Scrap_Steel_Bar_Straightening_Machine-2.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786311455/Astonic-Mart/Products/rebar%20machines/GT6-14_Scrap_Steel_Bar_Straightening_Machine-3.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786311456/Astonic-Mart/Products/rebar%20machines/GT6-14_Scrap_Steel_Bar_Straightening_Machine-4.webp'
        ],

        description: 'The GT6-14 Scrap Steel Bar Straightening Machine is a heavy-duty construction machine designed for straightening bent, scrap and recycled reinforcing steel bars. It is suitable for processing used rebars, waste steel bars and coiled steel materials for reuse in construction, fabrication and steel processing industries. The machine offers high efficiency, excellent straightening performance and durable operation for continuous industrial applications.',

        specification: `
Brand: Generic
Model: GT6-14
Type: Scrap Steel Bar Straightening Machine
Straightening Capacity: 6ΓÇô14mm
Suitable Material:
Scrap Rebar
Old Steel Bars
Deformed Rebars
Round Steel Bars
Coiled Steel

Motor Power: 5.5ΓÇô15kW
Voltage: 380V
Frequency: 50Hz
Straightening Speed: 25ΓÇô85m/min
Reducer Type: 250 / 300 Reducer
Straightening Accuracy: ┬▒2mm/m
Minimum Processing Length: 300mm
Operation Mode: Automatic
Drive System: Gear Transmission
Machine Structure: Heavy-Duty Steel Frame
Control System: Electrical Control Cabinet
Cooling System: Air Cooled
Application:
Construction Sites
Steel Processing Plants
Recycling Centers
Fabrication Workshops
Rebar Production Facilities

Accessories:
Electrical Control Cabinet
Straightening Rollers
Feeding Guide Assembly
Tool Kit
Maintenance Tools
User Manual
`,

        features: `
ΓÇó Suitable for straightening 6ΓÇô14mm steel bars
ΓÇó Processes scrap and recycled rebars efficiently
ΓÇó Heavy-duty industrial construction
ΓÇó High-speed operation up to 85m/min
ΓÇó Low maintenance requirements
ΓÇó Stable and reliable performance
ΓÇó High straightening precision
ΓÇó Designed for continuous production
ΓÇó Durable transmission system
ΓÇó Suitable for construction and recycling industries
`,

        capacity: '6ΓÇô14mm',

        inStock: true,
        showSimilar: true,

        jumiaLink: '',
        kongaLink: ''
    },


    {
        id: 19032,
        name: 'Electric Concrete Cement Mortar Trowel Wall Plaster Polishing Machine',
        category: 'power-trowels',
        group: 'construction equipments',
        brand: 'Generic',
        price: 350000,
        discountPrice: 320500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310492/Astonic-Mart/Products/power%20trowel/Electric_Concrete_Cement_Mortar_Trowel_Wall_Plaster_Polishing_Machine.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310496/Astonic-Mart/Products/power%20trowel/Electric_Concrete_Cement_Mortar_Trowel_Wall_Plaster_Polishing_Machine-2.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310500/Astonic-Mart/Products/power%20trowel/Electric_Concrete_Cement_Mortar_Trowel_Wall_Plaster_Polishing_Machine-3.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310503/Astonic-Mart/Products/power%20trowel/Electric_Concrete_Cement_Mortar_Trowel_Wall_Plaster_Polishing_Machine-4.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310507/Astonic-Mart/Products/power%20trowel/Electric_Concrete_Cement_Mortar_Trowel_Wall_Plaster_Polishing_Machine-5.webp'
        ],

        videos: [],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The Electric Concrete Cement Mortar Trowel Wall Plaster Polishing Machine is a high-performance finishing tool designed for smoothing, polishing, and leveling freshly applied cement, mortar, plaster, and concrete surfaces. Featuring a powerful motor with 6 variable speed settings, it delivers consistent performance for professional construction and renovation projects. The ergonomic long-handle design minimizes operator fatigue while allowing comfortable operation on floors and large wall surfaces. It is suitable for contractors, builders, plasterers, and flooring professionals who require smooth, high-quality finishes.`,

        specification: `
- Product Name: Electric Concrete Cement Mortar Trowel Wall Plaster Polishing Machine
- Brand: Generic
- Power Source: Electric
- Motor Type: Copper Motor
- Rated Voltage: 220-240V AC
- Frequency: 50/60Hz
- Rated Power: 1200W
- Speed Settings: 6 Variable Speeds
- No-Load Speed: 1,000 - 3,000 RPM
- Disc Diameter: 380 mm
- Handle Type: Adjustable Long Handle
- Handle Grip: Soft Rubber Grip
- Switch Type: Trigger Switch with Lock Button
- Body Material: High-Strength ABS & Metal
- Disc Material: Steel
- Power Cable Length: Approx. 2 m
- Application: Concrete, Cement, Mortar, Plaster, Epoxy, Floor Finishing
- Weight: Approx. 5.5 kg
- Accessories Included: Polishing Disc, Sponge Pad, Wrench, Carbon Brushes, User Manual
  `,

        features: `
- Powerful motor for efficient polishing and finishing
- 6 variable speed settings for different applications
- Ideal for concrete, cement, mortar, and plaster finishing
- Large polishing disc covers more surface in less time
- Ergonomic long handle reduces operator fatigue
- Soft-grip handle for comfortable operation
- Heavy-duty construction for professional use
- Stable and smooth polishing performance
- Suitable for construction, flooring, wall plastering, and renovation projects
- Easy to operate and maintain
  `
    },

    {
        id: 19033,
        name: 'HBTS80-13-132R Diesel Concrete Pump',
        category: 'concrete-pumps',
        group: 'construction equipments',
        brand: 'Generic',
        price: 75500000,
        discountPrice: 8000000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305612/Astonic-Mart/Products/concrete%20pumps/HBTS80-13-132R_Diesel_Concrete_Pump.png'
        ],

        videos: [],
        showSimilar: true,
        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The HBTS80-13-132R Diesel Concrete Pump is a heavy-duty trailer-mounted concrete pumping machine engineered for high-rise buildings, bridges, tunnels, dams, road construction, and other large-scale concrete placement projects. Powered by a reliable diesel engine, it delivers consistent pumping performance over long horizontal and vertical distances. Its robust hydraulic system, high pumping capacity, and durable construction make it suitable for demanding construction environments where efficiency and reliability are essential.`,

        specification: `
- Product Name: Diesel Concrete Pump
- Model: HBTS80-13-132R
- Machine Type: Trailer-Mounted Concrete Pump
- Brand: Generic
- Power Source: Diesel Engine
- Rated Engine Power: 132 kW
- Maximum Concrete Output: 80 m┬│/h
- Maximum Pumping Pressure: 13 MPa
- Concrete Cylinder Diameter ├ù Stroke: 200 ├ù 1800 mm
- Hopper Capacity: 600 L
- Hopper Height: Approx. 1400 mm
- Maximum Aggregate Size: 40 mm
- Delivery Pipe Diameter: 125 mm
- Maximum Horizontal Pumping Distance: Up to 1200 m
- Maximum Vertical Pumping Distance: Up to 300 m
- Hydraulic System: Double Hydraulic Circuit
- Lubrication System: Automatic
- Cooling System: Hydraulic Oil Cooler
- Control Method: Wired Remote Control
- Chassis Type: Two-Wheel Towable Trailer
- Tyres: Heavy-Duty Pneumatic Wheels
- Fuel Type: Diesel
- Application: Concrete Pumping for Commercial & Industrial Construction
  `,

        features: `
- High pumping capacity of up to 80 m┬│/h
- Powerful 132 kW diesel engine
- High-pressure hydraulic pumping system
- Trailer-mounted design for easy transportation
- Automatic lubrication system reduces maintenance
- Efficient hydraulic oil cooling system
- Suitable for long-distance horizontal and vertical pumping
- Heavy-duty steel hopper with wear-resistant components
- Reliable operation in demanding construction environments
- Ideal for high-rise buildings, bridges, tunnels, dams, roadworks, foundations, and large infrastructure projects
  `
    },


    {
        id: 19034,
        name: 'VR880 2 Ton Ride-On Double Drum Road Roller',
        category: 'drum-roller-compactors',
        group: 'Construction equipments',
        showSimilar: true,
        brand: 'Generic',
        price: 2200000,
        discountPrice: 2100000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306392/Astonic-Mart/Products/drum%20rollers/VR880_2_Ton_Ride-On_Double_Drum_Road_Roller.png',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306393/Astonic-Mart/Products/drum%20rollers/VR880_2_Ton_Ride-On_Double_Drum_Road_Roller-2.jpg'
        ],

        videos: [],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The VR880 2 Ton Ride-On Double Drum Road Roller is a compact vibratory compaction machine designed for asphalt paving, road construction, parking lots, sidewalks, bridges, sports grounds, and foundation projects. Featuring dual steel drums, hydraulic drive, and a powerful diesel engine, it delivers excellent compaction performance while remaining highly maneuverable in confined work areas. It is ideal for contractors, civil engineering projects, and municipal road maintenance.`,

        specification: `
- Product Name: Ride-On Double Drum Road Roller
- Model: VR880
- Operating Weight: 2 Ton
- Brand: Generic
- Drum Type: Double Steel Drum
- Drum Width: Approx. 900 mm
- Drum Diameter: Approx. 560 mm
- Vibration Type: Hydraulic Vibratory System
- Exciting Force: Approx. 30 kN
- Travel Speed: 0 ΓÇô 8 km/h
- Climbing Ability: Up to 30%
- Steering System: Hydraulic Articulated Steering
- Drive Type: Hydraulic Drive
- Engine Type: Single-Cylinder Air-Cooled Diesel Engine
- Starting System: Electric Start
- Water Tank: Integrated Sprinkler System
- Fuel Type: Diesel
- Application: Asphalt, Soil, Gravel, Sand, Pavement & Road Compaction
  `,

        features: `
- 2-ton operating weight for efficient compaction
- Double steel drums for uniform surface finishing
- Powerful diesel engine with electric start
- Hydraulic drive for smooth operation
- High-frequency vibration for superior compaction
- Comfortable ride-on operator platform
- Hydraulic articulated steering for easy maneuverability
- Integrated water sprinkler system prevents asphalt sticking
- Heavy-duty construction for long service life
- Ideal for road construction, pavement repairs, driveways, parking lots, walkways, and landscaping projects
  `
    },

    {
        id: 19036,
        name: 'Walk-Behind Thermoplastic Road Marking Machine',
        category: 'road-marking-machines',
        group: 'Construction equipments',

        brand: 'Generic',
        price: 4200000,
        discountPrice: 4300000,
        showSimilar: true,
        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786311610/Astonic-Mart/Products/road%20marker/Walk-Behind_Thermoplastic_Road_Marking_Machine.png'
        ],

        videos: [],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',
        description: `The Walk-Behind Thermoplastic Road Marking Machine is a professional pavement marking machine designed for applying clear, accurate, and durable traffic lines on roads, highways, parking lots, airports, factories, warehouses, schools, and sports facilities. Powered by a reliable petrol engine, it delivers smooth and consistent paint flow with adjustable line widths for various marking applications. Its ergonomic design, precision guide wheel, and heavy-duty construction ensure efficient operation and long service life under demanding working conditions.`,

        specification: `
- Product Name: Walk-Behind Road Marking Machine
- Machine Type: Thermoplastic / Cold Paint Line Marking Machine
- Brand: Generic
- Engine Type: 4-Stroke Petrol Engine
- Engine Power: 5.5 ΓÇô 6.5 HP
- Starting System: Recoil Start
- Fuel Type: Petrol
- Paint Tank Capacity: Approx. 50 L
- Line Width: 50 ΓÇô 300 mm (Adjustable)
- Marking Speed: Up to 1,500 m/hour
- Spray Method: High-Pressure Airless Spray
- Control System: Digital Speed Controller
- Spray Gun: High-Pressure Precision Spray Gun
- Guide Wheel: Adjustable Front Pointer
- Wheel Type: Pneumatic Rubber Wheels
- Application: Road, Parking Lot, Warehouse & Factory Line Marking
  `,

        features: `
- Produces clean and highly visible road markings
- Adjustable line width for different applications
- Reliable petrol engine for continuous operation
- High-pressure spray system for uniform paint application
- Digital control panel for easy operation
- Ergonomic walk-behind design reduces operator fatigue
- Precision front guide wheel ensures straight line marking
- Heavy-duty steel frame for durability
- Suitable for highways, parking lots, airports, factories, schools, sports courts, and warehouses
- Easy maintenance and transport
  `
    },

    {
        id: 19037,
        name: '20L Thermoplastic Road Marking Machine',
        category: 'road-marking-machines',
        group: 'Construction Machinery',
        parentCategory: 'road construction equipment',
        brand: 'Generic',
        price: 8900000,
        discountPrice: 8495000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786311570/Astonic-Mart/Products/road%20marker/20L_Thermoplastic_Road_Marking_Machine.png'
        ],

        videos: [],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `The 20L Thermoplastic Road Marking Machine is a professional walk-behind pavement marking machine designed for applying thermoplastic road marking paint with high precision and efficiency. Equipped with a pressure tank, glass bead dispenser, hydraulic control system, and a reliable gasoline engine, it produces durable, highly visible traffic markings on highways, streets, airports, parking lots, industrial facilities, and sports courts. It delivers consistent line thickness, excellent adhesion, and smooth operation for demanding road construction projects.`,

        specification: `
- Product Name: Thermoplastic Road Marking Machine
- Capacity: 20 Litres
- Machine Type: Walk-Behind Hot Melt Road Marking Machine
- Brand: Generic
- Engine Type: Honda GX160 Petrol Engine (or Equivalent)
- Engine Power: 5.5 HP
- Fuel Type: Petrol
- Starting System: Recoil Start
- Paint Tank Capacity: 20 L
- Glass Bead Tank Capacity: 10 kg
- Marking Width: 100 / 150 / 200 / 300 / 400 / 450 mm (Interchangeable Marking Shoes)
- Marking Thickness: 1 ΓÇô 5 mm
- Heating Method: External Thermoplastic Preheater Required
- Bead Dispensing: Automatic Surface Glass Bead Dispenser
- Paint Flow Control: Manual Precision Valve
- Pressure System: Hydraulic Pressure Tank
- Guide Wheel: Adjustable Front Guide Pointer
- Wheel Type: Heat-Resistant Rubber Wheels
- Application Speed: 1 ΓÇô 2 km/h
- Construction Material: Heavy-Duty Steel Frame
- Application: Roads, Highways, Airports, Parking Lots, Warehouses & Industrial Facilities
  `,

        features: `
- Heavy-duty 100-litre thermoplastic paint tank
- Suitable for hot melt thermoplastic road marking
- Produces highly visible and durable road markings
- Automatic glass bead dispensing system
- Precision marking shoe for straight, clean lines
- Adjustable line widths for various applications
- Reliable Honda GX160 petrol engine
- Durable hydraulic pressure system
- Ergonomic walk-behind design for easy operation
- Corrosion-resistant heavy-duty steel construction
- Ideal for highways, municipal roads, airports, parking lots, factories, schools, and sports facilities
  `
    },

    {
        id: 19038,
        name: 'Gasoline Powered Concrete Screed (4-Stroke)',
        category: 'concrete-screeders',
        group: 'construction-equipment',
        brand: 'OEM',
        showSimilar: true,
        price: 850000,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305614/Astonic-Mart/Products/concrete%20screeders/Gasoline_Powered_Concrete_Screed_4-Stroke-1.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305615/Astonic-Mart/Products/concrete%20screeders/Gasoline_Powered_Concrete_Screed_4-Stroke-2.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305613/Astonic-Mart/Products/concrete%20screeders/Gasoline_Powered_Concrete_Screed_4-Stroke.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305616/Astonic-Mart/Products/concrete%20screeders/Gasoline_Powered_Concrete_Screed_4-Stroke-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305617/Astonic-Mart/Products/concrete%20screeders/Gasoline_Powered_Concrete_Screed_4-Stroke-4.avif'
        ],

        description:
            'The Gasoline Powered Concrete Screed (4-Stroke) is a professional-grade concrete finishing machine designed for leveling, compacting, and smoothing freshly poured concrete surfaces. Powered by a reliable 4-stroke petrol engine, it delivers consistent vibration that removes trapped air, improves concrete density, and produces a flat, high-quality finish. Suitable for floors, roads, bridges, warehouses, pavements, industrial slabs, and construction projects. The lightweight design, adjustable handle, and interchangeable aluminum screed blades make it easy to operate while increasing productivity on-site.',

        specification: `
ΓÇó Product Type: Gasoline Powered Concrete Screed
ΓÇó Engine Type: 4-Stroke Air-Cooled Petrol Engine
ΓÇó Fuel Type: Gasoline (Petrol)
ΓÇó Starting System: Recoil Start
ΓÇó Engine Power: 1.6 ΓÇô 2.2 HP (depending on model)
ΓÇó Fuel Tank Capacity: Approx. 0.65 ΓÇô 1.0 L
ΓÇó Blade Material: High-Strength Aluminum Alloy
ΓÇó Compatible Blade Lengths:
  - 1.2 m
  - 1.8 m
  - 2.4 m
  - 3.0 m
  - 4.0 m
  - 5.0 m
  - 6.0 m
ΓÇó Vibration Type: High-Frequency Eccentric Vibration
ΓÇó Handle: Adjustable Anti-Vibration Handle
ΓÇó Frame Material: Steel
ΓÇó Engine Cooling: Air-Cooled
ΓÇó Application: Concrete Leveling, Compacting & Finishing
ΓÇó Suitable For: Floors, Warehouses, Roads, Bridges, Slabs, Parking Lots and Construction Sites
`,

        features: `
ΓÇó Reliable 4-stroke gasoline engine
ΓÇó Produces smooth and level concrete finishes
ΓÇó High-frequency vibration removes trapped air bubbles
ΓÇó Increases concrete strength and surface density
ΓÇó Lightweight and easy to maneuver
ΓÇó Adjustable ergonomic anti-vibration handle
ΓÇó Durable aluminum screed blade for long service life
ΓÇó Low fuel consumption and reduced maintenance
ΓÇó Easy recoil starting system
ΓÇó Interchangeable blade lengths for different project sizes
ΓÇó Ideal for residential, commercial and industrial concrete works
ΓÇó Rugged construction for demanding job site conditions
`
    },

    {
        id: 19039,
        name: 'INGCO GCP100-2 Gasoline Plate Compactor',
        category: 'plate-compactors',
        group: 'construction equipments',
        brand: 'INGCO',
        model: 'GCP100-2',
        price: 900000,
        showSimilar: true,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786309984/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/INGCO_GCP100-2_Gasoline_Plate_Compactor.jpg'
        ],

        inStock: true,

        description: 'The INGCO GCP100-2 Gasoline Plate Compactor is a heavy-duty construction machine engineered for compacting granular soils, sand, gravel, asphalt patches, paving blocks, and sub-base materials. Powered by a reliable 6.5HP four-stroke gasoline engine, it delivers high centrifugal force to achieve excellent soil density and surface stability. Its robust steel base plate, vibration-dampening handle, and compact design ensure efficient operation in road construction, landscaping, foundation preparation, drainage projects, and pavement maintenance. Designed for professional contractors and construction crews, the GCP100-2 provides dependable performance, durability, and ease of operation in demanding job site conditions.',

        specification: `
Product Type: Gasoline Plate Compactor
Brand: INGCO
Model: GCP100-2
Engine Type: Single-Cylinder, 4-Stroke, Air-Cooled Gasoline Engine
Engine Power: 6.5HP (196cc)
Fuel Type: Petrol (Gasoline)
Starting System: Recoil Start
Compaction Force: 20kN
Plate Size: 540 ├ù 420mm
Compaction Depth: Up to 300mm
Travel Speed: Up to 40cm/s
Working Efficiency: Up to 660m┬▓/h
Vibration Frequency: Approximately 5,600 VPM
Water Tank: Optional (for asphalt applications)
Handle: Foldable Anti-Vibration Handle
Machine Weight: Approximately 95kg
Application: Soil, Sand, Gravel, Asphalt, Interlocking Blocks and Pavement Base Preparation
  `,

        features: `
Powerful 6.5HP gasoline engine for reliable field performance
Produces 20kN centrifugal force for efficient soil compaction
Heavy-duty steel base plate for maximum durability
Suitable for compacting sand, gravel, soil, and asphalt
Large plate size covers more ground with every pass
Foldable anti-vibration handle improves operator comfort
Excellent manoeuvrability for confined job sites
Strong protective frame safeguards the engine during operation
Easy recoil starting mechanism
Low maintenance and fuel-efficient engine
Ideal for road construction, landscaping, paving, trench work, and foundation preparation
Designed for professional contractors and heavy-duty construction applications
  `,

        accessories: `
Spark Plug Wrench
Tool Kit
Operator's Manual
Engine Oil Funnel
Protective Frame
  `,

        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },

    {
        id: 19040,
        name: 'Oceanic VT-4000 Self-Loading Concrete Mixer (4 Cubic)',
        category: 'concrete-mixers',
        group: 'construction equipment',
        parentCategory: 'concrete-and-road-equipment',
        brand: 'Oceanic',
        model: 'VT-4000',
        price: 83000000,
        showSimilar: true,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305604/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/Oceanic_VT-4000_Self-Loading_Concrete_Mixer_4_Cubic-1.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305605/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/Oceanic_VT-4000_Self-Loading_Concrete_Mixer_4_Cubic-2.avif',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305603/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/Oceanic_VT-4000_Self-Loading_Concrete_Mixer_4_Cubic.jpg'
        ],

        inStock: true,

        description: 'The Oceanic VT-4000 Self-Loading Concrete Mixer is a heavy-duty mobile concrete mixing machine designed to load, weigh, mix, transport, and discharge concrete on-site without the need for a separate batching plant. With a 4 cubic metre mixing capacity, it is ideal for road construction, housing projects, bridges, drainage works, foundations, and remote construction sites. Powered by a 76kW diesel engine, the VT-4000 delivers reliable performance, excellent mobility, and high productivity while reducing labour costs and construction time. Its self-loading bucket, hydraulic mixing system, and rotating drum enable efficient concrete production directly at the job site.',

        specification: `
Product Type: Self-Loading Concrete Mixer
Brand: Oceanic
Model: VT-4000
Mixing Capacity: 4 Cubic Metres (4m┬│)
Rated Power: 76kW
Engine Type: Diesel Engine
Maximum Travelling Speed: 0ΓÇô30km/h
Gradeability: 30%
Loading System: Hydraulic Self-Loading Bucket
Mixing Drum: Hydraulic Driven
Steering: Hydraulic Steering
Drive System: Four-Wheel Drive
Application: Concrete Mixing, Loading, Transporting and Discharging
Manufactured Date: 10/2025
  `,

        features: `
4m┬│ concrete mixing capacity for medium and large construction projects
Hydraulic self-loading bucket eliminates the need for a wheel loader
76kW diesel engine provides powerful and reliable performance
Travelling speed of up to 30km/h for easy movement between job sites
30% gradeability enables operation on uneven terrain
Combines loading, mixing, transporting, and discharging in one machine
Hydraulic drum ensures uniform and consistent concrete mixing
Reduces labour requirements and operating costs
Suitable for roads, bridges, residential buildings, foundations, and civil engineering projects
Heavy-duty construction for demanding site conditions
Easy operation and maintenance
Designed for continuous on-site concrete production
  `,

        accessories: `
Hydraulic Loading Bucket
Operator's Cabin
Mixing Drum
Tool Kit
Operator's Manual
  `,

        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },

    {
        id:19041,
        name: 'Oceanic VT-3500 Self-Loading Concrete Mixer (3.5 Cubic)',
        category: 'concrete-mixers',
        group: 'construction equipments',
        brand: 'Oceanic',
        model: 'VT-3500',
        price: 65000000,
        showSimilar: true,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305601/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/Oceanic_VT-3500_Self-Loading_Concrete_Mixer_3.5_Cubic-1.avif',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305602/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/Oceanic_VT-3500_Self-Loading_Concrete_Mixer_3.5_Cubic-3.avif',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786305600/Astonic-Mart/Products/concrete%20mixers%20and%20rammers/Oceanic_VT-3500_Self-Loading_Concrete_Mixer_3.5_Cubic.jpg'
        ],

        inStock: true,

        description: 'The Oceanic VT-3500 Self-Loading Concrete Mixer is a versatile mobile concrete batching machine designed to load, weigh, mix, transport, and discharge concrete directly at construction sites. With a 3.5 cubic metre mixing capacity, it is ideal for residential buildings, commercial projects, road construction, drainage systems, bridges, and foundation works. Powered by a reliable 76kW diesel engine, the VT-3500 combines excellent mobility, high mixing efficiency, and low operating costs, eliminating the need for separate batching plants and transit mixers. Its hydraulic loading bucket, durable mixing drum, and four-wheel-drive system ensure outstanding performance even on rough construction sites.',

        specification: `
Product Type: Self-Loading Concrete Mixer
Brand: Oceanic
Model: VT-3500
Mixing Capacity: 3.5 Cubic Metres (3.5m┬│)
Rated Power: 76kW
Engine Type: Water-Cooled Diesel Engine
Fuel Type: Diesel
Loading System: Hydraulic Self-Loading Bucket
Mixing Drum: Hydraulic Driven
Drum Rotation: 270┬░
Drive System: Four-Wheel Drive (4WD)
Steering System: Hydraulic Articulated Steering
Maximum Travelling Speed: 0ΓÇô30km/h
Gradeability: 30%
Production Capacity: Up to 14ΓÇô18m┬│/hour
Water Supply System: Automatic
Operator Cabin: Enclosed Cabin
Application: Loading, Mixing, Transporting and Discharging Concrete
  `,

        features: `
3.5m┬│ concrete output per batch for high productivity
Hydraulic self-loading bucket speeds up material loading
Powerful 76kW diesel engine delivers reliable site performance
Four-wheel-drive system provides excellent off-road capability
Hydraulic drum rotates for uniform and consistent concrete mixing
Travelling speed of up to 30km/h allows quick movement between work areas
30% gradeability enables operation on sloped terrain
Reduces labour costs by combining batching, mixing, transport, and discharge in one machine
Strong steel chassis built for demanding construction environments
Suitable for roads, bridges, housing estates, commercial buildings, drainage works, and civil engineering projects
Easy to operate and maintain
Provides consistent, high-quality concrete on-site
  `,

        accessories: `
Hydraulic Loading Bucket
Operator's Cabin
Mixing Drum
Water Tank
Tool Kit
Operator's Manual
  `,

        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },

    {
        id: 19042,
        name: 'Makita Cordless Concrete Vibrator 18V',
        category: 'poker-hoses',
        group: 'construction equipments',
        brand: 'Makita',
        price: 180000,

        showSimilar: true,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310113/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Makita_Cordless_Concrete_Vibrator_18V-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310020/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Makita_Cordless_Concrete_Vibrator_18V.png',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310051/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Makita_Cordless_Concrete_Vibrator_18V-2.png',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310111/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Makita_Cordless_Concrete_Vibrator_18V-3.png',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786310114/Astonic-Mart/Products/poker%20vibrator%20and%20plate%20compactor/Makita_Cordless_Concrete_Vibrator_18V-5.jpg'
        ],

        inStock: true,

        description: 'The Makita 18V Cordless Concrete Vibrator is a portable battery-powered tool designed to eliminate trapped air bubbles from freshly poured concrete. It delivers powerful vibration through a flexible shaft, producing stronger, denser, and smoother concrete finishes without requiring an external power supply. Ideal for foundations, columns, beams, slabs, walls, and precast concrete work.',

        specification: `
Product Type: Cordless Concrete Vibrator
Brand: Makita
Voltage: 18V DC
Battery Type: Lithium-Ion
Power Source: Rechargeable Battery
Motor Type: Brushless (model dependent)
Operation: Cordless
Application: Concrete Compaction
Shaft Type: Flexible
Handle: Ergonomic Soft Grip

Package Includes:
Concrete Vibrator Motor
Flexible Vibrator Shaft
Vibrator Head
2 ├ù 18V Batteries
Battery Charger
Carrying Case
Instruction Manual
`,

        features: `
Portable cordless operation
Powerful vibration for concrete compaction
Lightweight and easy to handle
Rechargeable lithium-ion batteries
Flexible shaft for hard-to-reach areas
Comfortable ergonomic grip
Low maintenance
Durable construction
Fast setup and operation
Ideal for construction and civil engineering projects
`,


        jumiaLink: 'https://www.jumia.com.ng/makita-18v-cordless-concrete-vibrator-kit-lithium-battery-powered-with-flexible-shaft-charger-carry-case-419982019.html',
        kongaLink: ''
    },


    {
        id: 19043,
        name: 'Electric Concrete Floor Grinder',
        category: 'general-concrete-equipment',
        group: 'Construction Equipment',
        brand: 'Generic',
        price: 1700000,

        showSimilar: true,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786306791/Astonic-Mart/Products/general%20concrete%20equipment/Electric_Concrete_Floor_Grinder.png'
        ],

        inStock: true,

        description: 'The Electric Concrete Floor Grinder is a heavy-duty surface preparation machine designed for grinding, smoothing, and leveling concrete floors. Equipped with a powerful electric motor and high-speed rotating grinding disc, it efficiently removes old coatings, adhesives, paint, epoxy, and surface imperfections. The integrated dust extraction port helps maintain a cleaner working environment when connected to an industrial vacuum cleaner. Ideal for contractors, flooring professionals, warehouses, factories, workshops, garages, and construction sites.',

        specification: `
Product Type: Concrete Floor Grinder
Power Source: Electric
Motor Type: Copper Coil Induction Motor
Voltage: 220-240V
Frequency: 50Hz
Power Rating: 3kW (Model Dependent)
Grinding Disc Diameter: 250-300mm
Speed: Approx. 1400-1800RPM
Grinding Method: Diamond Grinding Disc
Dust Port: Yes
Handle Type: Adjustable
Frame Material: Heavy-Duty Steel
Mobility: Wheels for Easy Transport
Application: Concrete Grinding, Epoxy Removal, Surface Preparation and Floor Leveling

Package Includes:
Concrete Floor Grinder
Grinding Disc
Water Hose (if applicable)
Operating Tools
User Manual
`,

        features: `
Heavy-duty industrial construction
Powerful electric motor
Suitable for concrete and terrazzo floors
Removes epoxy, paint and adhesives
Levels uneven concrete surfaces
Supports diamond grinding discs
Integrated dust extraction port
Easy-to-move wheeled design
Low vibration operation
Long service life
Ideal for commercial and industrial flooring projects
`,
        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    }
    // construction equipments ends at 21000
]
