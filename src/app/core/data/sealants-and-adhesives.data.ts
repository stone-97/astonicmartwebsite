import { Product } from '../models/Product';

export const SEALANTS_AND_ADHESIVES_PRODUCTS: Product[] = [


    // start at 6001
    // ends at 7000
    {
        id: 6001,
        name: 'Abro 2000 RTV Silicone Sealant (310g)',
        category: 'silicone-sealants',
        group: 'sealants and adhesives',
        brand: 'ABRO',
        price: 9000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329124/Astonic-Mart/Products/silicone%20and%20adhesives/abro_2000_silicone_sealant.webp',
            // 'https://res.cloudinary.com/taus5tit/image/upload/v1786311378/Astonic-Mart/Products/pumping%20machines/electric_fuel_desiel_transfer_oil_pump.webp2.webp'
        ],

        inStock: true,

        description: 'Abro 2000 RTV Silicone Sealant is a high-quality, general-purpose adhesive sealant designed for sealing, bonding, and insulating a wide range of materials. It cures into a flexible, durable silicone rubber that provides excellent resistance to moisture, temperature changes, and weather conditions. Suitable for both indoor and outdoor use, this sealant is widely used in automotive, construction, marine, and household applications. It adheres effectively to surfaces such as glass, metal, wood, ceramic, porcelain, rubber, and painted materials, making it a versatile solution for sealing joints, repairing leaks, and forming gaskets.',

        specification: `
  Product Type: RTV Silicone Sealant
  Model: Abro 2000 (SS-2000)
  Size: 310g Cartridge
  Color Options: Clear, White, Black, Grey
  Cure Type: Moisture Cure (RTV - Room Temperature Vulcanizing)
  Application Type: Manual or Caulking Gun
  Temperature Resistance: High & Low Temperature Resistant
  Usage: Indoor and Outdoor Applications
  `,

        features: `
  General-purpose sealant for sealing, bonding, and insulating
  Cures into flexible and durable silicone rubber
  Waterproof and weather-resistant
  Strong adhesion to glass, metal, wood, ceramic, rubber, and painted surfaces
  Suitable for automotive gasket making and repairs
  Ideal for windows, panels, doors, and equipment sealing
  Resistant to moisture, chemicals, and environmental conditions
  Can be used for both indoor and outdoor applications
  `,

        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },

    // pu foam sealants
    {
        id: 6002,
        name: 'Soudal PU Foam Sealant 750ml',
        category: 'pu-foam-sealants',
        group: 'sealants and adhesives',
        brand: 'Soudal',
        price: 13500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329230/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_pu_foam_sealant.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329230/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_pu_foam_sealant-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329231/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_pu_foam_sealant-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329231/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_pu_foam_sealant-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329232/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_pu_foam_sealant-5.jpg'
        ],

        inStock: true,

        description: 'Soudal PU Foam Sealant 750ml is a high-quality one-component polyurethane expanding foam designed for professional sealing, filling, fixing, and insulation applications. The foam expands after application to fill gaps, cracks, cavities, and joints, creating an airtight and durable seal. It provides excellent thermal and acoustic insulation while offering strong adhesion to most construction materials, including concrete, brick, wood, metal, stone, plaster, and PVC. The moisture-cured formula ensures reliable performance and long-term durability in both indoor and outdoor environments. It is ideal for installing door and window frames, sealing pipe penetrations, filling wall openings, fixing insulation panels, and reducing air leakage. Once cured, the foam can be cut, sanded, plastered, and painted for a professional finish.',

        specification: `
  Product Type: Polyurethane Expanding Foam Sealant
  Brand: Soudal
  Capacity: 750ml
  Application Type: Aerosol Spray Can
  Cure Type: Moisture-Cured
  Color: Light Yellow
  Foam Yield: Up to 35ΓÇô45 Litres (depending on conditions)
  Density: Low Density Expanding Foam
  Temperature Resistance: -40┬░C to +90┬░C (Cured)
  Application Temperature: +5┬░C to +35┬░C
  Usage: Indoor and Outdoor Applications
  Shelf Life: Up to 12 Months (Unopened)
  `,

        features: `
  High expansion and gap-filling capability
  Excellent thermal insulation performance
  Superior soundproofing and acoustic insulation
  Strong adhesion to most construction materials
  Airtight and water-resistant seal after curing
  Can be cut, sanded, plastered, and painted
  Suitable for door and window frame installation
  Ideal for sealing pipe penetrations and wall cavities
  Fast curing and easy application
  Weather-resistant and durable performance
  Suitable for professional and DIY applications
  `,

        accessories: `
  1 x Soudal PU Foam Sealant 750ml Can
  1 x, Application Straw Nozzle
  `,

        jumiaLink: '',
        kongaLink: ''
    },


    // epoxy adhesives
    {
        id: 6003,
        name: 'Araldite Epoxy Adhesive (Strong Multi-Purpose Bonding Glue)',
        category: 'epoxy-adhesives',
        group: 'sealants and adhesives',
        brand: 'Araldite',
        price: 7500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329130/Astonic-Mart/Products/silicone%20and%20adhesives/araldite_epoxy_adhesive.webp'
        ],

        inStock: true,

        description: 'Araldite Epoxy Adhesive is a high-strength, two-component adhesive system designed for permanent bonding of a wide range of materials. It consists of a resin and hardener that, when mixed, form a tough, durable bond capable of withstanding heavy loads, impact, and environmental stress. Known for its superior bonding strength, Araldite is widely used in industrial, automotive, construction, and household applications. It provides excellent adhesion to metal, wood, glass, ceramic, plastic, and concrete surfaces. Once cured, the adhesive becomes rigid, waterproof, and resistant to chemicals, heat, and vibration, making it ideal for both indoor and outdoor use. It is perfect for repairs, fabrication, maintenance work, and structural bonding where reliability and long-lasting performance are essential.',

        specification: `
  Product Type: Epoxy Adhesive (Two-Part System)
  Brand: Araldite
  Components: Resin + Hardener
  Cure Type: Chemical Cure (Room Temperature)
  Setting Time: 5 minutes to several hours (depending on variant)
  Full Cure Time: 24 hours (approx.)
  Bond Strength: High-strength permanent bonding
  Color: Clear / Amber (varies by type)
  Application: Manual mixing and application
  Usage: Industrial, Automotive, Household, Construction
  `,

        features: `
  Strong and permanent bonding for multiple materials
  Two-component system for high durability and performance
  Resistant to water, chemicals, heat, and impact
  Suitable for metal, wood, glass, ceramic, plastic, and concrete
  Can be used for repairs, filling, and structural bonding
  Excellent gap-filling properties
  Ideal for indoor and outdoor applications
  Easy to mix and apply with reliable results
  `,

        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },


    // construction adhesives
    {
        id: 6004,
        name: 'Soudal PRO 40P Polyurethane Wood Adhesive 750g',
        category: 'construction-adhesives',
        group: 'sealants and adhesives',
        brand: 'Soudal',
        price: 18000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329236/Astonic-Mart/Products/silicone%20and%20adhesives/soudal_wood_glue.webp'
        ],

        inStock: true,

        description: 'Soudal PRO 40P is a professional-grade, one-component polyurethane adhesive designed for high-strength bonding of wood and construction materials. This ready-to-use adhesive offers excellent water resistance (D4 classification), making it suitable for both interior and exterior applications exposed to moisture and harsh weather conditions. It features a unique foaming expansion action that penetrates and fills gaps, pores, and uneven surfaces, ensuring maximum bonding strength and durability. The adhesive cures through moisture reaction, forming a tough and long-lasting bond that can withstand heavy loads, environmental stress, and temperature variations. Ideal for joinery, carpentry, furniture production, and structural wood bonding, it is especially effective on damp or humid wood surfaces and provides reliable performance in demanding industrial and construction environments.',

        specification: `
  Product Type: Polyurethane Wood Adhesive
  Model: PRO 40P
  Size: 750g
  Cure Type: Moisture Cure (PU Adhesive)
  Water Resistance: D4 (EN 204 Standard)
  Open Time: Approx. 15 minutes
  Pressing Time: Minimum 3 hours
  Temperature Resistance: -30┬░C to +100┬░C
  Application: Brush, spatula, or direct application
  Suitable Materials: Wood, timber, construction materials
  `,

        features: `
  One-component ready-to-use polyurethane adhesive
  High-strength bonding for wood and construction materials
  D4 water-resistant (suitable for indoor and outdoor use)
  Foaming action fills gaps and uneven surfaces
  Strong adhesion even on damp or humid wood
  Durable and resistant to weather, moisture, and temperature changes
  Professional-grade performance for carpentry and construction
  Easy application with reliable long-term results
  `,

        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },

    // glues
    {
        id: 6005,
        name: 'Loctite 406 Instant Adhesive (Fast Bonding Cyanoacrylate)',
        category: 'glues',
        group: 'sealants and adhesives',
        brand: 'Loctite',
        price: 15000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329163/Astonic-Mart/Products/silicone%20and%20adhesives/loctite_406_glue.webp'
        ],

        inStock: true,

        description: 'Loctite 406 is a high-performance instant adhesive specifically formulated for fast bonding of plastics and rubber materials. This low-viscosity cyanoacrylate adhesive provides rapid curing within seconds, making it ideal for precision assembly and quick repairs. It delivers strong, reliable bonds on a wide range of substrates including plastics, elastomers, metals, and other smooth surfaces. Loctite 406 is particularly effective when used with primers for difficult-to-bond plastics, ensuring enhanced adhesion and long-term durability. Its quick-setting nature, combined with excellent strength and resistance to environmental factors, makes it suitable for industrial, automotive, electronics, and maintenance applications where speed and performance are critical.',

        specification: `
  Product Type: Instant Adhesive (Cyanoacrylate)
  Brand: Loctite
  Model: 406
  Viscosity: Low (for tight-fitting parts)
  Cure Time: 2 ΓÇô 10 seconds (initial bond)
  Full Cure Time: 24 hours
  Color: Clear
  Temperature Resistance: Up to 120┬░C
  Application: Drop-wise precision bonding
  Suitable Materials: Plastics, Rubber, Metal, Elastomers
  `,

        features: `
  Fast curing within seconds for quick repairs
  Specially formulated for plastics and rubber bonding
  Low viscosity for tight-fitting surfaces
  Strong and durable bonding performance
  Clear finish for neat and invisible joints
  Works on a variety of materials including metal and elastomers
  Suitable for industrial, automotive, and electronics use
  Ideal for precision assembly and maintenance tasks
  `,

        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },

    {
        id: 6006,
        name: 'Abro 99 Multi-Purpose Contact Adhesive (Strong Bond Glue)',
        category: 'contact-adhesives',
        group: 'sealants and adhesives',
        brand: 'Abro',
        price: 20000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329123/Astonic-Mart/Products/silicone%20and%20adhesives/abro_99_contact_adhesives.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329126/Astonic-Mart/Products/silicone%20and%20adhesives/abro-contact-adhesive-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329131/Astonic-Mart/Products/silicone%20and%20adhesives/abro-contact-adhesive-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329126/Astonic-Mart/Products/silicone%20and%20adhesives/abro-contact-adhesive.jpg'
        ],

        inStock: true,

        description: 'Abro 99 Contact Adhesive is a high-strength, multi-purpose bonding solution designed for durable and long-lasting adhesion across a wide range of materials. Formulated for both industrial and household use, this solvent-based adhesive delivers an instant, firm bond when applied to surfaces such as leather, rubber, wood, foam, laminate, fabric, and certain plastics. Its contact bonding mechanism ensures superior grip once both surfaces are coated and allowed to become tacky before joining. Abro 99 is ideal for upholstery work, shoe repairs, furniture manufacturing, automotive interior fixes, and general maintenance tasks. The adhesive provides excellent resistance to heat, moisture, and aging, ensuring reliability even in demanding environments. With its easy application and fast bonding capability, Abro 99 is a trusted choice for professionals and DIY users who require strong, flexible, and permanent bonds.',

        specification: `
  Product Type: Contact Adhesive
  Brand: Abro
  Model: 99
  Base: Solvent-based
  Viscosity: Medium
  Bonding Method: Contact (apply on both surfaces)
  Tack Time: 5 ΓÇô 15 minutes (depending on conditions)
  Full Cure Time: 24 hours
  Color: Yellowish / Light Brown
  Temperature Resistance: Moderate heat resistance
  Application: Brush or spread evenly on surfaces
  Suitable Materials: Leather, Rubber, Wood, Foam, Fabric, Laminate
  `,

        features: `
  Strong and durable multi-purpose bonding
  Ideal for leather, rubber, wood, and upholstery applications
  Contact bonding system for maximum adhesion strength
  ┘à┘é╪ºFast setting with reliable long-term performance
  ┘à┘é╪ºResistant to heat, moisture, and aging
  Flexible bond that withstands stress and movement
  Easy to apply using brush or spreader
  Suitable for industrial, automotive, and home repairs
  Perfect for shoe repairs, furniture work, and DIY projects
  `,

        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },

    // Waterproofsealants
    {
        id: 6007,
        name: 'Waterproof Leak Filler Spray 450ml (Black Roof & Crack Sealant)',
        category: 'waterproof-sealants',
        group: 'sealants and adhesives',
        brand: 'Generic',
        price: 9000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329257/Astonic-Mart/Products/silicone%20and%20adhesives/waterproof_leak_filler_450ml_black.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329261/Astonic-Mart/Products/silicone%20and%20adhesives/waterproof_leak_filler_450ml_black-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329261/Astonic-Mart/Products/silicone%20and%20adhesives/waterproof_leak_filler_450ml_black-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329261/Astonic-Mart/Products/silicone%20and%20adhesives/waterproof_leak_filler_450ml_black-5.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329261/Astonic-Mart/Products/silicone%20and%20adhesives/waterproof_leak_filler_450ml_black-6.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329261/Astonic-Mart/Products/silicone%20and%20adhesives/waterproof_leak_filler_450ml_black-7.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329266/Astonic-Mart/Products/silicone%20and%20adhesives/waterproof_leak_filler_450ml_black-8.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329258/Astonic-Mart/Products/silicone%20and%20adhesives/waterproof_leak_filler_450ml_black-2.jpg'
        ],

        inStock: true,

        description: 'This Waterproof Leak Filler Spray is a fast-acting sealing solution designed to instantly stop water leaks and seal cracks on a wide range of surfaces. Formulated with a rubberized waterproof coating containing asphalt, petroleum resin, and sealing compounds, it creates a tough, flexible, and weather-resistant barrier that prevents further leakage. The spray penetrates deep into cracks and joints, forming a durable protective layer that withstands extreme weather conditions including heavy rain, heat, and moisture. It is ideal for emergency repairs and long-term maintenance on roofs, walls, gutters, pipes, and drainage systems. Its easy spray application allows it to reach difficult areas without tools, making it perfect for both professionals and DIY users. Once cured, it provides a smooth, black waterproof coating that can last for several years without cracking or peeling.',

        specification: `
  Product Type: Waterproof Leak Filler Spray
  Volume: 450ml
  Color: Black
  Form: Aerosol Spray
  Base Material: Rubberized Asphalt / Petroleum Resin
  Drying Time: 20 ΓÇô 30 minutes (touch dry)
  Full Cure Time: 24 hours
  Finish: Flexible waterproof coating
  Coverage: Approx. 8 ΓÇô 10 sq.ft (depending on surface)
  Application Method: Spray
  Suitable Surfaces: Concrete, Metal, PVC, Tiles, Roof Sheets
  `,

        features: `
  Instantly seals leaks and cracks
  Forms a strong waterproof and airtight barrier
  Quick drying with smooth finish
  Elastic and crack-resistant coating
  Withstands extreme weather conditions
  Adheres to multiple surfaces including metal and concrete
  Easy spray application for hard-to-reach areas
  Long-lasting protection (up to 3ΓÇô5 years)
  Ideal for roof repair, pipes, gutters, and wall cracks
  `,


        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },

    // spray paints and leak fillers
    {
        id: 6008,
        name: 'ECO-MAX Stop Leak Filler Spray 450ml',
        category: 'waterproof-sealants',
        group: 'sealants and adhesives',
        brand: 'ECO-MAX',
        price: 8500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329136/Astonic-Mart/Products/silicone%20and%20adhesives/ecomax_waterproof_leak_filler.jpg'
        ],

        inStock: true,

        description: 'ECO-MAX Stop Leak Filler Spray is a rubberized waterproof sealing spray designed to stop leaks, fill small cracks, and provide a protective waterproof barrier. It forms a flexible coating that helps prevent water penetration on roofs, gutters, walls, metal surfaces, pipes, concrete, and other building materials.',

        specification: `
  Brand: ECO-MAX
  Product Type: Waterproof Leak Sealing Spray
  Net Volume: 450ml

  Base Material: Rubberized Waterproof Coating
  Color: Grey

  Application Areas:
  - Roofs
  - Gutters
  - Pipes
  - Metal Sheets
  - Concrete Surfaces
  - Walls
  - Windows
  - Joints and Seams

  Features:
  - Waterproof Protection
  - Leak Sealing
  - Crack Filling
  - Corrosion Protection
  - Weather Resistant
  - Fast Drying

  Application Method:
  Aerosol Spray

  Packaging:
  Pressurized Spray Can
  `,

        features: `
  Stops water leaks quickly
  Forms flexible waterproof membrane
  Seals minor cracks and gaps
  Protects against rust and corrosion
  Suitable for indoor and outdoor use
  Easy aerosol spray application
  Weather-resistant coating
  Good adhesion to multiple surfaces
  Fast drying formula
  Ideal for maintenance and repair work
  `
    },

    {
        id: 6009,
        name: 'Duct Tape 48mm x 55m (2" x 60 Yards)',
        group: 'sealants and adhesives',
        category: 'adhesive-tapes',
        brand: 'Generic',
        model: 'DT-48X55',
        price: 85000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329480/Astonic-Mart/Products/tapes%20and%20adhesives/2_inch_by_55m_duct_tape.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329480/Astonic-Mart/Products/tapes%20and%20adhesives/2_inch_by_55m_duct_tape.jpg-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329480/Astonic-Mart/Products/tapes%20and%20adhesives/2_inch_by_55m_duct_tape.jpg-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329480/Astonic-Mart/Products/tapes%20and%20adhesives/2_inch_by_55m_duct_tape.jpg-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329480/Astonic-Mart/Products/tapes%20and%20adhesives/2_inch_by_55m_duct_tape.jpg-5.jpg'
        ],

        inStock: true,

        description: `The 48mm x 55m Duct Tape is a heavy-duty multipurpose adhesive tape designed for sealing, bundling, patching, repairing, reinforcing, and general maintenance applications. Featuring a strong cloth-backed construction with high-tack adhesive, it provides excellent adhesion to a variety of surfaces including metal, plastic, wood, concrete, fabric, and cardboard. Suitable for industrial, construction, automotive, electrical, packaging, and household use. Items Included in the Package: ΓÇó 1 ├ù Roll of Duct Tape (48mm x 55m)`,

        specification: `Category: Adhesive Tapes
Parent Category: Sealants and Adhesives
Brand: Generic
Model: DT-48X55
Product Type: Duct Tape
Tape Width: 48mm (2 Inches)
Tape Length: 55m (60 Yards)
Backing Material: Polyethylene-Coated Cloth
Adhesive Type: Rubber-Based Adhesive
Color: Silver
Water Resistance: Yes
Tear Resistance: High
Application: Sealing, repairing, bundling and reinforcement
Suitable Use: Industrial, construction, automotive and general-purpose applications`,

        features: `Strong cloth-backed construction
High-tack adhesive bonding
48mm width and 55m length
Water-resistant surface
Excellent tear resistance
Easy hand-tear application
Suitable for indoor and outdoor use
Bonds to multiple surfaces
Ideal for repairs and maintenance
Heavy-duty multipurpose performance`
    },

    {
        id: 6010,
        name: 'Flash Band 50mm x 10m (2" x 10m)',
        group: 'sealants and adhesives',
        category: 'adhesive-tapes',
        brand: 'Generic',
        model: 'FB-50X10',
        price: 5500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329485/Astonic-Mart/Products/tapes%20and%20adhesives/2_inch_flash_band.jpg-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329485/Astonic-Mart/Products/tapes%20and%20adhesives/2_inch_flash_band.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329485/Astonic-Mart/Products/tapes%20and%20adhesives/2_inch_flash_band.jpg-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329485/Astonic-Mart/Products/tapes%20and%20adhesives/2_inch_flash_band.jpg-4.jpg'
        ],

        inStock: true,

        description: `The 2" x 10m Flash Band is a self-adhesive waterproof sealing tape designed for roofing, gutter repairs, pipe sealing, flashing, and general waterproofing applications. Manufactured with an aluminum foil backing and a high-performance bitumen adhesive layer, it provides a durable, weatherproof, and watertight seal on a variety of surfaces including metal, concrete, brick, tiles, glass, and plastic. Suitable for residential, commercial, and industrial maintenance projects. Items Included in the Package: ΓÇó 1 ├ù Roll of Flash Band (50mm x 10m)`,

        specification: `Category: Roofing and Waterproofing
Parent Category: Sealants and Adhesives
Brand: Generic
Model: FB-50X10
Product Type: Flash Band
Tape Width: 50mm (2 Inches)
Tape Length: 10m
Backing Material: Aluminum Foil
Adhesive Type: Self-Adhesive Bitumen
Color: Silver
Water Resistance: Yes
Weather Resistance: Yes
UV Resistance: Yes
Application: Roof repairs, gutter sealing, flashing and waterproofing
Suitable Use: Metal roofs, concrete, brickwork, pipes, skylights and gutters`,

        features: `Self-adhesive installation
Provides a watertight seal
Durable aluminum foil backing
Weather and UV resistant
Excellent adhesion to multiple surfaces
Flexible and easy to apply
Corrosion-resistant finish
Suitable for indoor and outdoor use
Ideal for roofing and waterproofing repairs
Long-lasting performance`
    },

    {
        id: 6011,
        name: 'Flash Band 75mm x 10m (3" x 10m)',
        group: 'sealants and adhesives',
        category: 'adhesive-tapes',
        brand: 'Generic',
        model: 'FB-75X10',
        price: 7500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329509/Astonic-Mart/Products/tapes%20and%20adhesives/3_inch_flash_band.png-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329509/Astonic-Mart/Products/tapes%20and%20adhesives/3_inch_flash_band.png',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329509/Astonic-Mart/Products/tapes%20and%20adhesives/3_inch_flash_band.png-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329509/Astonic-Mart/Products/tapes%20and%20adhesives/3_inch_flash_band.png-3.jpg',

        ],

        inStock: true,

        description: `The 3" x 10m Flash Band is a self-adhesive waterproof sealing tape designed for roofing, gutter repairs, flashing, pipe sealing, and general waterproofing applications. Manufactured with an aluminum foil backing and a durable bitumen adhesive layer, it provides a long-lasting watertight seal on metal, concrete, brick, tiles, glass, and plastic surfaces. Items Included in the Package: ΓÇó 1 ├ù Roll of Flash Band (75mm x 10m)`,

        specification: `Category: Adhesive Tapes
Parent Category: Sealants and Adhesives
Brand: Generic
Model: FB-75X10
Product Type: Flash Band
Tape Width: 75mm (3 Inches)
Tape Length: 10m
Backing Material: Aluminum Foil
Adhesive Type: Self-Adhesive Bitumen
Color: Silver
Water Resistance: Yes
Weather Resistance: Yes
UV Resistance: Yes
Application: Roof repairs, flashing, gutter sealing and waterproofing
Suitable Use: Metal roofs, concrete, brickwork, gutters and pipes`,

        features: `Self-adhesive waterproof tape
75mm wide coverage
Durable aluminum foil backing
Weather and UV resistant
Excellent surface adhesion
Flexible and easy to apply
Corrosion-resistant finish
Suitable for indoor and outdoor use
Provides long-lasting watertight seals
Ideal for roofing and maintenance repairs`
    },

    {
        id: 6012,
        name: 'Flash Band 100mm x 10m (4" x 10m)',
        category: 'adhesive-tapes',
        parentCategory: 'sealants and adhesives',
        brand: 'Generic',
        model: 'FB-100X10',
        price: 9500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329523/Astonic-Mart/Products/tapes%20and%20adhesives/4_inch_flash_band.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329516/Astonic-Mart/Products/tapes%20and%20adhesives/4_inch_flash_band.jpg-2.jpg',
            'src/https://res.cloudinary.com/taus5tit/image/upload/v1786329523/Astonic-Mart/Products/tapes%20and%20adhesives/4_inch_flash_band.jpg-3.jpg'
        ],

        inStock: true,

        description: `The 4" x 10m Flash Band is a heavy-duty self-adhesive waterproof sealing tape designed for roofing, flashing, gutter repairs, leak sealing, and general waterproofing applications. Its aluminum foil backing and high-performance bitumen adhesive provide a durable, weatherproof barrier against water penetration and corrosion. Items Included in the Package: ΓÇó 1 ├ù Roll of Flash Band (100mm x 10m)`,

        specification: `Category: Adhesive Tapes
Parent Category: Sealants and Adhesives
Brand: Generic
Model: FB-100X10
Product Type: Flash Band
Tape Width: 100mm (4 Inches)
Tape Length: 10m
Backing Material: Aluminum Foil
Adhesive Type: Self-Adhesive Bitumen
Color: Silver
Water Resistance: Yes
Weather Resistance: Yes
UV Resistance: Yes
Application: Roof repairs, flashing, gutter sealing, waterproofing and leak prevention
Suitable Use: Metal roofs, concrete, brickwork, skylights, gutters and pipe joints`,

        features: `Heavy-duty waterproof sealing tape
100mm wide coverage for larger repairs
Durable aluminum foil backing
Weatherproof and UV resistant
Strong bitumen adhesive
Flexible and easy to install
Corrosion-resistant surface
Suitable for indoor and outdoor applications
Provides long-lasting leak protection
Ideal for roofing and waterproofing projects`
    },


    {
        id: 6013,
        name: 'Soujplus Weather Sealant',
        category: 'silicone-sealants',
        group: 'sealants and adhesives',
        brand: 'Soujplus',
        price: 8500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329254/Astonic-Mart/Products/silicone%20and%20adhesives/soulj_plus_sealants-2.png',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329250/Astonic-Mart/Products/silicone%20and%20adhesives/soulj_plus_sealants.png'
        ],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `
Soujplus Weather Sealant is a high-performance sealing compound designed for weatherproofing, waterproofing and gap sealing applications in construction, glazing and industrial environments. It provides strong adhesion and flexible sealing performance on various surfaces including glass, aluminum, metal, concrete, ceramic and plastic.
Formulated for both indoor and outdoor use, the sealant helps prevent water leakage, dust penetration and air infiltration while maintaining long-lasting flexibility under varying weather conditions. Its durable weather-resistant composition makes it suitable for windows, doors, roofing joints, curtain walls and general sealing applications.
The easy-to-apply formula ensures smooth application and reliable bonding performance for residential, commercial and industrial projects.
  `,

        specification: `
  - Product Name: Weather Sealant
  - Brand: Soujplus
  - Type: Weatherproof Sealant
  - Application: Sealing and Waterproofing
  - Surface Compatibility: Glass, Metal, Aluminum, Concrete and Plastic
  - Weather Resistance: Yes
  - Water Resistance: Yes
  - Flexibility: High Flexibility
  - Usage: Indoor and Outdoor
  `,

        features: `
  - Strong adhesion performance
  - Weatherproof and waterproof sealing
  - Flexible and durable formula
  - Suitable for indoor and outdoor use
  - Helps prevent water and air leakage
  - Easy application and smooth finish
  - Suitable for glass and aluminum applications
  - Resistant to cracking and shrinking
  - Multi-surface compatibility
  - Ideal for construction and maintenance work
  `
    },


    {
        id: 6014,
        name: 'Cubana Construction Adhesive',
        category: 'construction-adhesives',
        group: 'sealants and adhesives',

        brand: 'Cubana',

        price: 7500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329130/Astonic-Mart/Products/silicone%20and%20adhesives/cubana_construction_adhesive.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329130/Astonic-Mart/Products/silicone%20and%20adhesives/cubana_construction_adhesive-2.jpg'
        ],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `
Cubana Construction Adhesive is a high-strength bonding adhesive designed for heavy-duty construction, installation and maintenance applications. It provides strong and durable adhesion on a wide range of building materials including wood, concrete, metal, ceramics, glass, PVC and other construction surfaces.
Formulated for professional and industrial use, the adhesive offers excellent bonding performance, weather resistance and long-lasting durability for both indoor and outdoor applications. It is suitable for fixing panels, tiles, trims, boards, moldings and general construction assemblies.
Its easy-to-apply formula ensures reliable bonding while reducing the need for mechanical fasteners in many applications, making it ideal for contractors, fabricators and maintenance professionals.
  `,

        specification: `
  - Product Name: Construction Adhesive
  - Brand: Cubana
  - Type: Heavy-Duty Construction Adhesive
  - Application: Bonding and Installation
  - Surface Compatibility: Wood, Metal, Concrete, PVC, Glass and Ceramic
  - Weather Resistance: Yes
  - Water Resistance: Yes
  - Usage: Indoor and Outdoor
  - Application Method: Cartridge / Sealant Gun
  `,

        features: `
  - Strong and durable bonding performance
  - Suitable for multiple construction materials
  - Weather-resistant formulation
  - Indoor and outdoor application
  - Easy application using sealant gun
  - Fast and reliable adhesion
  - Reduces need for mechanical fasteners
  - Suitable for professional construction work
  - Long-lasting bonding strength
  - Ideal for installation and maintenance applications
  `
    },

    {
        id: 6015,
        name: 'Asmaco PU Foam Sealant 750ml (12 Pieces)',
        category: 'pu-foam-sealants',
        group: 'sealants and adhesives',

        brand: 'Asmaco',

        price: 13500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329130/Astonic-Mart/Products/silicone%20and%20adhesives/asmaco_pu_foam_sealant.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329231/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_pu_foam_sealant-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329231/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_pu_foam_sealant-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329232/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_pu_foam_sealant-5.jpg'
        ],

        inStock: true,
        jumiaLink: '',
        kongaLink: '',

        description: `Asmaco PU Foam Sealant 750ml (12 Pieces) is a professional bulk pack of one-component moisture-curing polyurethane expanding foam designed for sealing, filling, insulating and fixing applications in construction, plumbing, electrical and maintenance environments. The foam provides strong adhesion to concrete, brick, wood, PVC, metal and various building materials, making it ideal for window installation, door fitting, cavity filling and insulation work. Its fast-expanding and quick-curing formulation offers excellent thermal and acoustic insulation while helping prevent air and moisture penetration. Suitable for indoor and outdoor professional applications, the 12-piece pack is ideal for contractors, workshops and large-scale maintenance operations.`,

        specification: `
  - Product Name: PU Foam Sealant
  - Brand: Asmaco
  - Quantity: 12 Pieces
  - Capacity Per Can: 750ml
  - Type: Moisture-Curing Polyurethane Foam
  - Expansion Type: Expanding Foam Sealant
  - Application: Filling, Sealing and Insulation
  - Adhesion Surface: Concrete, Wood, Metal, PVC and Brick
  - Feature: Thermal and Acoustic Insulation
  - Usage: Construction and Industrial Applications
  `,

        features: `
  - Bulk pack of 12 foam sealant cans
  - Fast-expanding polyurethane foam
  - Excellent gap filling capability
  - Strong adhesion to multiple surfaces
  - Thermal and sound insulation properties
  - Moisture-resistant foam structure
  - Quick-curing performance
  - Suitable for indoor and outdoor use
  - Ideal for doors, windows and cavities
  - Suitable for construction and repair projects
  `
    },

    {
        id: 6016,
        name: 'MAGPOW 303 Contact Adhesive',
        category: 'silicone-sealants',
        group: 'sealants and adhesives',
        price: 19000,
        brand: 'MAGPOW',
        model: '303',


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329163/Astonic-Mart/Products/silicone%20and%20adhesives/MAGPOW_303_Contact_Adhesive-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329166/Astonic-Mart/Products/silicone%20and%20adhesives/MAGPOW_303_Contact_Adhesive-3.avif',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329163/Astonic-Mart/Products/silicone%20and%20adhesives/MAGPOW_303_Contact_Adhesive.jpg'
        ],

        inStock: false,
        jumiaLink: '',
        kongaLink: '',

        description: `MAGPOW 303 Contact Adhesive is a high-performance multipurpose bonding adhesive designed for strong and durable bonding applications in woodworking, furniture production, upholstery, leatherwork, automotive interiors, decoration and industrial maintenance. Formulated with fast-drying contact bonding technology, the adhesive delivers reliable adhesion on surfaces such as wood, rubber, leather, foam, laminate, fabric, plastic and metal. Its easy-spread formulation ensures smooth application and quick bonding performance for professional and industrial use. Suitable for workshops, construction sites, furniture manufacturing and repair operations, MAGPOW 303 provides long-lasting industrial-grade bonding strength.`,

        specification: `
  - Product Name: Contact Adhesive
  - Brand: MAGPOW
  - Model: 303
  - Type: Multipurpose Contact Adhesive
  - Available Sizes: 400ml, 800ml and 3KG
  - Adhesive Application: Bonding and Repair Work
  - Suitable Materials: Wood, Rubber, Leather, Foam, Fabric, Plastic and Metal
  - Drying Type: Fast-Drying Formula
  - Bond Strength: High-Strength Industrial Bond
  - Application Method: Brush or Spreader Application
  - Packaging Type: Tin Container
  - Usage: Industrial and Professional Applications
  `,

        features: `
  - Strong industrial-grade bonding performance
  - Fast-drying adhesive formulation
  - Durable long-lasting adhesion strength
  - Suitable for multiple surface materials
  - Smooth and easy application
  - Ideal for woodworking and upholstery
  - Reliable multipurpose repair adhesive
  - Suitable for furniture and automotive use
  - Professional workshop application
  - Available in multiple packaging sizes
  `
    },


    {
        id: 6017,
        name: 'Soudal Sausage Silicone Sealant 600ml',
        category: 'silicone-sealants',
        group: 'sealants and adhesives',
        showSimilar: true,
        brand: 'Soudal',
        model: '600ml Sausage',

        price: 15500,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329236/Astonic-Mart/Products/silicone%20and%20adhesives/soudal_wood_glue.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329234/Astonic-Mart/Products/silicone%20and%20adhesives/soudal_silicone_sealant.png'
        ],

        inStock: true,

        jumiaLink: '',
        kongaLink: '',

        description: `
Soudal Sausage Silicone Sealant is a professional-grade one-component sealant supplied in a convenient sausage pack for use with sausage applicator guns. It is designed for sealing, bonding and weatherproofing applications in construction, glazing, roofing, curtain wall installations, aluminum fabrication, sanitary installations and general building maintenance. The sealant provides excellent adhesion, flexibility and long-lasting resistance to weathering, moisture and UV exposure.
`,

        specification: `
  - Product Name: Sausage Silicone Sealant
  - Brand: Soudal
  - Packaging Type: Sausage Pack
  - Capacity: 600ml
  - Sealant Type: Silicone Sealant
  - Curing System: Moisture Cure
  - Application Temperature: +5┬░C to +35┬░C
  - Movement Capability: High Elasticity
  - UV Resistance: Yes
  - Weather Resistance: Excellent
  - Water Resistance: Yes
  - Application: Glazing, Construction, Roofing, Curtain Walls and General Sealing

  Accessories Included:
  - 1 x Soudal Sausage Silicone Sealant 600ml
  `,

        features: `
  - Professional-grade silicone sealant
  - Large 600ml sausage packaging
  - Excellent adhesion to common building materials
  - High flexibility after curing
  - UV and weather resistant
  - Waterproof sealing performance
  - Long service life
  - Suitable for indoor and outdoor applications
  - Easy application with sausage gun
  - Low maintenance requirements
  - Durable and reliable sealing solution
  - Ideal for construction and glazing work
  `
    },

    {
        id: 6018,
        name: 'Rongchang RC-791 Neutral Silicone Sealant (Sausage Type)',
        category: 'silicone-sealants',
        group: 'Building Materials And Accessories',
        parentCategory: 'Adhesives And Sealants',
        childCategory: 'Silicone Sealants',
        showSimilar: true,
        brand: 'Rongchang',
        model: 'RC-791',

        price: 14500,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329200/Astonic-Mart/Products/silicone%20and%20adhesives/ronchang_silicone_sealant.png',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329184/Astonic-Mart/Products/silicone%20and%20adhesives/ronchang_silicone_sealant.png-2.png'
        ],

        inStock: true,


        description: `Rongchang RC-791 Neutral Silicone Sealant is a high-performance one-component neutral curing sealant designed for construction, glazing and general sealing applications. The sausage-pack format is ideal for professional use with bulk sealant guns, offering cost-effective application and reduced packaging waste. It provides excellent adhesion to glass, aluminum, concrete, stone, ceramic, steel and many other building materials without causing corrosion.`,

        specification: `
  - Product Name: Neutral Silicone Sealant
  - Brand: Rongchang
  - Model: RC-791
  - Type: Neutral Cure Silicone Sealant
  - Package Type: Sausage Pack
  - Capacity: 590ml
  - Colour: White / Black / Grey (Depending on Variant)
  - Curing System: Neutral Cure
  - Weather Resistant: Yes
  - UV Resistant: Yes
  - Waterproof: Yes
  - Temperature Resistance: Excellent
  - Adhesion: Glass, Aluminum, Steel, Concrete, Ceramic, Stone and PVC
  - Application Method: Sausage Sealant Gun
  - Shelf Life: Up to 12 Months
  - Suitable For: Construction, Curtain Walls, Window Glazing, Expansion Joints and General Sealing

  Accessories Included:
  - 1 x RC-791 Neutral Silicone Sealant Sausage Pack
  `,

        features: `
  - Professional-grade construction sealant
  - Neutral curing formulation
  - Excellent weather resistance
  - UV-resistant performance
  - Strong adhesion to multiple substrates
  - Non-corrosive to metals
  - Waterproof and durable
  - Suitable for indoor and outdoor use
  - Flexible after curing
  - Easy application with sausage gun
  - Ideal for glazing and facade works
  - Long-lasting sealing performance
  `
    },

    {
        id: 6019,
        name: 'Fischer Injection Mortar FIS V Plus 360 S',
        category: 'silicone-sealants',
        group: 'sealants and adhesives',
        showSimilar: true,
        brand: 'Fischer',
        model: 'FIS V Plus 360 S',

        price: 59000,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329155/Astonic-Mart/Products/silicone%20and%20adhesives/fischer_FIS_V_Plus_360_S.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329156/Astonic-Mart/Products/silicone%20and%20adhesives/fischer_FIS_V_Plus_360_S-2.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329156/Astonic-Mart/Products/silicone%20and%20adhesives/fischer_FIS_V_Plus_360_S-3.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329156/Astonic-Mart/Products/silicone%20and%20adhesives/fischer_FIS_V_Plus_360_S-4.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329157/Astonic-Mart/Products/silicone%20and%20adhesives/fischer_FIS_V_Plus_360_S-5.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329160/Astonic-Mart/Products/silicone%20and%20adhesives/fischer_FIS_V_Plus_360_S-6.webp'
        ],

        inStock: true,

        description: `Fischer FIS V Plus 360 S is a high-performance vinyl ester hybrid injection mortar designed for heavy-duty anchoring and fixing applications in concrete, masonry and reinforced structures. It is suitable for cracked and non-cracked concrete, solid and hollow masonry, rebar connections and structural fastening applications. The mortar delivers excellent load-bearing performance, long-term durability and reliable anchoring even in water-filled boreholes, making it ideal for construction, industrial and infrastructure projects.`,

        specification: `
  - Product Name: Injection Mortar
  - Brand: Fischer
  - Model: FIS V Plus 360 S
  - Mortar Type: Vinyl Ester Hybrid Injection Mortar
  - Cartridge Capacity: 360ml
  - Colour: Grey
  - Curing System: Two-Component Chemical Mortar
  - Styrene Free: Yes
  - Suitable Materials: Concrete, Hollow Blocks, Solid Brick, Aerated Concrete and Masonry
  - Approved For: Rebar Connections and Heavy-Duty Anchoring
  - Service Life: Up to 100 Years
  - Application In Water-Filled Boreholes: Yes
  - Temperature Resistance: High Performance
  - Installation Method: Injection Gun Application
  - Standards: ETA Approved
  - Application: Structural Anchoring, Steel Structures, Machinery Installation, Railings, Facades and Construction Works

  Accessories Included:
  - 1 x Fischer FIS V Plus 360 S Cartridge (360ml)
  - 2 x Fischer FIS MR Plus Static Mixers
  `,

        features: `
  - High-strength chemical anchoring system
  - Suitable for cracked and non-cracked concrete
  - Excellent performance in masonry applications
  - Approved for post-installed rebar connections
  - Can be used in water-filled boreholes
  - Styrene-free formulation
  - Long service life of up to 100 years
  - High load-bearing capacity
  - Reliable bonding and sealing performance
  - Suitable for indoor and outdoor applications
  - Professional-grade construction solution
  - Compatible with Fischer anchoring systems
  `
    },

    {
        id: 6020,
        name: 'Soudal Silirub AC+ Sanitary Sealant',
        category: 'silicone-sealants',
        group: 'sealants and adhesives',
        brand: 'Soudal',
        price: 10500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429756/Astonic-Mart/Products/remaining-local-images/g917rcyavv2rziavx1ob.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429757/Astonic-Mart/Products/remaining-local-images/cpj62pdvykzeemuwkqzt.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429758/Astonic-Mart/Products/remaining-local-images/umqmyv0o54hw9cpc9qfj.jpg'
        ],

        inStock: true,

        description: 'Soudal Silirub AC+ Sanitary Sealant is a premium one-component, permanently elastic silicone sealant specially formulated for sanitary applications. It provides excellent adhesion to glass, ceramics, enamel, and glazed surfaces while offering long-lasting protection against mold and fungal growth. Ideal for sealing joints in kitchens, bathrooms, showers, sinks, toilets, and other wet environments where hygiene and water resistance are essential.',

        specification: `
  Product Type: Sanitary Silicone Sealant
  Brand: Soudal
  Model: Silirub AC+
  Base Material: Acetoxy Silicone
  Color: White / Transparent (Variant Dependent)
  Cartridge Size: 300ml
  Skin Formation Time: Approx. 5 - 10 Minutes
  Curing System: Moisture Cure
  Elasticity: Permanently Elastic
  Water Resistance: Excellent
  UV Resistance: Yes
  Anti-Fungal Protection: Yes
  Application Temperature: +5┬░C to +35┬░C

  Accessories:
  1 x Soudal Silirub AC+ Sanitary Sealant Cartridge
  `,

        features: `
  Premium sanitary-grade silicone sealant
  Excellent resistance to mold and fungal growth
  Permanently flexible after curing
  Superior waterproof sealing performance
  Excellent adhesion to glass, ceramics, porcelain, and enamel
  Resistant to humidity and cleaning agents
  UV-resistant for long-lasting durability
  Suitable for kitchens, bathrooms, showers, sinks, and sanitary installations
  Easy application using standard caulking guns
  Professional-quality finish with long service life
  `,

        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },

    {
        id: 6021,
        name: 'Heavy Duty Caulking Gun for Silicone Sealants',
        category: 'caulking-foam-guns',
        group: 'sealants and adhesives',
        brand: 'Generic',
        price: 9500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304652/Astonic-Mart/Products/caulking%20foam%20guns/caulking_guns.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304653/Astonic-Mart/Products/caulking%20foam%20guns/caulking_guns-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304654/Astonic-Mart/Products/caulking%20foam%20guns/caulking_guns-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304656/Astonic-Mart/Products/caulking%20foam%20guns/caulking_guns-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304657/Astonic-Mart/Products/caulking%20foam%20guns/caulking_guns-5.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304658/Astonic-Mart/Products/caulking%20foam%20guns/caulking_guns-6.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304659/Astonic-Mart/Products/caulking%20foam%20guns/caulking_guns-7.jpg'
        ],

        inStock: true,

        description: `The Heavy Duty Caulking Gun is designed for the smooth and controlled application of silicone sealants, acrylic sealants, PU sealants, and adhesive cartridges. Constructed from durable steel with a reinforced frame, it provides excellent dispensing force while minimizing hand fatigue during prolonged use. Suitable for glazing, construction, plumbing, HVAC installation, automotive work, and general maintenance applications. The ergonomic trigger mechanism ensures precise bead control for professional sealing and bonding jobs.

Items Included in the Package:
ΓÇó 1 ├ù Heavy Duty Caulking Gun
ΓÇó User Guide`,

        specification: `
Category: Silicone Sealant Guns
Parent Category: Sealants Accessories
Child Category: Sealant Guns
Brand: Generic
Model: Manual Caulking Gun
Material: Steel Frame
Handle Type: Ergonomic Trigger Handle
Cartridge Capacity: 280ml ΓÇô 310ml
Rod Type: Hex Rod
Application: Sealant & Adhesive Dispensing
Suitable For: Silicone, Acrylic, PU Sealants
Color: Orange
Operation: Manual
Packing Type: Carton Box
`,

        features: `
Heavy-duty steel construction
Suitable for 280ml and 310ml cartridges
Smooth trigger action
Professional dispensing control
Durable hex push rod
Ergonomic grip handle
Easy cartridge loading
Corrosion-resistant finish
Ideal for silicone sealants
Compatible with PU sealants
Suitable for acrylic sealants
Precise bead application
Low maintenance design
Lightweight and portable
Suitable for professional and DIY use

Accessories:
User Guide
`,

        capacity: '310ml',
        power: 'Manual'
    },

    {
        id: 6022,
        name: 'Heavy Duty PTFE Coated PU Foam Gun',
        category: 'caulking-foam-guns',
        group: 'sealants and adhesives',
        brand: 'Generic',
        price: 18000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304660/Astonic-Mart/Products/caulking%20foam%20guns/Pu_Expanding_Foam_Gun%2C.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304653/Astonic-Mart/Products/caulking%20foam%20guns/caulking_guns-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304661/Astonic-Mart/Products/caulking%20foam%20guns/Pu_Expanding_Foam_Gun-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304661/Astonic-Mart/Products/caulking%20foam%20guns/Pu_Expanding_Foam_Gun-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304662/Astonic-Mart/Products/caulking%20foam%20guns/Pu_Expanding_Foam_Gun-5.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304663/Astonic-Mart/Products/caulking%20foam%20guns/Pu_Expanding_Foam_Gun-6.jpg'
        ],

        inStock: true,

        description: `The Heavy Duty PTFE Coated PU Foam Gun is a professional-grade dispensing tool designed for the precise application of polyurethane expanding foam. Featuring a durable PTFE-coated body, needle, and adapter, it minimizes foam adhesion and simplifies cleaning after use. The adjustable flow control mechanism allows accurate foam output for sealing, insulating, filling gaps, window installation, door fitting, roofing, and general construction applications. Built for contractors, builders, installers, and maintenance professionals, this foam gun delivers reliable performance, durability, and excellent control on demanding job sites.

Items Included in the Package:
ΓÇó 1 ├ù Heavy Duty PTFE Coated PU Foam Gun
ΓÇó User Guide`,

        specification: `
Category: Caulking Foam Guns
Group: Sealants and Adhesives
Brand: Generic
Model: PTFE Coated PU Foam Gun
Material: Aluminum Alloy & Steel
Coating: PTFE Non-Stick Coating
Operation: Manual
Flow Control: Adjustable Trigger and Regulator
Application: PU Expanding Foam Dispensing
Suitable Foam Type: Standard Screw-On Foam Canisters
Handle Type: Ergonomic Grip
Cleaning: Easy-Clean PTFE Surface
Color: Black / Silver
Packing Type: Carton Box
`,

        features: `
Heavy-duty professional construction
PTFE non-stick coated body
Adjustable foam flow control
Precise foam application
Durable aluminum alloy frame
Corrosion-resistant components
Easy-clean design
Leak-resistant adapter connection
Comfortable ergonomic handle
Suitable for PU expanding foam
Reduces foam wastage
Excellent dispensing accuracy
Ideal for sealing and insulation work
Long service life
Suitable for professional and DIY applications

Accessories:
User Guide
`,

        capacity: 'Standard Foam Canisters',
        power: 'Manual'
    },

    {
        id: 6023,
        name: 'Professional Silicone Sealant Gun 400ml',
        category: 'caulking-foam-guns',
        group: 'sealants and adhesives',
        brand: 'Generic',
        price: 8500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786307540/Astonic-Mart/Products/guns%20and%20trimmers/Professional_Silicone_Sealant_Gun_400ml.jpg',

        ],

        inStock: true,

        description: 'The Professional Silicone Sealant Gun 400ml is a durable and easy-to-use hand tool designed for applying sealants, adhesives, and caulking materials with precision. Built with a strong metal frame and smooth trigger mechanism, it ensures consistent flow control for clean and professional results.\n\nThis sealant gun is compatible with standard 400ml cartridges, making it suitable for a wide range of applications including construction, plumbing, glazing, and general maintenance. Its ergonomic handle design reduces hand fatigue, allowing for extended use without discomfort.\n\nThe anti-drip mechanism helps prevent material wastage and mess, while the sturdy push rod ensures efficient dispensing of thick sealants. It is ideal for both professionals and DIY users who require accuracy and reliability in sealant application.',

        specification: `
  Capacity: 400ml
  Material: Heavy-duty metal frame
  Handle Type: Ergonomic grip
  Operation: Manual trigger mechanism
  Rod Type: Smooth push rod
  Compatibility: Standard sealant and silicone cartridges
  Application: Sealing, bonding, filling gaps and joints
  `,

        features: `
  Compatible with standard 400ml sealant cartridges
  Strong and durable metal construction
  Smooth trigger action for controlled application
  Anti-drip design to reduce mess and waste
  Ergonomic handle for comfortable use
  Ideal for silicone, adhesives, and caulking jobs
  Suitable for construction, plumbing, and DIY tasks
  Lightweight and easy to handle
  `
    },



    {
        id: 6024,
        name: 'TOTAL Caulking Gun 9" (THT21809)',
        category: 'caulking-foam-guns',
        group: 'sealants and adhesives',
        brand: 'TOTAL',
        model: 'THT21809',
        price: 10000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304667/Astonic-Mart/Products/caulking%20foam%20guns/TOTAL_CAULKING_GUN_9inch_THT21809.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304667/Astonic-Mart/Products/caulking%20foam%20guns/TOTAL_CAULKING_GUN_9inch_THT21809.jpg',

        ],

        inStock: true,

        description: `The TOTAL THT21809 Caulking Gun is a durable manual sealant applicator designed for smooth and accurate dispensing of silicone sealants, acrylic sealants, adhesives, and construction caulks. Built with a strong steel frame and iron shank with teeth, it provides reliable thrust force for consistent material flow and precise bead control. Its ergonomic trigger handle reduces hand fatigue, making it suitable for glazing, plumbing, construction, renovation, HVAC installation, and general maintenance applications. Compact and lightweight, the THT21809 is ideal for both professional tradesmen and DIY users. :contentReference[oaicite:0]{index=0}

Items Included in the Package:
ΓÇó 1 ├ù TOTAL THT21809 Caulking Gun
ΓÇó Product Label`,

        specification: `
Category: Caulking Foam Guns
Group: Sealants and Adhesives
Brand: TOTAL
Model: THT21809
Type: Manual Caulking Gun
Size: 9" (230mm)
Length: 230mm
Diameter: 51-55mm
Frame Material: Steel
Rod Type: Iron Shank with Teeth
Thickness: 0.8mm
Suitable Cartridge Size: Standard 280mlΓÇô310ml
Application: Silicone, Sealants and Adhesives
Operation: Manual
Packing Type: Label Packed
`,

        features: `
Heavy-duty steel construction
9-inch professional caulking gun
Iron shank with teeth
Smooth trigger operation
Precise sealant dispensing
Suitable for silicone sealants
Compatible with acrylic sealants
Works with construction adhesives
Comfortable ergonomic handle
Lightweight and durable design
Easy cartridge installation
Consistent bead control
Ideal for construction and maintenance
Suitable for professional and DIY use
Reliable daily performance

Accessories:
Product Label
`,

        capacity: '280ml - 310ml',
        power: 'Manual'
    },

    {
        id: 6025,
        name: '9" Sausage Silicone Sealant & PU Sealant Caulking Gun 300ml',
        category: 'caulking-foam-guns',
        group: 'sealants and adhesives',
        brand: 'Generic',

        price: 18500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304664/Astonic-Mart/Products/caulking%20foam%20guns/Silicone_Gun_9_inch_for_300_ml.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304666/Astonic-Mart/Products/caulking%20foam%20guns/Silicone_Gun_9_inch_for_300_ml-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786304665/Astonic-Mart/Products/caulking%20foam%20guns/Silicone_Gun_9_inch_for_300_ml-2.jpg',
        ],

        inStock: true,

        description: `The 9" Sausage Silicone Sealant & PU Sealant Caulking Gun is a professional-grade applicator designed for dispensing 300ml sausage packs of silicone sealants, polyurethane sealants, adhesives, and construction joint fillers. Featuring a durable aluminum barrel construction and heavy-duty trigger mechanism, it delivers smooth, controlled extrusion with minimal effort. The rotating barrel design allows easy application around corners and confined spaces while maintaining consistent bead orientation. Ideal for glazing, curtain wall installation, window and door fitting, waterproofing, construction, and industrial sealing applications. Built for contractors, installers, and maintenance professionals requiring reliable and accurate sealant application.`,

        specification: `
Category: Caulking Foam Guns
Group: Sealants and Adhesives
Brand: Generic
Model: 300ml Sausage Caulking Gun
Type: Sausage Sealant Gun
Capacity: 300ml
Size: 9" (230mm)
Barrel Material: Aluminum Alloy
Handle Material: Aluminum Alloy
Trigger Type: Heavy-Duty Manual Trigger
Frame Type: Rotating Barrel
Rod Material: Reinforced Steel
Operation: Manual
Suitable For: Silicone Sealants, PU Sealants, Adhesives
Application: Sealing, Bonding and Joint Filling
Packing Type: Carton Box
`,

        features: `
Suitable for 300ml sausage packs
Heavy-duty aluminum barrel
Professional-grade construction
Smooth sealant dispensing
Rotating barrel design
Precise bead control
Reinforced steel push rod
Comfortable ergonomic handle
Corrosion-resistant materials
Suitable for silicone sealants
Compatible with PU sealants
Reduced material wastage
Easy cleaning and maintenance
Ideal for construction and glazing work
Long service life

Accessories:
1 ├ù Application Nozzle
1 ├ù Sausage Piston
User Guide
`,

        capacity: '300ml',
        power: 'Manual'
    },

    {
        id: 6026,
        name: 'Fischer FIS EM Plus 390 S Epoxy Injection Mortar',
        category: 'silicone-sealants',
        group: 'sealants and adhesives',
        brand: 'Fischer',
        price: 65000,
        discountPrice: 58000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329160/Astonic-Mart/Products/silicone%20and%20adhesives/fischer_FIS_V_Plus_390_S-1.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329159/Astonic-Mart/Products/silicone%20and%20adhesives/fischer_FIS_V_Plus_390_S-2.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329161/Astonic-Mart/Products/silicone%20and%20adhesives/fischer_FIS_V_Plus_390_S-3.webp'
        ],

        model: 'FIS EM Plus 390 S',

        inStock: true,

        description: 'Fischer FIS EM Plus 390 S is a high-performance epoxy injection mortar designed for heavy-duty anchoring and structural fixing applications. It provides exceptional load-bearing capacity in cracked and non-cracked concrete, making it ideal for rebar connections, threaded rod installations, steel structures, and demanding construction projects. Suitable for indoor and outdoor use with excellent durability and chemical resistance.',

        specification: `
  Model: FIS EM Plus 390 S
  Product Type: Epoxy Injection Mortar
  Cartridge Capacity: 390ml
  Base Material: Epoxy Resin
  Application Temperature: Down to -5┬░C
  Suitable Substrates:
  - Cracked Concrete
  - Non-Cracked Concrete
  - Natural Stone
  - Reinforced Concrete
  Service Life: Up to 100 Years
  Rebar Diameter Range: 8mm - 40mm
  Installation Method: Injection System
  Indoor & Outdoor Use: Yes
  Chemical Resistance: Excellent

  Accessories Included:
  1 x 390ml Cartridge
  2 x Static Mixers
  User Instructions
  `,

        features: `
  High-performance epoxy anchoring system
  Exceptional load-bearing capacity
  Suitable for cracked and non-cracked concrete
  Approved for rebar and threaded rod installations
  Excellent resistance to moisture and chemicals
  Can be applied in low temperatures
  Long service life up to 100 years
  Suitable for heavy steel constructions
  Ideal for structural and industrial applications
  Indoor and outdoor performance
  `,


        showSimilar: true
    },

    {
        id: 6027,
        name: 'Soudal Mitre Kit 50g + 200ml',
        category: 'glues',
        group: 'sealants and adhesives',
        brand: 'Soudal',
        price: 15500,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429759/Astonic-Mart/Products/remaining-local-images/hlw3oryvym6ckiuxskdc.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429761/Astonic-Mart/Products/remaining-local-images/nhplmlwczsb4uyz83qny.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429763/Astonic-Mart/Products/remaining-local-images/nxfzewkkbuuqarw6ivjh.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429765/Astonic-Mart/Products/remaining-local-images/x4r5shucuo5l3rdsmrji.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429767/Astonic-Mart/Products/remaining-local-images/wptfme0pqmtf6jox7hwv.jpg'
        ],

        model: 'Mitre Kit 2C',

        inStock: true,

        description: 'Soudal Mitre Kit is a fast-acting two-component adhesive system consisting of a high-viscosity cyanoacrylate adhesive and an aerosol activator. It is designed for rapid bonding of wood, MDF, PVC, rubber, metals, and most plastics, making it ideal for furniture production, cabinet assembly, trim installation, and general repair applications. Curing occurs almost instantly after joining the surfaces.',

        specification: `
  Model: Mitre Kit 2C
  Product Type: Two-Component Instant Adhesive
  Adhesive Type: High Viscosity Cyanoacrylate
  Activator Type: Aerosol Spray
  Adhesive Content: 50g
  Activator Content: 200ml
  Color: Clear
  Open Time: Approx. 4 Seconds
  Drying Time: Approx. 1 Minute
  Temperature Resistance: -40┬░C to +120┬░C
  Application Temperature: +15┬░C to +25┬░C

  Suitable Materials:
  - MDF
  - Wood
  - Chipboard
  - PVCu
  - Rubber
  - Leather
  - Metals
  - Most Plastics

  Accessories Included:
  50g Cyanoacrylate Adhesive
  200ml Aerosol Activator
  `,

        features: `
  Two-part instant bonding system
  High bond strength
  Fast curing performance
  Suitable for vertical surfaces
  High-viscosity adhesive reduces running
  Activator reduces blooming effect
  Activator reduces odour
  Ideal for furniture and joinery work
  Bonds wood, MDF, PVC, rubber and metals
  Easy-to-use professional adhesive kit
  `,

        accessories: `
  50g Cyanoacrylate Adhesive
  200ml Aerosol Activator
  `,

        showSimilar: true
    },


    {
        id: 6028,
        name: 'Soudal Mitre Kit 100g + 400ml',
        category: 'glues',
        group: 'sealants and adhesives',
        brand: 'Soudal',
        price: 18250,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429768/Astonic-Mart/Products/remaining-local-images/farslkqzrfsga2jruelq.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429769/Astonic-Mart/Products/remaining-local-images/slutitqpp8mu0ayqt5vi.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786429771/Astonic-Mart/Products/remaining-local-images/kc342lk6njyvobygbvso.webp'
        ],

        model: 'Mitre Kit',
        showSimilar: true,
        inStock: true,

        description: 'Soudal Mitre Kit is a professional two-component instant adhesive system comprising a cyanoacrylate adhesive and an aerosol activator. Designed for rapid bonding of MDF, wood, PVC, rubber, plastics, and other materials, it provides strong and reliable joints within seconds, making it ideal for furniture manufacturing, joinery, trim work, and repair applications.',

        specification: `
  Product Type: Mitre Adhesive Kit
  Adhesive Type: Cyanoacrylate
  Adhesive Content: 100g
  Activator Content: 400ml
  Color: Clear
  Curing System: Activator Assisted
  Initial Bond Time: 4-10 Seconds
  Full Cure Time: Approximately 1 Minute
  Temperature Resistance: -40┬░C to +120┬░C

  Suitable Materials:
  - MDF
  - Wood
  - Chipboard
  - PVC
  - Rubber
  - Leather
  - Metal
  - Most Plastics
  `,

        features: `
  Fast instant bonding
  Professional-grade adhesive system
  Strong and durable bond strength
  High-viscosity adhesive reduces dripping
  Suitable for porous and non-porous materials
  Excellent for furniture and cabinet assembly
  Quick curing with activator spray
  Easy application process
  Indoor and outdoor use
  Ideal for woodworking and joinery
  `
    },

    {
        id: 6029,
        name: 'Pidilite M-Seal Regular Epoxy Compound',
        category: 'epoxy-adhesives',
        group: 'sealants and adhesives',
        brand: 'Pidilite',
        price: 8500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329166/Astonic-Mart/Products/silicone%20and%20adhesives/pidilite_m_seal_regular_epoxy_compound.webp',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329166/Astonic-Mart/Products/silicone%20and%20adhesives/pidilite_m_seal_regular_epoxy_compound-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329166/Astonic-Mart/Products/silicone%20and%20adhesives/pidilite_m_seal_regular_epoxy_compound-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329167/Astonic-Mart/Products/silicone%20and%20adhesives/pidilite_m_seal_regular_epoxy_compound-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329181/Astonic-Mart/Products/silicone%20and%20adhesives/pidilite_m_seal_regular_epoxy_compound-5.jpg'
        ],

        model: 'M-Seal Regular',

        inStock: true,

        description: 'Pidilite M-Seal Regular is a two-component epoxy putty designed for sealing, joining, fixing, building, and repairing a wide range of materials. Once mixed, it forms a strong and durable bond that can be drilled, sanded, filed, machined, and painted after curing. It is suitable for household repairs, plumbing maintenance, automotive applications, and industrial use.',

        specification: `
  Product Type: Epoxy Putty Compound
  Brand: Pidilite
  Model: M-Seal Regular
  Component Type:
  - Resin Base
  - Hardener
  Mixing Ratio: 1:1
  Color:
  - Green Resin
  - Black Hardener
  Setting Time: Approximately 30 Minutes
  Full Cure Time: Approximately 60-90 Minutes
  Water Resistance: Yes
  Paintable: Yes
  Sandable: Yes
  Drillable: Yes

  Suitable Materials:
  - Metal
  - Wood
  - Glass
  - Ceramic
  - Marble
  - Concrete
  - Porcelain
  - Certain Plastics

  Available Pack Sizes:
  - 25g
  - 60g
  - 90g
  - 250g
  - 1kg
  `,

        features: `
  Two-part epoxy repair compound
  Strong and durable bond
  Seals, joins, fixes and builds
  Waterproof after curing
  Can be drilled, filed and sanded
  Paintable after curing
  Excellent gap-filling properties
  Good electrical insulation properties
  Suitable for indoor and outdoor repairs
  Easy hand-mix application
  `
    },

    {
        id: 6030,
        name: 'PU 988 Polyurethane Adhesive Sealant 310ml Black',
        category: 'polyurethane-sealants',
        group: 'sealants and adhesives',
        brand: 'PU 988',
        price: 9000,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329256/Astonic-Mart/Products/silicone%20and%20adhesives/TCM_PU9688_Glazing_Sealant.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329256/Astonic-Mart/Products/silicone%20and%20adhesives/TCM_PU9688_Glazing_Sealant-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329257/Astonic-Mart/Products/silicone%20and%20adhesives/TCM_PU9688_Glazing_Sealant-3.jpg'
        ],

        model: 'PU 988',
        capacity: '310ml',

        inStock: true,

        description: 'PU 988 is a one-component moisture-curing polyurethane adhesive sealant designed for sealing, bonding, and glazing applications. It provides excellent adhesion to metal, glass, concrete, wood, painted surfaces, and most construction materials. The cured sealant remains flexible, weather-resistant, and durable, making it ideal for automotive, marine, construction, and industrial applications.',

        specification: `
  Model: PU 988
  Product Type: Polyurethane Adhesive Sealant
  Color: Black
  Capacity: 310ml
  Curing Type: Moisture Cure
  Skin Formation Time: 20-60 Minutes
  Hardness: Shore A 40-45
  Elongation at Break: ΓëÑ400%
  Temperature Resistance: -40┬░C to +90┬░C
  Application Temperature: +5┬░C to +35┬░C
  Paintable: Yes
  UV Resistant: Yes
  Waterproof: Yes

  Suitable Materials:
  - Metal
  - Glass
  - Concrete
  - Wood
  - Aluminum
  - Fiberglass
  - Ceramics
  - Painted Surfaces
  `,

        features: `
  Strong adhesive and sealing performance
  Permanent elastic flexibility
  Excellent weather resistance
  Waterproof and UV resistant
  Vibration and shock resistant
  Paintable after curing
  Non-sag formulation
  Suitable for indoor and outdoor applications
  Excellent adhesion to most construction materials
  Ideal for automotive body sealing and glazing
  `,

        showSimilar: true
    },

    {
        id: 6031,
        name: 'Bostik Flashband Self-Adhesive Flashing Tape 10m x 3"',
        category: 'adhesive-tapes',
        group: 'sealants and adhesives',
        brand: 'Bostik',
        model: 'Flashband 10m x 3in',

        price: 18000,


        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329539/Astonic-Mart/Products/tapes%20and%20adhesives/Bostik_Flashband_Tape_10m_x_3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329539/Astonic-Mart/Products/tapes%20and%20adhesives/Bostik_Flashband_Tape_10m_x_3-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329538/Astonic-Mart/Products/tapes%20and%20adhesives/Bostik_Flashband_Tape_10m_x_3-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329540/Astonic-Mart/Products/tapes%20and%20adhesives/Bostik_Flashband_Tape_10m_x_3-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329541/Astonic-Mart/Products/tapes%20and%20adhesives/Bostik_Flashband_Tape_10m_x_3-5.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329545/Astonic-Mart/Products/tapes%20and%20adhesives/Bostik_Flashband_Tape_10m_x_3-6.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329545/Astonic-Mart/Products/tapes%20and%20adhesives/Bostik_Flashband_Tape_10m_x_3-7.jpg'
        ],

        inStock: true,

        description: `Bostik Flashband Self-Adhesive Flashing Tape is a durable, waterproof sealing tape designed for repairing, sealing, and protecting roofs, gutters, skylights, flashings, and other exterior surfaces. Manufactured with a reinforced aluminium foil backing and a strong bitumen-based adhesive, it provides an instant watertight seal without the need for heat or special tools. The tape conforms easily to irregular surfaces and delivers long-lasting resistance against rain, moisture, UV exposure, and weathering. It is ideal for emergency roof repairs, sealing leaks, and general maintenance applications in residential, commercial, and industrial environments.`,

        specification: `
  Product Type: Self-Adhesive Flashing Tape
  Brand: Bostik
  Model: Flashband 10m x 3in
  Length: 10m
  Width: 3 Inches (75mm)
  Backing Material: Reinforced Aluminium Foil
  Adhesive Type: Bitumen-Based Self-Adhesive
  Colour: Aluminium Silver
  Waterproof: Yes
  UV Resistant: Yes
  Weather Resistant: Yes
  Application: Roof Repairs, Gutters, Flashings, Skylights and General Sealing
  Surface Compatibility: Metal, Concrete, Brick, Slate, Asphalt, Roofing Felt, Wood and PVC
  Installation Method: Peel and Stick

  Accessories Included:
  - 1 x Roll Bostik Flashband Self-Adhesive Flashing Tape (10m x 3in)
  `,

        features: `
  Instant waterproof sealing solution
  Strong self-adhesive backing
  No heat or special tools required
  Flexible and easy to apply
  Excellent weather resistance
  UV-resistant aluminium surface
  Conforms to irregular shapes and surfaces
  Long-lasting outdoor performance
  Ideal for roof and gutter repairs
  Suitable for professional and DIY applications
  `
    },

    {
        id: 6032,
        name: 'General Purpose PU Foam',
        category: 'pu-foam-sealants',
        group: 'sealants and adhesives',
        brand: 'Generic',

        price: 11500,
        inStock: true,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329158/Astonic-Mart/Products/silicone%20and%20adhesives/general_purpose_pu_foam.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329163/Astonic-Mart/Products/silicone%20and%20adhesives/general_purpose_pu_foam-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329163/Astonic-Mart/Products/silicone%20and%20adhesives/general_purpose_pu_foam-3.jpg'
        ],

        description: `
  General Purpose Polyurethane (PU) Foam is a one-component,
  moisture-curing expanding foam designed for filling, sealing,
  insulating, and bonding applications. It adheres to most common
  building materials and provides excellent thermal and sound insulation.
  `,

        specification: `
  Product Type: Polyurethane Expanding Foam
  Application Method: Aerosol Spray
  Foam Type: One-Component PU Foam
  Color: Light Yellow
  Curing Method: Moisture Curing
  Expansion: High Expansion
  Insulation: Thermal and Acoustic
  Paintable: Yes (After Curing)
  Operating Temperature: As Recommended by Manufacturer
  Usage: Indoor and Outdoor
  `,

        features: `
  ΓÇó Excellent gap-filling capability
  ΓÇó Strong adhesion to most construction materials
  ΓÇó Provides thermal insulation
  ΓÇó Reduces sound transmission
  ΓÇó Easy application with spray nozzle
  ΓÇó Fast curing and durable performance
  ΓÇó Suitable for sealing doors and windows
  ΓÇó Moisture and weather resistant after curing
  `,


        showSimilar: true
    },

    {
        id: 6033,
        name: 'SUCCL Multi-Purpose PU Foam Sealant',
        category: 'pu-foam-sealants',
        group: 'sealants and adhesives',
        brand: 'Sea Unique',

        price: 11500,
        inStock: true,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329201/Astonic-Mart/Products/silicone%20and%20adhesives/sea_unik_pu_foam.jpg'
        ],

        description: `
  SUCCL Multi-Purpose PU Foam Sealant is a one-component,
  moisture-curing polyurethane expanding foam designed for
  sealing, filling, insulating, and bonding applications.
  It provides excellent adhesion to most building materials
  and helps improve thermal and sound insulation.
  `,

        specification: `
  Brand: SUCCL
  Product Type: Multi-Purpose PU Foam Sealant
  Foam Type: Polyurethane Expanding Foam
  Application: Gap Filling and Sealing
  Curing Method: Moisture Curing
  Color: Light Yellow (After Dispensing)
  Expansion: High Expansion
  Insulation: Thermal and Acoustic
  Paintable: Yes (After Full Cure)
  Usage: Indoor and Outdoor
  Packaging: Aerosol Can
  `,

        features: `
  ΓÇó Expands to fill gaps and cavities
  ΓÇó Excellent adhesion to common building materials
  ΓÇó Provides thermal insulation
  ΓÇó Helps reduce noise transmission
  ΓÇó Easy-to-use aerosol application
  ΓÇó Fast curing performance
  ΓÇó Suitable for door and window installations
  ΓÇó Moisture-resistant after curing
  ΓÇó Ideal for construction and maintenance work
  `,


        showSimilar: true
    },

    {
        id: 6034,
        name: 'SKY-7000 Silicone Weatherproof Adhesive Sealant',
        category: 'silicone-sealants',
        group: 'sealants and adhesives',
        brand: 'SKY',

        price: 9500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329201/Astonic-Mart/Products/silicone%20and%20adhesives/SKY-7000_Silicone_one_Weatherproof_Adhesive.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329201/Astonic-Mart/Products/silicone%20and%20adhesives/SKY-7000_Silicone_one_Weatherproof_Adhesive.jp-2.jpg'
        ],

        inStock: true,

        jumiaLink: '',
        kongaLink: '',


        description: `SKY-7000 Silicone Weatherproof Sealant is a one-component neutral curing silicone sealant supplied in a sausage pack for professional glazing, curtain wall, roofing, window, and construction sealing applications. It provides excellent adhesion, flexibility, UV resistance, and long-term weatherproof performance for both indoor and outdoor environments.`,

        specification: `
- Product Name: SKY-7000 Silicone Weatherproof Sealant
- Brand: SKY
- Type: Neutral Cure Silicone Sealant
- Packaging: Sausage Pack
- Net Weight: 590ml / 600ml
- Application: Weatherproofing and Joint Sealing
- Curing Type: Moisture Cure
- UV Resistant: Yes
- Waterproof: Yes
- Movement Capability: High Flexibility
- Temperature Resistance: -40┬░C to +150┬░C
- Indoor/Outdoor Use: Yes

Suitable For:
- Curtain Walls
- Glass Glazing
- Aluminum Panels
- Windows
- Doors
- Roofing Systems
- Expansion Joints
- Building Facades

Accessories Included:
- 1 x SKY-7000 Silicone Sealant Sausage
  `,

        features: `
- Neutral cure silicone technology
- Excellent weather resistance
- UV resistant formulation
- Waterproof sealing performance
- Strong adhesion to building materials
- Flexible and durable after curing
- Suitable for professional construction applications
- Low odor formulation
- Crack-resistant seal
- Long service life
  `
    },


    {
        id: 6035,
        name: 'SKY-9000 Silicone Weatherproof Sealant (Sausage Type)',
        category: 'silicone-sealants',
        group: 'sealants and adhesives',
        brand: 'SKY',

        price: 10500,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329208/Astonic-Mart/Products/silicone%20and%20adhesives/SKY-9000_Silicone_one_Weatherproof_Adhesive.jp-2.png'
        ],

        inStock: true,

        description: `SKY-9000 Silicone Weatherproof Sealant is a premium-grade neutral curing silicone sealant designed for demanding structural glazing, curtain wall, fa├ºade, roofing, and weatherproof sealing applications. Supplied in a convenient sausage pack, it offers superior adhesion, elasticity, UV resistance, and long-lasting weather protection in commercial and industrial construction projects.`,

        specification: `
- Product Name: SKY-9000 Silicone Weatherproof Sealant
- Brand: SKY
- Type: Premium Neutral Cure Silicone Sealant
- Packaging: Sausage Pack
- Net Weight: 590ml / 600ml
- Application: Structural and Weatherproof Sealing
- Curing Type: Moisture Cure
- UV Resistant: Yes
- Waterproof: Yes
- Weather Resistant: Excellent
- Temperature Resistance: -50┬░C to +180┬░C
- Indoor/Outdoor Use: Yes

Suitable For:
- Structural Glazing
- Curtain Wall Systems
- Aluminum Composite Panels
- Glass Facades
- Skylights
- Roofing Applications
- Expansion Joints
- Commercial Buildings

Accessories Included:
- 1 x SKY-9000 Silicone Sealant Sausage
  `,

        features: `
- Premium weatherproof silicone sealant
- Excellent adhesion to glass and metal
- Superior UV resistance
- Long-lasting waterproof performance
- High flexibility and elasticity
- Crack and shrink resistant
- Suitable for structural glazing applications
- Excellent aging resistance
- Durable outdoor performance
- Professional construction-grade formulation
- Low odor and easy application
- Designed for commercial and industrial projects
  `
    },


    {
        id: 6036,
        name: "Rongtai RT-308 Silicon Rubber Sealant 300ML",
        category: 'silicone-sealants',
        group: 'sealants and adhesives',
        brand: "Rongtai",
        price: 8000, // Update with your selling price


        images: [
            "https://res.cloudinary.com/taus5tit/image/upload/v1786329171/Astonic-Mart/Products/silicone%20and%20adhesives/Rongtai_RT-308_Silicon_Rubber_Sealant_300ML.jpg",
            "https://res.cloudinary.com/taus5tit/image/upload/v1786329200/Astonic-Mart/Products/silicone%20and%20adhesives/Rongtai_RT-308_Silicon_Rubber_Sealant_300ML-2.png",

        ],

        description: `
The Rongtai RT-308 Silicone Rubber Sealant is a premium one-component RTV (Room Temperature Vulcanizing) silicone sealant designed for sealing, bonding, and waterproofing a wide range of construction and industrial materials. It cures upon exposure to atmospheric moisture, forming a durable, flexible rubber seal with excellent resistance to water, weather, UV rays, and temperature changes. Ideal for glass, aluminum, ceramic, plastic, metal, windows, doors, kitchens, bathrooms, and general construction applications.
`,

        specification: `
Brand: Rongtai
Model: RT-308
Product Type: RTV Silicone Rubber Sealant
Volume: 300 ml
Sealant Type: One-Component Silicone
Curing Method: Moisture Cure (Room Temperature Vulcanizing)
Appearance: Smooth Paste
Application Method: Standard Caulking Gun
Skin Formation Time: 30ΓÇô60 Minutes
Full Cure Time: 24ΓÇô48 Hours
Temperature Resistance: High Temperature Resistant
Water Resistance: Waterproof
Mold Resistance: Yes
UV Resistance: Yes
Weather Resistance: Excellent
Flexibility: High
Adhesion: Excellent

Suitable Materials:
ΓÇó Glass
ΓÇó Aluminium
ΓÇó Ceramic
ΓÇó Tiles
ΓÇó Stainless Steel
ΓÇó Plastic
ΓÇó Concrete
ΓÇó Painted Surfaces
ΓÇó Most Non-Porous Building Materials

Common Applications:
ΓÇó Window Installation
ΓÇó Door Frames
ΓÇó Glass Panels
ΓÇó Bathroom & Kitchen Sealing
ΓÇó Sink & Bathtub Joints
ΓÇó Mirror Installation
ΓÇó Roof Flashing
ΓÇó Wall Cracks & Gaps
ΓÇó Electrical Insulation
ΓÇó General Construction Sealing

Accessories Included:
ΓÇó 300 ml Cartridge
ΓÇó Plastic Nozzle
`,

        features: `
ΓÇó One-component RTV silicone sealant
ΓÇó Strong adhesion to most construction materials
ΓÇó Waterproof and weatherproof
ΓÇó Mold and mildew resistant
ΓÇó UV resistant for outdoor applications
ΓÇó Flexible after curing
ΓÇó Excellent crack-filling capability
ΓÇó High temperature resistance
ΓÇó Easy cartridge application
ΓÇó Suitable for indoor and outdoor use
`,


        model: "RT-308",
        capacity: "300 ml",

        inStock: true,
        showSimilar: true
    },

    {
        id: 6037,
        name: 'Soudal Fire Rated Expanding Foam',
        category: 'expanding-foam',
        group: 'sealants and adhesives',
        brand: 'Soudal',
        model: 'Fire Rated PU Foam',
        price: 35000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329224/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_Fire_Rated_Expanding_Foam-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329201/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_Fire_Rated_Expanding_Foam.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329224/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_Fire_Rated_Expanding_Foam-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329224/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_Fire_Rated_Expanding_Foam-4.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329224/Astonic-Mart/Products/silicone%20and%20adhesives/Soudal_Fire_Rated_Expanding_Foam-5.jpg'
        ],



        description: 'Soudal Fire Rated Expanding Foam is a one-component, self-expanding polyurethane foam specially developed for fire-resistant sealing applications. It is designed to seal joints, gaps, penetrations and cavities where fire protection is required. The foam provides excellent thermal and acoustic insulation while maintaining high adhesion to most building materials.',

        specification: `
Brand: Soudal
Model: Fire Rated Expanding Foam
Type: Fire Resistant Polyurethane Foam
Capacity: 750ml
Color: Pink/Red
Application Method: Foam Gun or Straw Applicator
Base Material: Polyurethane
Fire Resistance: Up to 4 Hours (depending on joint configuration)
Density: Approx. 20ΓÇô25 kg/m┬│
Tack Free Time: 8ΓÇô12 Minutes
Cutting Time: 30ΓÇô60 Minutes
Curing Time: 24 Hours
Temperature Resistance: -40┬░C to +90┬░C
Application Temperature: +5┬░C to +35┬░C
Expansion Type: Controlled Expansion
Sound Insulation: Excellent
Thermal Insulation: High Performance
Suitable Surfaces:
Concrete
Brick
Masonry
Metal
Wood
PVC

Accessories:
Applicator Nozzle
Protective Cap
User Instructions
`,

        features: `
ΓÇó Fire resistant sealing foam
ΓÇó Provides up to 4 hours fire protection
ΓÇó Excellent adhesion to most construction materials
ΓÇó High thermal insulation performance
ΓÇó Good acoustic insulation properties
ΓÇó Controlled expansion for precise filling
ΓÇó Fast curing and easy trimming
ΓÇó Suitable for sealing service penetrations
ΓÇó Ideal for fire doors, walls and partitions
ΓÇó Professional construction-grade quality
`,

        capacity: '750ml',

        inStock: true,
        showSimilar: true,

        jumiaLink: '',
        kongaLink: ''
    },

    {
        id: 6038,
        name: 'Fevicol 1K PUR Adhesive 1kg - 6 Pieces',
        category: 'glues',
        group: 'sealants and adhesives',
        brand: 'Fevicol',
        model: '1K PUR',
        price: 40000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329151/Astonic-Mart/Products/silicone%20and%20adhesives/Fevicol_1K_PUR_Adhesive_1kg_-_6_Pieces.png'
        ],

        description: 'The Fevicol 1K PUR Adhesive is a premium single-component moisture-curing polyurethane adhesive designed for industrial bonding applications. It provides exceptional bonding strength, high water resistance, superior heat resistance and excellent durability for wood, metal, laminates, plastics and construction materials. This package contains six 1kg cartridges suitable for professional woodworking, furniture production, doors, windows and panel fabrication.',

        specification: `
Brand: Fevicol
Model: 1K PUR
Type: Polyurethane Adhesive
Packaging: Cartridge
Quantity: 6 Pieces
Weight per Tube: 1kg
Total Weight: 6kg
Colour: Light Brown
Adhesive Type: Single Component Moisture Curing PUR
Open Time: 10ΓÇô15 Minutes
Handling Time: 2ΓÇô3 Hours
Full Cure Time: 24 Hours
Application Temperature: 15┬░C ΓÇô 45┬░C
Water Resistance: EN 204 D4 Standard
Heat Resistance: WATT 91 Standard
Coverage: 8ΓÇô9m┬▓/kg
Suitable Materials:
Wood
MDF
Particle Board
Plywood
Laminate
Metal
Stone
Ceramic
Foam
Fiberglass
Rubber
Concrete
Masonry

Accessories:
6 x Fevicol 1K PUR Adhesive Tubes
`,

        features: `
ΓÇó High-strength polyurethane adhesive
ΓÇó Moisture-curing single-component formula
ΓÇó Excellent water resistance (D4 grade)
ΓÇó High heat resistance
ΓÇó Suitable for interior and exterior applications
ΓÇó Strong bonding on wood, metal and laminates
ΓÇó Fast setting and durable performance
ΓÇó Gap-filling capability
ΓÇó Resistant to weather exposure
ΓÇó Ideal for industrial woodworking applications
ΓÇó Professional-grade adhesive solution
`,

        inStock: true,
        showSimilar: true,

        jumiaLink: '',
        kongaLink: ''
    },

    {
        id: 6039,
        name: 'Fevicol 1K PUR Adhesive 1kg',
        category: 'glues',
        group: 'sealants and adhesives',
        brand: 'Fevicol',
        model: '1K PUR',
        price: 29000,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329136/Astonic-Mart/Products/silicone%20and%20adhesives/Fevicol_1K_PUR_Adhesive_1kg-1.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329136/Astonic-Mart/Products/silicone%20and%20adhesives/Fevicol_1K_PUR_Adhesive_1kg-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329136/Astonic-Mart/Products/silicone%20and%20adhesives/Fevicol_1K_PUR_Adhesive_1kg.webp'
        ],

        description: 'Fevicol 1K PUR Adhesive is a premium moisture-curing polyurethane adhesive formulated for high-strength industrial bonding applications. It offers excellent adhesion, superior water resistance, heat resistance and long-lasting performance on wood, laminates, metals and composite materials.',

        specification: `
Brand: Fevicol
Model: 1K PUR
Type: Polyurethane Adhesive
Weight: 1kg
Technology: Moisture Curing Polyurethane
Colour: Light Brown
Consistency: Thick Liquid
Water Resistance: D4 Grade
Heat Resistance: High Temperature Resistant
Open Time: 10ΓÇô15 Minutes
Curing Time: Approximately 24 Hours
Application: Industrial Bonding

Suitable Materials:
Wood
MDF
Plywood
Particle Board
Laminate
Aluminium
Metal
PVC
Composite Panels
Stone

Accessories:
1kg Adhesive Bottle
Application Nozzle
`,

        features: `
ΓÇó High-strength polyurethane adhesive
ΓÇó Moisture-curing technology
ΓÇó Excellent waterproof properties
ΓÇó Superior heat resistance
ΓÇó Strong bonding on wood and laminates
ΓÇó Ideal for furniture production
ΓÇó Fast curing performance
ΓÇó Long-lasting adhesion
ΓÇó Suitable for indoor and outdoor applications
ΓÇó Professional industrial-grade formula
`,

        inStock: true,
        showSimilar: true,

        jumiaLink: '',
        kongaLink: ''
    },


    {
        id: 6040,
        name: 'ABRO Leakage Repair Waterproof Tape-2"',
        group: 'sealants and adhesives',
        category: 'adhesive-tapes',
        brand: 'ABRO',
        price: 6500,
        showSimilar: true,

        images: [
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329124/Astonic-Mart/Products/silicone%20and%20adhesives/ABRO_Leakage_Repair_Waterproof_Tape-1.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329126/Astonic-Mart/Products/silicone%20and%20adhesives/ABRO_Leakage_Repair_Waterproof_Tape-2.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329126/Astonic-Mart/Products/silicone%20and%20adhesives/ABRO_Leakage_Repair_Waterproof_Tape-3.jpg',
            'https://res.cloudinary.com/taus5tit/image/upload/v1786329126/Astonic-Mart/Products/silicone%20and%20adhesives/ABRO_Leakage_Repair_Waterproof_Tape-4.jpg'
        ],

        inStock: true,

        description: 'The ABRO Leakage Repair Waterproof Tape is a heavy-duty self-adhesive sealing tape designed for quickly repairing leaks, cracks, joints, and damaged surfaces. Made with a butyl rubber adhesive and a durable aluminum foil backing, it provides a long-lasting waterproof, weatherproof, and airtight seal on roofs, pipes, gutters, tanks, windows, and other indoor or outdoor surfaces. It is easy to apply by simply peeling off the backing and firmly pressing it onto a clean surface.',

        specification: `
Product Type: Leakage Repair Waterproof Tape
Brand: ABRO
Material: Butyl Rubber with Aluminum Foil
Tape Width: 50mm (5cm)
Tape Length: 5m
Color: Silver
Backing: Aluminum Foil
Adhesive Type: Butyl Rubber
Waterproof: Yes
Weather Resistant: Yes
UV Resistant: Yes
Application: Roofs, Pipes, Gutters, Water Tanks, Metal Sheets, PVC, Concrete, Glass, Wood
  `,

        features: `
Provides an instant waterproof and airtight seal
Strong butyl rubber adhesive for long-lasting performance
Aluminum foil backing resists UV rays and harsh weather
Ideal for repairing roof leaks, pipe leaks, gutters, and tanks
Excellent adhesion to metal, plastic, concrete, glass, and wood
Easy peel-and-stick installation without special tools
Flexible design conforms to uneven surfaces
Suitable for both indoor and outdoor applications
  `,

        jumiaLink: 'https://www.jumia.com.ng/',
        kongaLink: 'https://www.konga.com/'
    },


]

